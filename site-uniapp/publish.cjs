...
const publish = () => {
    ...
    const dest = '../niucloud/public/adminapp'
...
const solve = () => {
    ...
    filemaps.forEach(file => {
        if (/^(index-)(\w{8})(.js)$/.test(file)) {
            ...
            const first = 'const match = location.href.match(/\\/adminapp\\/(\\d*)\\//);'

            if (content.indexOf(first) == -1) {
                content = first + content
                const replace = 'router:{mode:"history",base: match ? `/adminapp/${match[1]}/` : "/adminapp/",assets:"assets",routerBase: match ? `/adminapp/${match[1]}/` : "/adminapp/"},darkmode'
