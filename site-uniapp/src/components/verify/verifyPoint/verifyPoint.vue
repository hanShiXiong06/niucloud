...
const props = defineProps({
    ...
    // 新增：兼容原watch的type字段（原代码watch type但props未定义）
    type: {
        type: String,
        default: 'point'
    }
...
const checkNum = ref(3); // 需点击的点数
const fontPos = ref<Array<any>>([]); // 选中的坐标信息
const checkPosArr = ref<Array<any>>([]); // 用户点击的坐标
const num = ref(1); // 点击的记数
const pointBackImgBase = ref(''); // 后端获取到的背景图片
const poinTextList = ref<Array<any>>([]); // 后端返回的点击字体顺序
const backToken = ref(''); // 后端返回的token值
const imgRand = ref(0); // 随机的背景图片
const showImage = ref(true);
const tempPoints = ref<Array<any>>([]);
const text = ref('');
...
const canvasClick = (e: any) => {
    ...
    query.select('#image').boundingClientRect((data: any) => {
        ...
        imgLeft.value = Math.ceil(data.left).toString();
        imgTop.value = Math.ceil(data.top).toString();
        ...
        if (num.value === checkNum.value) {
            ...
            setTimeout(() => {
                // 构造加密参数
                const captchaData = JSON.stringify(checkPosArr.value);
...
const getMousePos = (e: any) => {
    return {
        x: Math.ceil(e.detail.x) - Number(imgLeft.value),
        y: Math.ceil(e.detail.y) - Number(imgTop.value)
    };
};
