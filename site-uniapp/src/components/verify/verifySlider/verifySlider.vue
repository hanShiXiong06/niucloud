...
// 获取组件实例（替代this.$parent）
const instance = getCurrentInstance();

// 响应式数据
const secretKey = ref(''); // 后端返回的加密秘钥
...
const init = () => {
    ...
    getPictrue();
    nextTick(() => {
        instance?.parent?.emit('ready', instance);
    });
};

// 触摸开始
const start = (e: any) => {
    startMoveTime.value = new Date().getTime().toString(); // 开始滑动的时间
    if (!isEnd.value) {
        text.value = '';
...
const move = (e: any) => {
    ...
    barArea.boundingClientRect((data: any) => {
        if (!data) return;
        
        const bar_area_left = Math.ceil(data.left);
        const barArea_offsetWidth = Math.ceil(data.width);
        ...
        if (status.value && !isEnd.value) {
            ...
            // 图片滑动类型的边界处理
            if (props.type !== '1') {
                if (move_block_left >= barArea_offsetWidth - parseInt(props.blockSize.width) / 2 - 2) {
                    move_block_left = barArea_offsetWidth - parseInt(props.blockSize.width) / 2 - 2;
                }
            }

            if (move_block_left <= 0) {
                move_block_left = parseInt(props.blockSize.width) / 2;
            }

            // 更新滑块位置
            moveBlockLeft.value = `${move_block_left - parseInt(props.blockSize.width) / 2}px`;
            leftBarWidth.value = `${move_block_left - parseInt(props.blockSize.width) / 2}px`;
        }
    }).exec();
};
...
// 触摸结束
const end = () => {
    endMovetime.value = new Date().getTime().toString();
  
    if (status.value && !isEnd.value) {
        if (props.type !== '1') { // 图片滑动验证
            const moveLeftDistance = parseInt(moveBlockLeft.value?.replace('px', '') || '0');
            // 计算实际滑动距离（适配不同宽度）
            const realMoveLeft = moveLeftDistance * 310 / parseInt(props.imgSize.width);

            // 构造加密参数
            const captchaData = JSON.stringify({ x: realMoveLeft, y: 5.0 });
            const captchaVerification = secretKey.value 
                ? aesEncrypt(`${backToken.value}---${captchaData}`, secretKey.value)
                : `${backToken.value}---${captchaData}`;

            // 验证请求
            myRequest({
                url: '/captcha/check',
                data: {
                captcha_type: props.captchaType,
                captcha_code: secretKey.value ? aesEncrypt(captchaData, secretKey.value) : captchaData,
                ...
                if (res.code === 1) {
                    ...
                    isEnd.value = true;
                    passFalg.value = true;
                    tipWords.value = `${((Number(endMovetime.value) - Number(startMoveTime.value)) / 1000).toFixed(2)}s验证成功`;

                    // 延迟关闭/刷新
                    setTimeout(() => {
                        if (props.mode === 'pop') {
                            instance?.parent?.emit('close'); // 替代原this.$parent.clickShow = false
                        }
                        refresh();
                    }, 1000);
