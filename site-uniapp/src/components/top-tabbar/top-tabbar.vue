        ...
        <view class="u-navbar" :class="{'fixed': props.scrollBool != -1, 'absolute': props.scrollBool == -1}" :style="{ backgroundColor: bgColor}">
            <view class="navbar-inner" :style="{ width: '100%', height: placeholderHeight + 'px' }">
                ...
                <view v-if="topStatusBarData.style == 'style-4'" :style="navbarInnerStyle" class="content-wrap">
                    ...
                    <view class="title-wrap" @click.stop="locationVal.reposition()" :style="{ color: topStatusBarData.textColor }" v-if="systemStore.diyAddressInfo">{{ systemStore.diyAddressInfo.city }}</view>
...
const goBack = () => {
    ...
    if (pages.length === 1) {
        if (pages[0].route === 'app/pages/auth/login') {
            uni.getStorage({
                key: 'loginBack',
                success: (res: any) => {
                    res ? redirect(
