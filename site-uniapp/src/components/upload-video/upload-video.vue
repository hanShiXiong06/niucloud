<template>
    <view class="flex flex-wrap">
        <view v-for="(item, index) in images.data" class="mb-[20rpx] relative rounded-[10rpx] ovflow-hidden" :class="{'mb-[20rpx]': maxCount > 1 }">
            <up-image class="rounded-[10rpx] overflow-hidden" width="110rpx" height="110rpx" :src="img('')" model="aspectFill" @click="videoListPreview(item)">
                <template #error>
                    <view class="w-[110rpx] h-[110rpx] rounded-[10rpx] flex flex-col justify-center items-center" :style="{background: 'url(' + img('/addon/sow_community/detail/video.jpg') + ') left top /cover no-repeat'}">
                        <image class="w-[40rpx] h-[40rpx]" :src="img('/addon/sow_community/index/play.png')" :mode="'aspectFill'"></image>
                    </view>
                </template>
            </up-image>
