    ...
    <view :style="themeColor()" class="bg-[var(--page-bg-color)] min-h-[100vh] overflow-hidden">
        <template v-if="!loading">
            <view class="pt-[20rpx] sidebar-margin">
                <view class="flex flex-col card-template mb-[20rpx] text-[28rpx]">
                    ...
                    <view class="card-template-item justify-between">
                        <text class="text-[28rpx] text-[#333]">核销码</text>
                        <view class="text-[28rpx] text-[#333]">{{ verifyInfo.code }}</view>
                    </view>
                    <view class="card-template-item justify-between">
                    ...
                    <view class="card-template-item justify-between">
                        <text class="text-[28rpx] text-[#333]">核销员类型</text>
                        <view class="text-[28rpx] text-[#333]">{{ verifyInfo.verifier_type_name }}</view>
                    </view>
                    <view class="card-template-item justify-between">
                    ...
                    <view class="card-template-item justify-between">
                        ...
                        <view class="text-[#333333] text-[28rpx]">{{ verifyInfo.verifier_name || '--' }}</view>
                    </view>
                    <template v-for="(item,index) in verifyInfo.verify_info">
                        <view class="card-template-item justify-between" v-for="(val,key) in item">
                            <text class="text-[28rpx] text-[#333]">{{ val.name }}</text>
                            <view class="text-[28rpx] text-[#333]">{{ val.value }}</view>
                        </view>
                    </template>
                ...
                <view class="flex flex-col card-template">
                    <view class="flex" :class="{'mb-[20rpx]': verifyInfo.value.list.length-1 != index}" v-for="(item,index) in verifyInfo.value.list" :key="index">
                        ...
                        <view class="flex flex-col flex-1 ml-[20rpx] py-[4rpx]">
                            ...
                            <view class="mt-[14rpx] truncate text-[24rpx] text-[var(--text-color-light9)] leading-[28rpx] max-w-[490rpx]" v-if="item.sub_name"></view>
                            <view class="text-[var(--text-color-light6)] text-[26rpx] mt-[20rpx] text-right">x{{item.num}}</view>
                        </view>
                    </view>
                </view>
            </view>
