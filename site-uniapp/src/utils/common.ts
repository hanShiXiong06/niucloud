 ...
 * 金额格保留两位，不进行四舍五入
 */
export function moneyFormatTwo(money: string): string {
    const parsed = parseFloat(money);
    return isNaN(parsed) ? money : (Math.floor(parsed * 100) / 100).toFixed(2);
}

/**
 * 金额保留4位
 */
export function moneyFormatFour(money: string): string {
    return isNaN(parseFloat(money)) ? money : parseFloat(money).toFixed(4)
}
 ...
 */
export function copy(value: any, callback: any = null) {
    // #ifdef H5
...
export function	filterDigit(e){
    ...
    // 限制最多两位小数
    const decimalParts = e.detail.value.split('.');
    if (decimalParts.length > 1 && decimalParts[1].length > 2) {
        // 如果有小数部分且超过两位，则截取前两位
        e.detail.value = `${decimalParts[0]}.${decimalParts[1].slice(0, 2)}`;
    }
