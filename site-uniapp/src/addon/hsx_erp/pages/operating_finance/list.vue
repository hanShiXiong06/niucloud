<template>
    <view class="erp-page">
        <ErpListHeader v-model="keyword" placeholder="单号 / 经营说明" :show-scan="false" :compact-mp="true" @search="reload">
            <template #below>
        ...
        <z-paging ref="pagingRef" v-model="list" @query="queryList" :fixed="true" :default-page-size="15" :paging-style="pagingStyle">
            ...
            <view class="list-wrap"><view v-for="row in list" :key="`${row.direction}-${row.id}`" class="erp-card op-card">
                <view class="erp-card__head"><view><text class="card-title">{{ row.category_name || '-' }}</text><text class="card-sub">{{ partyDisplayName(row) }}</text></view><u-tag :text="statusLabel(row.status)" :type="statusType(row.status)" plain plainFill size="mini" /></view>
                ...
                <view class="op-foot"><text>{{ formatErpTime(row.occurred_at) }}</text></view>
            ...
            <view class="create-popup"><view class="popup-head"><view><text class="popup-title">记一笔经营收支</text><text class="popup-sub">不影响具体设备成本</text></view><u-icon name="close" size="20" color="#64748b" @click="createVisible=false" /></view>
                <scroll-view scroll-y class="popup-body">
                    ...
                    <view class="form-card">
                        ...
                        <view class="field-row" @click="partyVisible=true"><text>往来主体 *</text><view><strong>{{ selectedPartyLabel }}</strong><u-icon name="arrow-right" size="14" color="#94a3b8" /></view></view>
        ...
        <ErpPartyPopup v-model:show="partyVisible" v-model:party-id="form.party_id" v-model:party-name="form.party_name" v-model:member-name="form.member_name" :role-type="form.direction==='expense'?'supplier':'customer'" />
        <ErpPartyPopup v-model:show="filterPartyVisible" v-model:party-id="filterPartyId" v-model:party-name="filterPartyName" v-model:member-name="filterMemberName" existing-only @select="onFilterPartySelect" />
...
const {pagingStyle}=useListHeader({tabs:true,compactMp:true,h5TopRpx:178}),pagingRef=ref<any>(null),list=ref<any[]>([]),keyword=ref(''),direction=ref(''),financeStatus=ref(''),categoryKey=ref(''),filterPartyId=ref(0),filterPartyName=ref(''),filterMemberName=ref(''),summary=ref({income:0,expense:0,unsettled:0}),categories=ref<any[]>([]),accounts=ref<any[]>([]),createVisible=ref(false),categoryVisible=ref(false),partyVisible=ref(false),filterPartyVisible=ref(false),accountVisible=ref(false),saving=ref(false)
const tabs=[{label:'全部',value:''},{label:'收入',value:'income'},{label:'支出',value:'expense'}],inputStyle={textAlign:'right',background:'#f8fafc',borderRadius:'10rpx',padding:'8rpx 12rpx' ,maxWidth:'220rpx', marginTop:'12rpx'}
const financeStatusOptions=[{label:'全部状态',value:''},{label:'待结算',value:'pending'},{label:'部分结算',value:'partial'},{label:'已结清',value:'settled'}]
const quickFilters=computed(()=>[
    {key:'party_id',label:filterPartyLabel.value,title:'往来主体',value:filterPartyId.value,options:[]},
    {key:'direction',label:'收支方向',title:'收支方向',value:direction.value,options:tabs},
    {key:'status',label:'结算状态',title:'结算状态',value:financeStatus.value,options:financeStatusOptions},
    {key:'category_key',label:'收支类型',title:'收支类型',value:categoryKey.value,options:[{label:'全部类型',value:''},...operatingCategories.value.filter(row=>!direction.value||row.direction===direction.value).map(row=>({label:row.name,value:row.key}))]},
])
const form=ref<any>({direction:'expense',category_key:'',party_id:0,party_name:'',member_name:'',amount:'',settlement_mode:'pending',capital_account_id:0,voucher_urls:'',remark:''})
const operatingCategories=computed(()=>categories.value.filter(row=>row.scope==='operating'&&Number(row.enabled??1)===1)),formCategories=computed(()=>operatingCategories.value.filter(row=>row.direction===form.value.direction)),selectedCategory=computed(()=>operatingCategories.value.find(row=>row.key===form.value.category_key)),selectedAccountLabel=computed(()=>{const a=accounts.value.find(row=>Number(row.id)===Number(form.value.capital_account_id));return a?`${a.account_name} · ¥${money(a.balance)}`:''}),selectedPartyLabel=computed(()=>partyDisplayName(form.value,'请选择'))
const filterPartyLabel=computed(()=>partyDisplayName({party_name:filterPartyName.value,member_name:filterMemberName.value},'往来主体'))
...
function reload(){pagingRef.value?.reload()} function changeDirection(v:string){direction.value=v;if(categoryKey.value&&!operatingCategories.value.some(row=>row.key===categoryKey.value&&(!v||row.direction===v)))categoryKey.value='';reload()}
const onQuickFilter=({key,value}:{key:string,value:string|number})=>{if(key==='direction')return changeDirection(String(value));if(key==='status')financeStatus.value=String(value);if(key==='category_key')categoryKey.value=String(value);reload()}
const onQuickFilterTrigger=({key}:{key:string})=>{if(key==='party_id')filterPartyVisible.value=true}
function onFilterPartySelect(party:any){if(party){filterPartyId.value=Number(party.party_id||party.counterparty_id||0);filterPartyName.value=String(party.party_name||party.counterparty_name||'');filterMemberName.value=String(party.member_name||party.nickname||party.username||'')}reload()}
async function queryList(page:number,limit:number){try{const res:any=await getMobileOperatingFinanceList({page,limit,direction:direction.value,status:financeStatus.value,category_key:categoryKey.value,party_id:filterPartyId.value,keyword:keyword.value});summary.value=res?.data?.summary||summary.value;pagingRef.value?.complete(res?.data?.data||[])}catch(error){pagingRef.value?.complete(false);showErpError(error,'经营收支数据加载失败，请检查网络后重试')}}
...
function openCreate(){form.value={direction:'expense',category_key:'',party_id:0,party_name:'',member_name:'',amount:'',settlement_mode:'pending',capital_account_id:accounts.value[0]?.id||0,voucher_urls:'',remark:''};setFormDirection('expense');createVisible.value=true}
...
const money=(v:any)=>Number(v||0).toFixed(2),statusLabel=(s:string)=>({pending:'待结算',partial:'部分结算',settled:'已结清',void:'已作废'}[s] || '状态待确认'),statusType=(s:string)=>({pending:'warning',partial:'primary',settled:'success',void:'info'}[s]||'info')
const partyDisplayName = erpPartyDisplayName
</script>
