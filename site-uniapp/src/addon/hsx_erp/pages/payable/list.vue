    ...
    <view class="erp-page">
        ...
        <z-paging ref="pagingRef" v-model="list" @query="queryList" :fixed="true" :default-page-size="15" :paging-style="pagingStyle">
            ...
            <view class="list-wrap">
                <view v-for="row in list" :key="row.party_id + '_' + row.purchase_order_id" class="erp-card">
                    ...
                    <view class="card-meta">采购单：{{ row.batch_no || row.purchase_no || '-' }}</view>
                    <view class="card-meta">采购员：{{ row.purchaser_name || '-' }}</view>
                    ...
                    <view v-if="row.finance_status !== 'settled'" class="mt-16rpx">
            ...
            <view v-if="payRow" class="pay-popup">
                ...
                <view class="pay-form">
                    <view class="pay-form__item">
                        <text class="pay-label">付款金额</text>
                        <u-input v-model="payForm.amount" type="number" placeholder="输入付款金额" :customStyle="inputStyle" />
                    </view>
                    <view class="pay-form__item">
                    ...
                    <view class="pay-form__item">
                        ...
                        <u-input v-model="payForm.remark" placeholder="可选备注" :customStyle="inputStyle" />
                    </view>
                </view>
                <view class="action-bar">
                    <u-button @click="payVisible = false" :customStyle="{flex:'1'}">取消</u-button>
                    <u-button type="primary" :loading="paying" :disabled="!canPay" @click="submitPay" :customStyle="{flex:'2'}">确认付款</u-button>
                </view>
            </view>
        </u-popup>
    </view>
...
const tabs = [{ name: '全部', value: '' }, { name: '待付款', value: 'pending' }, { name: '部分付款', value: 'partial' }]
const tabIndex = ref(1) // 默认显示"待付款"
...
const canPay = computed(() => Number(payForm.value.amount) > 0 && payForm.value.capital_account_id > 0)
...
const queryList = async (pageNo: number, pageSize: number) => {
    try {
        const res: any = await getMobilePayableList({ keyword: keyword.value, status: curStatus.value, page: pageNo, limit: pageSize })
...
function openPay(row: any) {
    payRow.value = row
    payForm.value = { amount: Number(row.remain_amount || 0), capital_account_id: accounts.value[0]?.id || 0, remark: '' }
...
async function submitPay() {
    ...
    paying.value = true
    try {
        await confirmMobilePurchasePayment(payRow.value.party_id, payForm.value)
        uni.showToast({ title: '付款已确认', icon: 'success' })
        payVisible.value = false
        reload()
    } catch (e: any) {
        uni.showToast({ title: e?.message || '付款失败', icon: 'error' })
...
.pay-info__remain { font-size: 24rpx; color: #ea580c; margin-top: 4rpx; display: block; }
