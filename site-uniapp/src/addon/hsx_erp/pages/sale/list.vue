<template>
    <view class="erp-page">
        ...
        <view class="page-search">
            <u-search v-model="keyword" placeholder="型号/IMEI/销售单/客户" :showAction="false" bgColor="#f1f5f9" height="34" @search="reload" @clear="reload" />
        ...
        <z-paging ref="pagingRef" v-model="list" @query="queryList" :fixed="true" :default-page-size="15" :paging-style="pagingStyle">
            ...
            <view class="list-wrap">
                <view v-for="row in list" :key="row.id" class="erp-card">
                    ...
                    <view class="card-meta">客户：{{ row.party_name || '-' }}</view>
                    <view class="erp-card__foot">
                        ...
                        <view class="amount-box">
                            ...
                            <text class="amt-value green">¥{{ money(row.profit) }}</text>
                        </view>
                        <view class="amount-box">
...
const queryList = async (pageNo: number, pageSize: number) => {
    try {
        const res: any = await getMobileSaleList({ keyword: keyword.value, finance_status: curStatus.value, page: pageNo, limit: pageSize })
        pagingRef.value?.complete(res?.data?.data || [])
    } catch { pagingRef.value?.complete(false) }
}

const money = (v: any) => Number(v || 0).toFixed(2)
const financeLabel = (s: string) => ({ pending: '待收款', partial: '部分收款', settled: '已结清', void: '已撤销' }[s] || s)
