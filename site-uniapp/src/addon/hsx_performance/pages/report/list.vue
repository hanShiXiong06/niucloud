<template>
    <view class="page">
        <view class="head"><view><text class="title">经营报告</text><text class="subtitle">店铺经营与员工产出快照</text></view><view class="refresh" @click="load(true)"><u-icon name="reload" color="#3b6ef5" size="20" /></view></view>
        ...
        <view v-else class="list">
            <view v-for="row in rows" :key="row.id" class="report" @click="open(row.id)">
                ...
                <view class="metrics"><view v-for="item in coreMetrics(row)" :key="item.label"><strong :class="item.className">{{ item.value }}</strong><text>{{ item.label }}</text></view></view>
                <view class="snapshot-line"><text>生成时库存 {{ quantity(summary(row).stock_count) }} 台</text><text :class="{ warning: todoCount(row) > 0 }">待处理 {{ todoCount(row) }} 项</text></view>
...
const coreMetrics = (row: any) => { const s = summary(row); return [
    ...
    { label: '销售件数', value: `${quantity(s.sale_count)} 件` }, { label: '回收入库', value: `${quantity(s.recycle_in_count)} 台` },
] }
...
.page { min-height: 100vh; padding: 28rpx; box-sizing: border-box; background: #f3f4f6; }
...
.tabs { margin: 24rpx 0 18rpx; white-space: nowrap; }.tabs-inner { display: inline-flex; gap: 12rpx; }.tab { min-width: 96rpx; padding: 14rpx 22rpx; box-sizing: border-box; color: #475569; font-size: 25rpx; text-align: center; background: #fff; border: 2rpx solid #e5e7eb; border-radius: 12rpx; }.tab.active { color: #fff; background: #3b6ef5; border-color: #3b6ef5; }
.list { display: flex; flex-direction: column; gap: 18rpx; }.report { padding: 24rpx; background: #fff; border: 2rpx solid #e8edf3; border-radius: 16rpx; }.report-head { display: flex; align-items: flex-start; justify-content: space-between; }.report-title { display: block; color: #111827; font-size: 29rpx; font-weight: 650; }.period { display: block; margin-top: 6rpx; color: #94a3b8; font-size: 21rpx; }.metrics { margin-top: 22rpx; display: grid; grid-template-columns: repeat(2, 1fr); gap: 12rpx; }.metrics view { min-width: 0; padding: 16rpx; display: flex; flex-direction: column; background: #f8fafc; border-radius: 10rpx; }.metrics strong { color: #111827; font-size: 28rpx; }.metrics text { margin-top: 4rpx; color: #64748b; font-size: 20rpx; }.positive { color: #059669 !important; }.negative { color: #dc2626 !important; }.snapshot-line { margin-top: 16rpx; display: flex; justify-content: space-between; color: #64748b; font-size: 20rpx; }.warning { color: #d97706; }.report-foot { margin-top: 16rpx; padding-top: 16rpx; display: flex; justify-content: space-between; color: #64748b; font-size: 21rpx; border-top: 2rpx solid #f1f5f9; }.notify-success { color: #059669; }.notify-failed { color: #dc2626; }.notify-pending { color: #d97706; }.state { height: 500rpx; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 18rpx; color: #94a3b8; font-size: 24rpx; }
</style>
