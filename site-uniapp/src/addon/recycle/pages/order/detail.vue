...
import { isConsignedDevice, shouldShowConfirmStatus } from '@/addon/recycle/utils/device'
...
const handleDeviceAction = async (type: string, device: any) => {
    ...
    if (type === 'view_consignment') {
        uni.navigateTo({
            url: `/addon/recycle/pages/consignment/detail?id=${ device.consignment_order_id }`
        })
        return
    }
    ...
    if (type === 'view_return') {
        ...
        uni.navigateTo({
            url: `/addon/recycle/pages/return/detail?id=${ returnOrderId }`
        })
    }
}
