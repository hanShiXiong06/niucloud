            ...
            <view class="pt-[var(--top-m)]">
                ...
                <view class="w-full footer">
                    <view class="py-[var(--top-m)] px-[var(--sidebar-m)] footer w-full fixed bottom-0 left-0 right-0 box-border">
                        <button class="primary-btn-bg text-[#fff] h-[80rpx] leading-[80rpx] rounded-[16rpx] text-[26rpx] mx-0 flex-1 font-500" @click="redirect({url: '/addon/mall/pages/refund/detail', param: { refund_id: refundId }})">返回详情</button>
                    </view>
                </view>
            </view>
        </view>
...
import { orderRefundDetail } from '@/addon/mall/api/refund';
...
onLoad((option:any) => {
    ...
    if (option.refund_id) {
	    refundDetailFn(option.refund_id);
    } else {
        let parameter = {
...
const refundDetailFn = (refundNo: any) => {
	loading.value = true;
	orderRefundDetail(refundNo).then((res: any) => {
