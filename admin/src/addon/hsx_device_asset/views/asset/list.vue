<template>
    <div class="main-container device-asset-page">
        <el-card class="!border-none" shadow="never">
            <div class="page-head">
                ...
                <div class="head-actions">
                    <el-input ref="scanInputRef" v-model.trim="scanKeyword" class="scan-input" placeholder="扫码枪扫描 IMEI / SN / 资产编号，回车拍摄" :disabled="scanLoading" clearable @keyup.enter="scanForPhoto" />
                    <el-button :loading="scanLoading" :disabled="!scanKeyword" @click="scanForPhoto">扫码拍摄</el-button>
        ...
        <el-dialog v-model="mediaDialogVisible" title="商品图片拍摄" width="min(960px, 96vw)" destroy-on-close :close-on-click-modal="false" @closed="scanInputRef?.focus()">
            <div v-if="mediaAsset" class="dialog-body">
                ...
                <AutomaticPhotoStation :key="mediaAsset.id" :asset="mediaAsset" @fallback="mobileSections = ['phone']" @uploaded="refreshMediaAsset" @completed="finishAutomaticCapture" />
                <el-collapse v-model="mobileSections">
                    <el-collapse-item name="phone" title="手机补拍 / 已回传图片 · 工位不可用时展开">
                <div class="mobile-capture-panel">
                    <div class="mobile-capture-panel__qr">
                        <img v-if="mobileQrCode" :src="mobileQrCode" alt="移动拍照二维码" />
                        <div v-else class="qr-empty">{{ mobileQrError || '生成中' }}</div>
                    </div>
                    <div class="mobile-capture-panel__content">
                        <div class="mobile-capture-panel__title">手机扫码拍照并回传</div>
                        <div class="mobile-capture-panel__desc">扫码进入管理移动端，登录后补拍当前设备。照片按系统存储配置保存；回传成功后在这里确认，或直接在手机确认。</div>
                        ...
                        <div class="mobile-capture-panel__actions">
                            ...
                            <el-button link type="primary" :disabled="!mobileCaptureUrl" @click="copyMobileCaptureUrl">复制链接</el-button>
                        </div>
                    </div>
                </div>
            ...
            <template #footer>
                ...
                <template v-if="mobileSections.includes('phone')">
                    <el-button v-if="mediaForm.url" :loading="mediaSaveLoading" @click="handleSaveMedia">保存补录链接</el-button>
                    <el-button type="primary" :loading="mediaAsset?._confirmingPhotos" :disabled="!canConfirmPhotos(mediaAsset)" @click="confirmAndClose">确认回传图片并关闭</el-button>
                    <el-button :disabled="!canConfirmPhotos(mediaAsset) || mediaAsset?._confirmingPhotos" @click="confirmAndNext">确认并拍下一台</el-button>
                </template>
            </template>
        </el-dialog>
...
import AutomaticPhotoStation from '../../components/AutomaticPhotoStation.vue'
import {
    confirmAssetPhotos,
    exportAssetExcel,
    getAssetInfo,
...
const scanForPhoto = async () => {
    if (!scanKeyword.value || scanLoading.value) return
    scanLoading.value = true
    try {
        const res: any = await scanImportAsset(scanKeyword.value)
        if (!res.data?.asset?.id) throw new Error('未找到可拍摄的设备，请从 ERP 库存中心发起拍摄')
        await openMediaDialog(res.data.asset)
        scanKeyword.value = ''
    } catch (e: any) { ElMessage.error(e?.msg || e?.message || '设备查找失败，请检查扫码内容') }
    finally { scanLoading.value = false }
}
const finishAutomaticCapture = async () => {
    mediaDialogVisible.value = false
    await Promise.all([loadAssets(), loadStats()])
    ElMessage.success('交接已完成，可扫描下一台设备')
}
const mediaAsset = ref<any>(null)
const mobileCaptureUrl = ref('')
const mobileQrCode = ref('')
const mobileQrError = ref('')
...
onMounted(() => {
    loadAssets()
    loadStats()
    const id = Number(route.query.photo_asset_id || 0)
    if (id > 0) openMediaDialog({ id })
})
...
const generateMobileCaptureQr = async (asset: any) => {
    mobileQrCode.value = ''
    mobileQrError.value = ''
    mobileCaptureUrl.value = String(asset.mobile_photo_url || '')
    if (!mobileCaptureUrl.value) { mobileQrError.value = '未获取到管理端拍摄入口，请更新拍照插件'; return }
    try {
        const code = await QRCode.toDataURL(mobileCaptureUrl.value, { errorCorrectionLevel: 'L', margin: 1, width: 180 })
        if (mediaAsset.value?.id === asset.id) mobileQrCode.value = code
    } catch { mobileQrError.value = '二维码生成失败，可复制链接在手机打开' }
}
...
const startMediaAutoRefresh = () => {
    ...
    mediaRefreshTimer = setInterval(() => {
        if (mediaDialogVisible.value && mobileSections.value.includes('phone') && mediaAsset.value?.id && !mediaRefreshLoading.value) {
            void refreshMediaAsset().catch(() => { /* 请求组件已提示失败，保留现有图片并等待手动重试。 */ })
        }
    }, 5000)
}
...
const handleConfirmPhotos = async (asset: any, refreshDetail = false) => {
    if (!asset?.id || !canConfirmPhotos(asset) || asset._confirmingPhotos) return
    asset._confirmingPhotos = true
    try {
        const response: any = await confirmAssetPhotos(asset.id)
        ElMessage.success(response.data?.message || '照片已确认')
        await Promise.all([loadAssets(), loadStats()])
        if (refreshDetail) {
...
const photoWorkflowSteps = (asset: any) => {
    ...
    return [
        { key: 'capture', index: 1, title: '拍摄商品图', desc: hasMedia ? `已回传 ${ asset.image_count || 0 } 张` : '自动拍摄，手机补拍', done: hasMedia, active: !hasMedia },
        { key: 'review', index: 2, title: '确认拍照完成', desc: reviewed ? '照片已确认' : '删掉不清晰的，确认后进入定价', done: reviewed, active: hasMedia && !reviewed },
        { key: 'price', index: 3, title: asset?.ext_json?.erp_asset_id ? '交 ERP 定价' : '生成定价工单', desc: asset?.status === 'erp_handoff' ? 'ERP 已接收' : priced ? '定价已完成' : '选图确认后交接', done: priced || asset?.status === 'erp_handoff', active: reviewed && !priced },
    ]
}
