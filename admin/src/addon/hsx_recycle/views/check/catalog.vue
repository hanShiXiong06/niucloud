<template>
    <PremiumTheme class="check-catalog-page">
        ...
        <el-dialog v-model="imp.visible" title="导入检测表" width="460px" :close-on-click-modal="false" :show-close="!imp.running" class="hsx-premium-overlay">
            ...
            <div class="imp-line imp-muted">生成模板 {{ imp.templates }} · 绑定型号 {{ imp.bindings }}</div>
            <template #footer>
...
const imp = reactive({
    ...
    total: 0, rows: 0, templates: 0, bindings: 0, batch_id: 0, token: '',
})
...
async function onFileChange(e: Event) {
    ...
    input.value = ''
    if (!f) return
    Object.assign(imp, { visible: true, running: true, done: false, fileName: f.name, total: 0, rows: 0, templates: 0, bindings: 0 })
    try {
