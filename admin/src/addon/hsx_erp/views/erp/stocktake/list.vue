<template>
    <HsxPage padding="none" class="main-container">
        ...
        <HsxDialog :confirm-loading="createDialog.loading" v-model="createDialog.visible" title="新建库存盘点" width="560px" destroy-on-close>
        ...
        <HsxDrawer v-model="detail.visible" size="82%" destroy-on-close @closed="detail.row=null">
        ...
        <HsxDialog :confirm-loading="resolveDialog.loading" v-model="resolveDialog.visible" title="处理盘点差异" width="520px" :destroy-on-close="false">
...
const queryModel = computed({
    get: () => ({ keyword: search.keyword, warehouse_id: search.warehouse_id }),
    set: value => Object.assign(search, { keyword: '', warehouse_id: '' }, value)
})
const querySchema = computed<ProFormField[]>(() => [
    { prop: 'keyword', label: '关键词', component: 'input', placeholder: '盘点单号 / 仓库 / 操作人', props: { clearable: true } },
    { prop: 'warehouse_id', label: '仓库', component: 'select', placeholder: '全部仓库', props: { clearable: true }, options: warehouses.value.map(item => ({ label: item.warehouse_name, value: item.id })) }
])
function searchStocktakes(values: Record<string, any>) {
    Object.assign(search, { keyword: '', warehouse_id: '' }, values)
