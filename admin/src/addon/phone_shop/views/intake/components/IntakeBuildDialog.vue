<template>
    <HsxDialog v-model="visible" title="完善并上架" subtitle="核对交接资料，确认后发布到客户商城"
        ...
        <div v-else-if="ready" class="intake-build">
            <aside class="intake-build__preview">
                <div class="intake-build__eyebrow"><span>ERP 交接设备</span><el-tag size="small" effect="plain">待上架</el-tag></div>
            ...
            <section class="intake-build__editor">
                <div class="intake-build__notice">
                    ...
                    <span>{{ basicFirst ? '确认分类即可上架购买；细节由运营在资料待办中继续完善。' : '图片和质检已带入，核对分类与销售资料后即可上架。' }}</span>
                </div>
                <el-form :model="form" label-position="top" :disabled="saving" @submit.prevent>
                    <el-tabs v-model="activeTab" class="intake-build__tabs">
                        <el-tab-pane label="基本信息" name="basic">
                            <el-form-item label="商品分类" required :error="fieldError('category')">
                            ...
                            <el-form-item label="商品标题" required :error="fieldError('title')">
                                <el-input ref="titleRef" v-model="form.goods_name" maxlength="60" show-word-limit placeholder="建议：型号 + 容量 + 颜色" />
                            </el-form-item>
                            <div class="intake-build__grid">
                                ...
                                <el-form-item v-if="!basicFirst" :label="specLabel">
                                    <el-select v-model="form.memory" :loading="specLoading" filterable allow-create default-first-option clearable placeholder="可选择，也可输入规格">
                                        <el-option v-for="value in specItems" :key="value" :label="value" :value="value" />
                                    </el-select>
                                </el-form-item>
                                <el-form-item v-if="!basicFirst" label="成色">
                                    <el-select v-model="form.condition_grade" filterable allow-create default-first-option clearable placeholder="选择或输入实际成色">
                            ...
                            <el-collapse class="intake-build__optional">
                                <el-collapse-item title="更多展示信息（选填）" name="display">
                                    <el-form-item label="展示摘要"><el-input v-model="form.sub_title" maxlength="100" show-word-limit placeholder="填写已确认的卖点或售后说明" /></el-form-item>
                                    <template v-if="!basicFirst">
                        ...
                        <el-tab-pane label="价格与配送" name="sale">
                        ...
                        <el-tab-pane label="质检与详情" name="quality">
                            ...
                            <el-form-item v-if="!basicFirst" label="商品详情文案（选填）" class="intake-build__description">
        ...
        <template #footer>
            <div class="intake-build__footer">
                ...
                <div class="intake-build__footer-actions">
                    ...
                    <el-button type="primary" :loading="saving" :disabled="!ready || loading || !!loadError" @click="submit">确认上架</el-button>
                </div>
            </div>
        </template>
    </HsxDialog>
...
import { buildDeviceIntake, previewDeviceIntake } from '@/addon/phone_shop/api/device_intake'
...
type TabName = 'basic' | 'sale' | 'quality'
...
const deliveryOptions = [{ value: 'express', label: '快递发货' }, { value: 'local', label: '同城配送' }, { value: 'store', label: '到店自提' }]
...
const dirty = computed(() => initialForm.value && signature(form) !== signature(initialForm.value))
...
async function loadSpecOptions(path: number[] | null) {
    ...
    } catch { if (sequence === specSequence) specError.value = '分类规格加载失败，可重试或手动填写，不影响已填资料。' }
...
async function open(row: Record<string, any>) {
    ...
    initialForm.value = null; attempted.value = false; activeTab.value = 'basic'; specGroups.value = []; specError.value = ''
...
async function beforeClose(done: () => void) {
    ...
    if (dirty.value && !await feedback.confirm({ title: '资料尚未上架', message: '关闭会放弃本次修改，ERP 原始交接资料不受影响。', confirmText: '放弃修改', cancelText: '继续编辑' })) return
    invalidateRequests(); done()
}

async function submit() {
    if (saving.value || loading.value || !ready.value || loadError.value || !visible.value) return
