<template>
    <div class="main-container">
        ...
        <el-card class="box-card !border-none" shadow="never">
            <el-page-header :content="goodsEdit.formData.goods_id ? t('updateGoods') : t('addGoods')" :icon="ArrowLeft"
                @back="$router.back()" />
        </el-card>
        ...
        <el-card class="box-card mt-[15px] !border-none" shadow="never">
            ...
            <el-tabs v-model="goodsEdit.activeName" @tab-click="goodsEdit.tabHandleClick">
                <el-tab-pane :label="t('basicInfoTab')" name="basic">
                    ...
                    <el-form :model="goodsEdit.formData" label-width="120px" ref="basicFormRef"
                        :rules="goodsEdit.formRules" class="page-form">
                        <el-form-item :label="t('goodsType')" v-if="goodsEdit.formData.goods_id">
                            <template v-for="(item) in goodsEdit.goodsType" :key="item.type">
                                <div class="goods-type-wrap"
                                    :class="[goodsEdit.formData.goods_type == item.type ? 'selected' : 'disabled']">
                        ...
                        <el-form-item :label="t('goodsType')" v-else>
                            <div class="goods-type-wrap"
                                :class="{ 'selected': goodsEdit.formData.goods_type == item.type }"
                                v-for="(item) in goodsEdit.goodsType" :key="item.type"
                                @click="goodsEdit.changeGoodsType(item)">
                                <div class="goods-type-name">{{ item.name }}</div>
                                <div class="goods-type-desc">({{ item.desc }})</div>
                                <template v-if="goodsEdit.formData.goods_type == item.type">
                                    <div class="triangle"></div>
                        ...
                        <el-form-item :label="t('goodsCategory')" prop="goods_category">
                            <el-cascader v-model="goodsEdit.formData.goods_category"
                                :options="goodsEdit.goodsCategoryOptions" :props="goodsEdit.goodsCategoryProps"
                                clearable filterable @change="goodsEdit.categoryHandleChange" popper-class="choice" />
                            <div class="ml-[10px]">
                                <span class="cursor-pointer text-primary mr-[10px]"
                                    @click="goodsEdit.refreshGoodsCategory(true)">{{
                                        t('refresh') }}</span>
                                <span class="cursor-pointer text-primary" @click="goodsEdit.toGoodsCategoryEvent">{{
                                    t('addGoodsCategory')
                                }}</span>
                            </div>
                        </el-form-item>
                        ...
                        <el-form-item :label="t('brand')">
                            <el-select v-model="goodsEdit.formData.brand_id" :placeholder="t('brandPlaceholder')"
                                clearable>
                                <el-option v-for="item in goodsEdit.brandOptions" :key="item.brand_id"
                                    :label="item.brand_name" :value="item.brand_id" />
                            </el-select>
                            <div class="ml-[10px]">
                                <span class="cursor-pointer text-primary mr-[10px]"
                                    @click="goodsEdit.refreshGoodsBrand(true)">{{ t('refresh')
                                    }}</span>
                                <span class="cursor-pointer text-primary" @click="goodsEdit.toGoodsBrandEvent">{{
                                    t('addGoodsBrand') }}</span>
                            </div>
                        </el-form-item>

                        <el-form-item :label="t('poster')">
                            <el-select v-model="goodsEdit.formData.poster_id" :placeholder="t('posterPlaceholder')"
                                clearable>
                                <el-option v-for="item in goodsEdit.posterOptions" :key="item.id" :label="item.name"
                                    :value="item.id" />
                            </el-select>
                            <div class="ml-[10px]">
                                <span class="cursor-pointer text-primary mr-[10px]"
                                    @click="goodsEdit.refreshGoodsPoster(true)">{{ t('refresh')
                                    }}</span>
                                <span class="cursor-pointer text-primary" @click="goodsEdit.toPosterEvent">{{
                        ...
                        <el-form-item :label="t('label')">
                            <el-checkbox-group v-model="goodsEdit.formData.label_ids">
                                <el-checkbox :label="item.label_id" v-for="(item, index) in goodsEdit.labelOptions"
                                    :key="index">{{
                                        item.label_name }}</el-checkbox>
                            </el-checkbox-group>
                            <div class="ml-[10px]">
                                <span class="cursor-pointer text-primary mr-[10px]"
                                    @click="goodsEdit.refreshGoodsLabel">{{ t('refresh')
                                    }}</span>
                                <span class="cursor-pointer text-primary" @click="goodsEdit.toGoodsLabelEvent">{{
                        ...
                        <el-form-item :label="t('goodsService')">
                            ...
                            <div class="ml-[10px]">
                                <span class="cursor-pointer text-primary mr-[10px]"
                                    @click="goodsEdit.refreshGoodsService">{{ t('refresh')
                                    }}</span>
                                <span class="cursor-pointer text-primary" @click="goodsEdit.toGoodsServiceEvent">{{
                                    t('addGoodsService')
                                }}</span>
                            </div>
                        </el-form-item>
                        ...
                        <el-form-item :label="t('supplier')"
                            v-if="goodsEdit.formData.addon_shop_supplier && goodsEdit.formData.addon_shop_supplier.status == 1">
                            <el-select v-model="goodsEdit.formData.supplier_id" :placeholder="t('supplierPlaceholder')"
                                clearable>
                                <el-option v-for="item in goodsEdit.supplierOptions" :key="item.supplier_id"
                                    :label="item.supplier_name" :value="item.supplier_id" />
                            </el-select>
                            <div class="ml-[10px]">
                                <span class="cursor-pointer text-primary mr-[10px]"
                                    @click="goodsEdit.refreshSupplier">{{ t('refresh') }}</span>
                                <span class="cursor-pointer text-primary" @click="goodsEdit.toSupplierEvent">{{
                                    t('addSupplier') }}</span>
                            </div>
                        </el-form-item>
                        ...
                        <el-form-item :label="t('virtualSaleNum')" prop="virtual_sale_num">
                            <div>
                                <el-input v-model.trim="goodsEdit.formData.virtual_sale_num" clearable
                                    ...
                                    <template #append>{{ goodsEdit.formData.unit ? goodsEdit.formData.unit : '件'
                                        }}</template>
                                </el-input>
                                <div class="mt-[10px] text-[12px] text-[#999] leading-[20px]">{{ t('virtualSaleNumDesc')
                                    }}</div>
                            </div>
                        </el-form-item>
                        <el-form-item :label="t('sort')" prop="sort">
                            <el-input v-model.trim="goodsEdit.formData.sort" clearable
                                :placeholder="t('sortPlaceholder')" class="input-width" show-word-limit maxlength="8"
                                @keyup="filterNumber($event)" @blur="goodsEdit.formData.sort = $event.target.value" />
                        </el-form-item>
                ...
                <el-tab-pane :label="t('priceStockTab')" name="price_stock">
                    <el-form :model="goodsEdit.formData" label-width="120px" ref="priceStockFormRef"
                        :rules="goodsEdit.formRules" class="page-form">
                        <el-form-item :label="t('specType')" prop="spec_type">
                            <div>
                                <el-radio-group v-model="goodsEdit.formData.spec_type">
                                    <el-radio label="single" :disabled="goodsEdit.isDisabledPrice()">{{ t('singleSpec')
                                        }}</el-radio>
                                    <el-radio label="multi" :disabled="goodsEdit.isDisabledPrice()">{{ t('multiSpec')
                                        }}</el-radio>
                                </el-radio-group>
                                <div class="mt-[10px] text-[12px] text-[#999] leading-[20px]"
                                    v-if="goodsEdit.isDisabledPrice()">{{
                                        t('participateInActiveDisableTips') }}</div>
                            </div>
                        </el-form-item>
                        ...
                        <template v-if="goodsEdit.formData.spec_type == 'single'">
                            ...
                            <el-form-item :label="t('costPrice')" prop="cost_price">
                                <el-input v-model.trim="goodsEdit.formData.cost_price" clearable placeholder="0.00"
                                    class="input-width" maxlength="8">
                                    <template #append>{{ t('yuan') }}</template>
                                </el-input>
                            </el-form-item>
                            ...
                            <el-form-item :label="t('volume')" prop="volume">
                                <el-input v-model.trim="goodsEdit.formData.volume" clearable placeholder="0.00"
                                    class="input-width" maxlength="6">
                            ...
                            <el-form-item :label="t('goodsStock')" prop="stock">
                                <el-input v-model.trim="goodsEdit.formData.stock" clearable
                                    ...
                                    @keyup="filterNumber($event)" :disabled="goodsEdit.isDisabledPrice()">
                                    <template #append>{{ goodsEdit.formData.unit ? goodsEdit.formData.unit :
                                        t('defaultUnit') }}</template>
                                </el-input>
                            </el-form-item>
                            <el-form-item :label="t('skuNo')">
                                <el-input v-model.trim="goodsEdit.formData.sku_no" clearable
                                    :placeholder="t('skuNoPlaceholder')" class="input-width" maxlength="50"
                                    @keyup="goodsEdit.filterSpecial($event)" />
                            </el-form-item>
                        ...
                        <div class="el-form-item asterisk-left" v-show="goodsEdit.formData.spec_type == 'multi'">
                            ...
                            <div class="spec-wrap">
                                ...
                                <div class="spec-edit-list">
                                    <div class="spec-item" v-for="(item, index) in goodsEdit.goodsSpecFormat"
                                        :key="item.id">
                                        <div class="spec-name-wrap">
                                            <el-input v-model.trim="item.spec_name" clearable
                                            ...
                                            <ul ref="specValueRef">
                                                <li :class="'draggable-element' + index"
                                                    ...
                                                    <el-icon class="icon" :size="20" color="#7b7b7b"
                                                        @click="goodsEdit.deleteSpecValue(index, specIndex)">
                                                        <CircleCloseFilled />
                                                    </el-icon>
                                                </li>
                                            </ul>
                                            <span class="text-color text-[14px] add-spec-value"
                                                @click="goodsEdit.addSpecValue(index)">{{ t('addSpecValue') }}</span>
                                            <div class="box"></div>
                                        </div>

                                        <el-icon class="del-spec" :size="20" color="#7b7b7b"
                                            @click="goodsEdit.deleteSpec(index)">
                                            <CircleCloseFilled />
                                        </el-icon>
                                    </div>
                                ...
                                <div class="batch-operation-sku" v-show="Object.keys(goodsEdit.goodsSkuData).length">
                                    ...
                                    <el-input v-if="!goodsEdit.isDisabledPrice()"
                                        v-model.trim="goodsEdit.batchOperation.price" clearable
                                        :placeholder="t('price')" class="set-input" maxlength="8" />
                                    <el-input v-model.trim="goodsEdit.batchOperation.market_price" clearable
                                        :placeholder="t('marketPrice')" class="set-input" maxlength="8" />
                                    <el-input v-model.trim="goodsEdit.batchOperation.cost_price" clearable
                                        :placeholder="t('costPrice')" class="set-input" maxlength="8" />
                                    <el-input v-if="!goodsEdit.isDisabledPrice()"
                                        v-model.trim="goodsEdit.batchOperation.stock" clearable
                                        :placeholder="t('stock')" class="set-input" maxlength="8" />
                                    <el-input v-model.trim="goodsEdit.batchOperation.weight" clearable
                                        :placeholder="t('skuWeight')" class="set-input" maxlength="6" />
                                    <el-input v-model.trim="goodsEdit.batchOperation.volume" clearable
                                        :placeholder="t('skuVolume')" class="set-input" maxlength="6" />
                                    <el-input v-model.trim="goodsEdit.batchOperation.sku_no" clearable maxlength="50"
                                        :placeholder="t('skuNo')" class="set-input" />
                                    <el-button type="primary" @click="goodsEdit.saveBatch">{{ t('confirm')
                                        }}</el-button>
                                </div>
                                                ...
                                                <table class="el-table__header" border="0" cellpadding="0"
                                                    ...
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <template v-for="(item, index) in goodsEdit.goodsSpecFormat"
                                                                :key="index">
                                                            ...
                                                            <table class="el-table__body" cellspacing="0"
                                                                ...
                                                                <tbody tabindex="-1">
                                                                    <tr class="el-table__row"
                                                                        v-for="(item, key, index) in goodsEdit.goodsSkuData"
                                                                        :key="key">
                                                                        <template
                                                                            v-for="(c, cIndex) in goodsEdit.specData"
                                                                            :key="cIndex">
                                                                            <td class="el-table__cell"
                                                                                :rowspan="c.rowSpan"
                                                                                v-if="c.index == index">
                                                                                <div class="cell">{{ c.spec_value_name
                                                                                    }}</div>
                                                                            </td>
                                                                        </template>
                                                                        ...
                                                                        <td class="el-table__cell">
                                                                            <div class="cell">
                                                                                <el-form-item
                                                                                    ...
                                                                                    :rules="goodsEdit.skuMarketPriceRules()"
                                                                                    class="sku-form-item-wrap">
                                                                                    <el-input
                                                                        ...
                                                                        <td class="el-table__cell">
                                                                            <div class="cell">
                                                                                <el-form-item
                                                                                    :prop="key + '.cost_price'"
                                                                                    :rules="goodsEdit.skuCostPriceRules()"
                                                                                    class="sku-form-item-wrap">
                                                                                    <el-input
                                                                        ...
                                                                        <td class="el-table__cell">
                                                                            <div class="cell">
                                                                                <el-form-item :prop="key + '.stock'"
                                                                                    :rules="goodsEdit.skuStockRules()"
                                                                                    class="sku-form-item-wrap">
                                                                                    <el-input v-model.trim="item.stock"
                                                                                        clearable placeholder="0"
                                                                                        @input="goodsEdit.specStockSum"
                                                                                        maxlength="8"
                                                                                        :disabled="goodsEdit.isDisabledPrice()" />
                                                                                </el-form-item>
                                                                            </div>
                                                                        ...
                                                                        <td class="el-table__cell">
                                                                            <div class="cell">
                                                                                <el-form-item :prop="key + '.volume'"
                                                                                    :rules="skuVolumeRules()"
                                                                                    class="sku-form-item-wrap">
                                                                                    <el-input v-model.trim="item.volume"
                                                                                        clearable placeholder="0.00"
                                                                                        maxlength="6" />
                                                                                </el-form-item>
                                                                            </div>
                                                                        ...
                                                                        <td class="el-table__cell">
                                                                            <div class="cell">
                                                                                <el-input v-model.trim="item.sku_no"
                                                                        ...
                                                                        <td class="el-table__cell">
                                                                            <div class="cell">
                                                                                <el-switch v-model="item.is_default"
                                                                                    :active-value="1"
                                                                                    :inactive-value="0"
                                                                                    @change="goodsEdit.specValueIsDefaultChangeListener($event, key)" />
                                                                            </div>
                                                                        </td>
                                                                    </tr>
                                                                </tbody>
                        ...
                        <el-form-item :label="t('memberDiscount')">
                            <div>
                                ...
                                <div class="text-[12px] text-[#999] leading-[20px]"
                                    v-if="goodsEdit.formData.member_discount == 'discount'">{{ t('discountHint') }}
                                </div>
                                <div class="text-[12px] text-[#999] leading-[20px]"
                                    v-if="goodsEdit.formData.member_discount == 'fixed_price'">{{ t('fixedPriceHint') }}
                                </div>
                            </div>
                        </el-form-item>
                    </el-form>
                ...
                <el-tab-pane :label="t('deliveryTab')" name="delivery">
                    ...
                    <el-form :model="goodsEdit.formData" label-width="120px" ref="deliveryFormRef"
                        :rules="goodsEdit.formRules" class="page-form">
                        <el-form-item :label="t('deliveryType')" prop="delivery_type">
                            <div>
                                <el-checkbox-group v-model="goodsEdit.formData.delivery_type">
                                    <!--                                    v-show="item.status == '1'"-->
                                    <el-checkbox v-for="(item, index) in deliveryTypeCheckBox" :key="index"
                                        :label="item.key">{{
                                            item.name }}</el-checkbox>
                                </el-checkbox-group>
                                <div v-if="deliveryTypeFlag" class="text-[12px] text-[#999] leading-[20px]">
                                    请先在配置设置中设置配送方式</div>
                            </div>
                        </el-form-item>

                        <el-form-item :label="t('isFreeShipping')"
                            v-show="goodsEdit.formData.delivery_type.indexOf('express') != -1">
                            <el-switch v-model="goodsEdit.formData.is_free_shipping" :active-value="1"
                                :inactive-value="0" />
                        </el-form-item>

                        <el-form-item :label="t('feeType')" prop="fee_type"
                            v-show="goodsEdit.formData.delivery_type.indexOf('express') != -1 && goodsEdit.formData.is_free_shipping == 0">
                        ...
                        <el-form-item :label="t('deliveryMoney')" prop="delivery_money"
                            v-show="goodsEdit.formData.delivery_type.indexOf('express') != -1 && goodsEdit.formData.is_free_shipping == 0 && goodsEdit.formData.fee_type == 'fixed'">
                            <el-input v-model="goodsEdit.formData.delivery_money" clearable placeholder="0.00"
                                class="input-width" maxlength="8">
                                <template #append>{{ t('yuan') }}</template>
                            </el-input>
                        </el-form-item>
                        <el-form-item :label="t('deliveryTemplateId')" prop="delivery_template_id"
                            v-show="goodsEdit.formData.delivery_type.indexOf('express') != -1 && goodsEdit.formData.is_free_shipping == 0 && goodsEdit.formData.fee_type == 'template'">
                            <el-select v-model="goodsEdit.formData.delivery_template_id"
                                :placeholder="t('deliveryTemplateIdPlaceholder')" filterable autocomplete="off"
                                clearable>
                ...
                <el-tab-pane :label="t('goodsArguments')" name="goods_arguments">
                    <el-form :model="goodsEdit.formData" label-width="120px" ref="goodsArgumentsFormRef"
                        :rules="goodsEdit.formRules" class="page-form">

                        <el-form-item :label="t('goodsArgumentsTemp')">
                            <div>
                                <el-select v-model="goodsEdit.formData.attr_id"
                                    :placeholder="t('goodsArgumentsTempPlaceholder')" clearable
                                    @change="goodsEdit.attrChange" @clear="goodsEdit.attrChange(-1)">
                                    <el-option v-for="item in goodsEdit.attrOptions" :key="item.attr_id"
                                        :label="item.attr_name" :value="item.attr_id" />
                                </el-select>
                                <el-button class='ml-2' @click="oneKeyAll"> 一键全选</el-button>
                                <div class="mt-[10px] text-[12px] text-[#999] leading-[20px]">
                                    {{ t('goodsArgumentsTempHint') }}</div>
                                            ...
                                            <th class="prompt-block">
                                                <div class="flex items-center">
                                                    ...
                                                    <el-tooltip class="box-item" effect="dark"
                                                        :content="t('argumentsSortHint')" placement="right">
                                                        <span
                                                            class="iconfont iconwenhao cursor-pointer ml-[3px] mt-[3px] !text-[14px]"></span>
                                                    </el-tooltip>
                                                </div>
                                            </th>
                                            <th>{{ t('operation') }}</th>
                                    ...
                                    <tbody class="attr-new">
                                        <template v-if="goodsEdit.attrTableData.length">
                                            <tr class="goods-attr-tr goods-new-attr-tr"
                                                v-for="(item, index) in goodsEdit.attrTableData" :key="index">
                                                <td v-if="item.attr_value_id > 0">{{ item.attr_value_name }}</td>
                                                <td v-else>
                                                    <el-input v-model.trim="item.attr_value_name" maxlength="20"
                                                        show-word-limit clearable />
                                                </td>
                                                <td>
                                                    <el-input v-if="item.type == 'text'" maxlength="20" show-word-limit
                                                        v-model.trim="item.select_child_val" clearable />
                                                    <div v-else-if="item.type == 'radio'">
                                                        <el-radio-group v-model="item.select_child_name"
                                                            @change="goodsEdit.attrRadioChange(index, $event)">

                                                            <el-radio v-for="(childItem, childIndex) in item.child"
                                                                :key="childIndex" :label="childItem.id">{{
                                                                    childItem.name }}</el-radio>
                                                        </el-radio-group>
                                                    </div>
                                                    <div v-else-if="item.type == 'checkbox'">
                                                        <el-checkbox-group v-model="item.select_child_name"
                                                            @change="goodsEdit.attrCheckboxChange(index, $event)">

                                                            <el-checkbox v-for="(childItem, childIndex) in item.child"
                                                                :key="childIndex" :label="childItem.id">{{
                                                                    childItem.name
                                                                }}</el-checkbox>
                                                        </el-checkbox-group>
                                                ...
                                                <td>
                                                    <el-input v-model.trim="item.sort" maxlength="6" show-word-limit
                                                        clearable @keyup="filterNumber($event)" />
                                                </td>
                                                <td>
                                                    <span class="cursor-pointer text-[var(--el-color-primary)]"
                                                        @click="goodsEdit.delAttr(index)">{{ t('delAttr') }}</span>
                                                </td>
                                            </tr>
                                        </template>
                                        <tr v-else>
                                ...
                                <el-button class="mt-[15px]" @click="goodsEdit.addAttr">{{ t('addGoodsArguments')
                                    }}</el-button>
                            </div>
                        </el-form-item>
                ...
                <el-tab-pane :label="t('goodsDesc')" name="detail">
                    <el-form :model="goodsEdit.formData" label-width="120px" ref="detailFormRef"
                        :rules="goodsEdit.formRules" class="page-form">
                        <el-form-item :label="t('goodsDesc')" prop="goods_desc">
                            <editor v-model="goodsEdit.formData.goods_desc" :height="600"
                                @handleBlur="goodsEdit.handleBlur" class="editor-width" />
                        </el-form-item>
                    </el-form>
                </el-tab-pane>
            </el-tabs>
...
} from '@/addon/phone_shop/api/delivery'
...
const oneKeyAll = () => {
    ...
    goodsEdit.attrTableData.forEach(attribute => {

        if (attribute.child.length > 0 && attribute.type == 'radio') {
            attribute.select_child_name = attribute.child[0].id;
