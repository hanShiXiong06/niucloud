...
import { addBatchOrderDelivery } from '@/addon/phone_shop/api/order'
...
const examineTemplate = () => {
    let url = `${import.meta.env.VITE_IMG_DOMAIN || location.origin}/addon/phone_shop/batch/batch_delivery_order.xls`
    if (importData.value.type == 'order_goods') {
        url = `${import.meta.env.VITE_IMG_DOMAIN || location.origin}/addon/phone_shop/batch/batch_delivery_order_goods.xls`
    }
    window.open(url)
}
