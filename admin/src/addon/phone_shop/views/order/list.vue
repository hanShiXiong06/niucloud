            ...
            <el-card class="box-card !border-none my-[10px] table-search-wrap" shadow="never">
                <el-form :inline="true" :model="orderTable.searchParam" ref="searchFormRef">
                    ...
                    <el-form-item :label="t('memberInfo')" prop='keyword'>
                        <el-input class="w-[200px]" v-model.trim="orderTable.searchParam.keyword"
            ...
            <div>
                ...
                <div class="mb-[10px] flex items-center" v-if="activeName == 3">
                    <el-button @click="batchPrintElectronicSheet" size="small" v-if="activeName == 3">{{
                        t('batchPrintElectronicSheet') }}</el-button>
                </div>
                                ...
                                <div
                                    ...
                                    <div>
                                        ...
                                        <span class="ml-5" v-if="item.pay">{{ t('payType') }}：{{ (item as
                                            any).pay.type_name }}</span>
                                    </div>
                                    <div>
                                        <!-- <el-button type="primary" link>{{ t('offlinePayment') }}</el-button> -->
                                        <el-button type="primary" link @click="printTicketEvent(item)"
                                            v-if="item.isSupportPrintTicket">{{ t('printTicket') }}</el-button>
                                        <el-button type="primary" link @click="openElectronicSheetPrintDialog(item)"
                                            v-if="item.isSupportElectronicSheet">{{ t('electronicSheetPrintTitle')
                                            }}</el-button>
                                        <el-button type="primary" link @click="detailEvent(item)">{{ t('info')
                                            }}</el-button>
                                        <el-button type="primary" link @click="setNotes(item)">{{ t('notes')
                                            }}</el-button>
                                    </div>
                                </div>

                                <el-table :data="item.order_goods" size="large" :show-header="false"
                                    :span-method="arraySpanMethod" ref="multipleTable" @select="handleSelectChange">
                                        ...
                                        <template #default="{ row }">
                                            <div class="flex cursor-pointer">
                                                ...
                                                <div class="flex flex-col">
                                                    ...
                                                    <span class="text-[12px] text-[#999] truncate">{{ row.sku_name
                                                        }}</span>
                                    ...
                                    <el-table-column min-width="120">
                                        <template #default="{ row }">
                                            <div class="flex flex-col">
                                                <span v-if="item.activity_type == 'exchange'">{{ row.extend.point }}{{
                                                    t('point') }}
                                    ...
                                    <el-table-column min-width="120"
                                        ...
                                        <template #default>
                                            <span v-if="item.activity_type == 'exchange'" class="text-[14px]">{{
                                                item.point }}{{ t('point') }}
                                                <span v-if="parseFloat(item.order_money)">+￥{{ item.order_money
                                                    }}</span>
                                            </span>
                                    ...
                                    <el-table-column align="right" min-width="120">
                                        <template #default>
                                            <template v-if="item.status == 1">
                                                ...
                                                <el-button type="primary" link @click="orderAdjustMoney(item)">{{
