					...
					<div class="mt-[10px]">
						<el-table :data="settlementTableData.data" size="large" v-loading="settlementTableData.loading"
							...
							<template v-if="activeName === 1">
								<el-table-column prop="technician_id" :label="t('ID')" min-width="80"
									:show-overflow-tooltip="true" />
								<!-- 师傅名称列 -->
								<el-table-column prop="real_name" :label="t('technicianName')" min-width="120"
									:show-overflow-tooltip="true" />
								<!-- 结算周期列 -->
								<el-table-column prop="month" :label="t('settlementCycle')" min-width="100"
									:show-overflow-tooltip="true" />
								<!-- 结算时间列 -->
								<!-- <el-table-column prop="total_amount" :label="t('settlementTime')" min-width="180"
									:show-overflow-tooltip="true" /> -->
								<!-- 汇总订单数列 -->
								<el-table-column prop="total_count" :label="t('totalOrders')" min-width="100"
									align="center" />
								<!-- 结算金额列 -->
								<el-table-column prop="settlementAmount" :label="t('settlementAmount')" min-width="120"
									align="center">
							...
							<template v-if="activeName === 2">
								<el-table-column prop="technician_id" :label="t('ID')" min-width="80"
									:show-overflow-tooltip="true" />
								<!-- 师傅名称列 -->
								<el-table-column prop="real_name" :label="t('technicianName')" min-width="150"
									:show-overflow-tooltip="true" />
								<!-- 结算订单数列 -->
								<el-table-column prop="settled_count" :label="t('settlementOrders')" min-width="120"
									align="center" />
								<!-- 结算佣金列 -->
								<el-table-column prop="settlementCommission" :label="t('settlementCommission')"
								...
								<!-- 金额合计列 -->
								<el-table-column prop="totalAmount" :label="t('totalAmount')" min-width="120"
									align="center">
									<template #default="{ row }">
										<span class="text-[12px]">￥</span>
										<span class="font-bold">{{ row.total_amount }}</span>
...
<script lang="ts" setup>
	...
	import { getTechnicianListFinance,getTechnicianListFinanceStat } from '@/addon/home_service/api/finance'
	// 路由相关
	...
	const export_type = ref('home_service_technician_settlement');
	...
	const loadSettlementList = (page : number = 1) => {
			...
			const url = activeName.value == 1 ? getTechnicianListFinance : getTechnicianListFinanceStat
			url({
	...
	const detailEvent = (row : any) => {
		router.push(`/home_service/finance/technician_detail?id=${row.technician_id}&date=${row.month}`);
	};
