<template>
	<div class="main-container" v-loading="loading">
		<el-card class="card !border-none mb-[15px]" shadow="never">
			<el-page-header :content="t('editstore')" :icon="ArrowLeft" @back="back" />
		</el-card>

		<el-card class="box-card !border-none" shadow="never">
			<el-form :model="formData" label-width="180px" ref="formRef" :rules="formRules" class="page-form" :disabled="formData.audit_status != 0">
				...
				<el-form-item :label="t('member')" prop="member_id">
					<!-- <div class="!w-[214px] border-[1px] border-[#e4e4e4] border-solid px-[11px] h-[30px] leading-[30px]" :class="formData.member.nickname ? 'text-[#000]' : 'text-[#999]'">
						{{ formData.member.nickname || t('chooseMemberPlaceholder') }}
					</div> -->
						<el-input v-model.trim="formData.member.nickname" clearable :placeholder="t('chooseMemberPlaceholder')"
						class="input-width !w-[214px]" disabled />
				...
				<el-form-item :label="t('licenseImg')" prop="license_img">
					<upload-image v-model="formData.license_img" />
				</el-form-item>
				<el-form-item :label="t('serviceRatio')" prop="service_ratio" v-if="formData.audit_status == 0">
					<el-input v-model.trim="formData.service_ratio" maxlength="6" clearable class="input-width !w-[150px]" :placeholder="t('serviceRatioPlaceholder')"
						max="100" @keyup="filterDigit($event)">
						<template #append>%</template>
					</el-input>
				</el-form-item>

				<el-form-item :label="t('applyReason')" prop="apply_desc">
					<el-input v-model.trim="formData.apply_desc" clearable :placeholder="t('applyReasonPlaceHolder')" type="textarea"
						class="input-width !w-[320px]" />
				</el-form-item>
				<el-form-item :label="t('applyText')" prop="audit_remark">
					<el-input v-model.trim="formData.audit_remark" clearable :placeholder="t('applyTextPlaceholder')" type="textarea"
						class="input-width !w-[320px]" />
				</el-form-item>
		...
		<div class="fixed-footer-wrap " v-if="formData.audit_status == 0">
			<div class="fixed-footer">
				<el-button type="primary" @click="onSave(formRef,1)" >{{ t('save') }}</el-button>
				<el-button  @click="onSave(formRef,-1)">{{ t('errorBtn') }}</el-button>
			</div>
		</div>
	</div>
...
<script lang="ts" setup>
	...
	const setFormData = async (id : number = 0) => {
		...
		const data = await (await getApplyDetail(id)).data
		...
		formData.address = formData.full_address
	...
	const formRules = computed(() => {
		return {
			...
			service_ratio: [{ required: true, trigger: 'blur',message:t('servicePlaceholder') }],
			// 地址相关验证
			...
			audit_remark: [{ required: true, message: t('applyTextPlaceholder'), trigger: 'blur' }],
			apply_desc: [{ required: true, message: t('applyReasonPlaceHolder'), trigger: 'blur' }],
			service_time: [
			    {
	...
	const onSave = async (formEl : FormInstance | undefined,status) => {
		...
		if(status == -1){
			...
			let value = formData.service_ratio
			if (!value) {
				ElMessage.error(t('oneRatePlaceholderOne'))
				return false;
			} else if (!regExp.digit.test(value)) {
				ElMessage.error(t('oneRatePlaceholderTwo'))
				return false;
			} else if (value >= 100) {
				ElMessage.error(t('oneRatePlaceholderThree'))
				return false;
			} else if (value < 0) {
				ElMessage.error(t('oneRatePlaceholderFour'))
				return false;
			} 
		}
		// 执行表单验证
