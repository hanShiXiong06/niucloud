<template>
	<div class="main-container" v-loading="loading">
		<el-card class="card !border-none mb-[15px]" shadow="never">
			<el-page-header :content="route.query.id?'编辑门店':t('addstore')" :icon="ArrowLeft" @back="back" />
		</el-card>
		...
		<el-card class="box-card !border-none" shadow="never">
			<el-form :model="formData" label-width="180px" ref="formRef" :rules="formRules" class="page-form">
				...
				<el-form-item :label="t('member')" prop="member_id">
					<div class="!w-[214px] border-[1px] border-[#e4e4e4] border-solid px-[11px] h-[30px] leading-[30px]" v-if="formData.member" :class="formData.member.nickname ? 'text-[#000]' : 'text-[#999]'">
		...
		await formEl.validate(async (valid) => {
			if (valid) {
				...
				try {
					...
					// 提交数据
					const response = await editStore(params)
					// 处理成功结果
					history.back()
				} catch (error : any) {
					// 处理错误
