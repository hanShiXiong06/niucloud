  ...
  <div class="main-container">
    ...
    <el-card class="box-card rounded-lg !border-none" shadow="never">
      ...
      >
        ...
        >
          <el-form-item
            :label="t('device_query.result.queryCode')"
            prop="query_code"
          >
            <el-input
          ...
          <el-form-item
            :label="t('device_query.result.queryType')"
            prop="query_type"
          >
            <el-select
              v-model="queryParams.query_type"
              :placeholder="t('device_query.result.queryTypePlaceholder')"
              class="input-width"
            >
              <el-option :label="t('device_query.result.all')" value="" />
              <el-option
                :label="t('device_query.result.queryTypes.imei')"
                value="imei"
              />
              <el-option
                :label="t('device_query.result.queryTypes.serial')"
                value="serial"
              />
              <el-option
                :label="t('device_query.result.queryTypes.model')"
                value="model"
              />
              <el-option
                :label="t('device_query.result.queryTypes.coverage')"
                value="coverage"
              />
              <el-option
                :label="t('device_query.result.queryTypes.activationlock')"
                value="activationlock"
              />
            </el-select>
          </el-form-item>
          <el-form-item :label="t('device_query.result.status')" prop="status">
            <el-select
              ...
              :placeholder="t('device_query.result.statusPlaceholder')"
              class="input-width"
            >
              <el-option :label="t('device_query.result.all')" value="" />
              <el-option
                :label="t('device_query.result.statusOptions.success')"
                :value="1"
              />
              <el-option
                :label="t('device_query.result.statusOptions.failed')"
                :value="0"
              />
            </el-select>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="loadList()">{{
              t("device_query.result.search")
            }}</el-button>
            <el-button @click="resetSearch">{{
              t("device_query.result.reset")
            }}</el-button>
          </el-form-item>
        </el-form>
      </el-card>
      ...
      <div class="mt-[10px]">
        ...
        >
          ...
          <el-table-column
            :label="t('device_query.result.queryType')"
            prop="query_type_name"
            min-width="100"
          />
          <el-table-column
            :label="t('device_query.result.api_endpoint')"
            prop="api_endpoint"
            min-width="200"
          />
          <el-table-column
            :label="t('device_query.result.status')"
            prop="status"
            min-width="80"
          >
          ...
          <el-table-column
            :label="t('device_query.result.cost')"
            prop="cost_amount"
            min-width="80"
          >
            <template #default="{ row }"> ¥{{ row.cost_amount }} </template>
          </el-table-column>
          <el-table-column
            :label="t('device_query.result.responseTime')"
            prop="response_time"
            min-width="80"
          >
          ...
          <el-table-column
            :label="t('device_query.result.operator')"
            prop="operator_name"
            min-width="100"
          />
          <el-table-column
            :label="t('device_query.result.queryTime')"
            prop="create_at"
            min-width="160"
          >
            <template #default="{ row }">
          ...
          <el-table-column
            :label="t('device_query.result.operation')"
            fixed="right"
            width="180"
          >
            <template #default="{ row }">
              <el-button type="primary" link @click="viewEvent(row)">{{
                t("device_query.result.view")
              }}</el-button>
              <el-button
                type="warning"
                link
                @click="requeryEvent(row)"
                v-if="row.status !== 1"
                >{{ t("device_query.result.requery") }}</el-button
              >
              <el-button type="danger" link @click="deleteEvent(row.id)">{{
                t("device_query.result.delete")
              }}</el-button>
            </template>
          </el-table-column>
        </el-table>
        ...
        <div class="mt-[16px] flex justify-between">
          <div>
            ...
            >
              {{ t("device_query.result.batchDelete") }} ({{
                selectedIds.length
              }})
            </el-button>
          </div>
    ...
    <el-dialog
      ...
      :title="t('device_query.result.detailDialog')"
      width="90%"
      max-height="80vh"
    >
      <div v-if="viewData">
        ...
        <div v-if="isWarrantyQuery(viewData)" class="warranty-result-container">
          <WarrantyInfoDisplay
            :warrantyData="parseWarrantyData(viewData.query_result)"
          />
        </div>
        ...
        <div v-else>
          <el-descriptions :column="2" border>
            <el-descriptions-item :label="t('device_query.result.queryCode')">{{
              viewData.query_code
            }}</el-descriptions-item>
            <el-descriptions-item :label="t('device_query.result.queryType')">{{
              viewData.query_type_name
            }}</el-descriptions-item>
            <el-descriptions-item
              :label="t('device_query.result.apiEndpoint')"
              >{{ viewData.api_endpoint }}</el-descriptions-item
            >
            <el-descriptions-item :label="t('device_query.result.apiName')">{{
              viewData.api_name
            }}</el-descriptions-item>
            <el-descriptions-item :label="t('device_query.result.status')">{{
              viewData.status_name
            }}</el-descriptions-item>
            <el-descriptions-item :label="t('device_query.result.cost')"
              >¥{{ viewData.cost_amount }}</el-descriptions-item
            >
            <el-descriptions-item :label="t('device_query.result.responseTime')"
              >{{ viewData.response_time }}ms</el-descriptions-item
            >
            <el-descriptions-item :label="t('device_query.result.operator')">{{
          ...
          <div class="mt-4">
            <el-tabs>
              <el-tab-pane :label="t('device_query.result.queryResult')">
                <pre class="bg-gray-100 p-4 rounded max-h-96 overflow-auto">{{
                  JSON.stringify(viewData.query_result, null, 2)
                }}</pre>
              </el-tab-pane>
              <el-tab-pane :label="t('device_query.result.rawResponse')">
                <pre class="bg-gray-100 p-4 rounded max-h-96 overflow-auto">{{
                  JSON.stringify(viewData.raw_response, null, 2)
                }}</pre>
              </el-tab-pane>
            </el-tabs>
          </div>
...
const deleteEvent = (id: number) => {
  ElMessageBox.confirm(
    t("device_query.result.deleteConfirm"),
    t("device_query.result.warning"),
    {
...
const batchDelete = () => {
  ...
  ElMessageBox.confirm(
    t("device_query.result.batchDeleteConfirm").replace(
      "{count}",
      selectedIds.value.length.toString()
    ),
    t("device_query.result.batchDeleteTitle"),
    {
      confirmButtonText: t("device_query.result.confirmText"),
      cancelButtonText: t("device_query.result.cancelText"),
      type: "warning",
...
const cleanCache = () => {
  ElMessageBox.confirm(
    t("device_query.result.cleanCacheConfirm"),
    t("device_query.result.warning"),
    {
      confirmButtonText: t("device_query.result.confirmText"),
      cancelButtonText: t("device_query.result.cancelText"),
    ...
    try {
      ...
      ElMessage.success(
        t("device_query.result.cleanSuccess").replace(
          "{count}",
          data.data.cleaned_count.toString()
        )
      );
    } catch (error) {
      console.error(error);
...
const exportData = async () => {
  try {
    ...
    ElMessage.success(
      t("device_query.result.exportSuccess").replace(
        "{count}",
...
const parseWarrantyData = (queryResult: any) => {
  ...
  try {
    ...
    const data =
      typeof queryResult === "string" ? JSON.parse(queryResult) : queryResult;
