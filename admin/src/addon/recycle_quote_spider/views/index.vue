...
const downloadExcelTemplate = () => {
    const rows = [
        ['型号', '品牌', '内存', '靓机', '小花', '内爆', '备注'],
        ['iPhone 15 Pro Max 256G', '苹果', '256GB', 5200, 5000, 4300, '正常回收报价'],
        ['iPhone 15 Pro 128G', '苹果', '128GB', 4500, 4300, 3600, ''],
        ['Mate 60 Pro 512G', '华为', '12+256GB', 4100, 3900, 3300, '']
    ]
    const tips = [
        ['字段', '是否必填', '说明'],
        ['型号', '必填', '每一行会导入为一个行价格；没有型号的行会被跳过。'],
        ['品牌', '选填', '为空时使用导入表单里填写的品牌。'],
        ['内存', '选填', '为空时使用导入表单里填写的内存。'],
