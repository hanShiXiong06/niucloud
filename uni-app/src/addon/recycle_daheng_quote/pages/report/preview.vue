...
function buildGroups(ctx: Ctx) {
	...
	ctx.rows.forEach((r: any) => {
		...
		if (!g) {
			...
			g = { model: r.model, cols, signature, gw, headerH: headerHeightFor(cols, gw), items: [] }
			m.set(key, g)
			groups.push(g)
		}
		g.items.push(r)
	...
	groups.forEach(g => {
		...
		balanceMergedRemarkHeights(g, ctx)
...
function balanceMergedRemarkHeights(group: any, ctx: Ctx) {
	if (!group.remarkCols?.length) return
	group.remarkCols.forEach((col: string) => {
		for (let i = 0; i < group.items.length; i++) {
			const value = remarkValue(group.items[i], col)
			const normalized = normalizeRemarkText(value)
			if (!normalized) continue
			let span = 1
			while (i + span < group.items.length && normalizeRemarkText(remarkValue(group.items[i + span], col)) === normalized) {
				span++
			}
			if (span <= 1) continue
			const required = Math.max(44, estLines(value, group.remarkW, FONT_REMARK) * LH + VPAD)
			let current = 0
...
function buildColumnStack(groups: any[], ctx: Ctx): { views: any[]; height: number } {
	...
	function pushHeader(group: any) {
		...
		prevSignature = group.signature
	}

	// 数据
	groups.forEach((group, gi) => {
		if (group.signature !== prevSignature) pushHeader(group)
		const gw = group.gw
		...
		group.remarkCols.forEach((col: string) => {
			...
			for (let ri = 0; ri < group.items.length; ri++) {
				const value = remarkValue(group.items[ri], col)
				const normalized = normalizeRemarkText(value)
				let span = 1
				if (normalized) {
					while (ri + span < group.items.length && normalizeRemarkText(remarkValue(group.items[ri + span], col)) === normalized) span++
				}
				let h = 0
				...
				remarkRows.push({
					type: 'view',
					css: { width: group.remarkW + 'rpx', height: h + 'rpx', display: 'flex', alignItems: 'center', justifyContent: 'center', background: bg, boxSizing: 'border-box', borderLeft: `1rpx solid ${t.lineColor}` },
					views: [txt(value || '', group.remarkW, { color: t.subText, fontSize: FONT_REMARK + 'rpx', lineHeight: '26rpx' })]
				})
				ri += span - 1
			}
...
function buildSheetBoards(ctx: Ctx) {
	...
	let h = 0
	groups.forEach(g => {
		const prev = cur.length ? cur[cur.length - 1].signature : ''
		const blockH = (prev !== g.signature ? g.headerH : 0) + g.height
		...
		cur.push(g)
		h += (h === 0 ? g.headerH : (prev !== g.signature ? g.headerH : 0)) + g.height
	})
...
const GEN_FILES_KEY = 'qr_dh_gen_files'
