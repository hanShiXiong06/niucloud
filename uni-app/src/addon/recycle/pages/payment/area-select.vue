...
watch(() => selected.city, (newVal) => {
    ...
    if (newVal) {
        getAreaListByPid(newVal.id).then((res: any) => {
            ...
            if (data.length === 0) { // 如果没有区县数据，说明city就是最后一级（直辖市的区）
                // 将city作为district，因为直辖市的city实际上就是区县
                // 必须先保存city的值，再清空city，最后设置district
                const tempCity = selected.city
                selected.city = null
                // 清空city列表
                areaList.city = []
                // 最后设置district，触发完成逻辑
                selected.district = tempCity
            }
        }).catch()
    } else {
        areaList.district = []
