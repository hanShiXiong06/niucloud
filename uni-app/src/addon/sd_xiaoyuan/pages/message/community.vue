            ...
            <view class="message-list-content">
                ...
                >
                    <view class="message-avatar">
                        <view class="text-avatar">{{ (item.title || '社').charAt(0) }}</view>
...
const loadMessages = async (refresh = false) => {
    ...
    loading.value = true
    try {
        const res: any = await getMessageList({ page: page.value, limit, type: 'COMMUNITY' })
        if (res.code === 1) {
...
.message-item {
    ...
    .message-avatar {
        ...
        .text-avatar {
            width: 80rpx; height: 80rpx; border-radius: 50%; background: linear-gradient(135deg, #43e97b, #38f9d7);
