<template>
    <view class="order-detail">
        ...
        <view class="info-section">
            ...
            <view class="info-item column" v-if="order.yinsi_text">
                <text class="label">隐私信息</text>
                <text class="value yinsi-pre">{{ order.yinsi_text }}</text>
            </view>
            ...
            <!-- 兼职招聘 PARTTIME -->
            <view class="info-item" v-if="order.task_type === 'PARTTIME' && extData.job_type">
                <text class="label">岗位类型</text>
                <text class="value">{{ extData.job_type }}</text>
            </view>
            <view class="info-item" v-if="order.task_type === 'PARTTIME' && extData.salary">
                <text class="label">薪资说明</text>
                <text class="value">{{ extData.salary }}</text>
            </view>
            <view class="info-item" v-if="order.task_type === 'PARTTIME' && extData.work_time">
                <text class="label">工作时间</text>
                <text class="value">{{ extData.work_time }}</text>
            </view>
            <view class="info-item" v-if="order.task_type === 'PARTTIME' && extData.location">
                <text class="label">工作地点</text>
                <text class="value">{{ extData.location }}</text>
            </view>
            <view class="info-item" v-if="order.task_type === 'PARTTIME' && extData.contact">
                <text class="label">联系方式</text>
                <text class="value">{{ extData.contact }}</text>
            </view>
            <view class="info-item" v-if="order.task_type === 'PARTTIME' && extData.recruit_count">
                <text class="label">招聘人数</text>
                <text class="value">{{ extData.recruit_count }}人</text>
            </view>
            <!-- 约伴组局 COMPANION -->
            <view class="info-item" v-if="order.task_type === 'COMPANION' && extData.activity_type">
                <text class="label">活动类型</text>
                <text class="value">{{ extData.activity_type }}</text>
            </view>
            <view class="info-item" v-if="order.task_type === 'COMPANION' && extData.activity_time">
                <text class="label">活动时间</text>
                <text class="value">{{ extData.activity_time }}</text>
            </view>
            <view class="info-item" v-if="order.task_type === 'COMPANION' && extData.location">
                <text class="label">活动地点</text>
                <text class="value">{{ extData.location }}</text>
            </view>
            <view class="info-item" v-if="order.task_type === 'COMPANION' && extData.people_count">
                <text class="label">人数需求</text>
                <text class="value">{{ extData.people_count }}人</text>
            </view>
            <view class="info-item" v-if="order.task_type === 'COMPANION' && extData.budget">
                <text class="label">费用说明</text>
                <text class="value">{{ extData.budget }}</text>
            </view>
            <!-- 代清洁 CLEAN -->
        ...
        <!-- 底部操作 -->
        <view class="action-bar" v-if="showMainActions">
            <view class="btn-cancel" v-if="canCancelOrder" @click="cancelOrder">取消订单</view>
            <view class="btn-pay" v-if="order.status === 0" @click="payOrder">立即支付 ¥{{ order.actual_fee }}</view>
            <view class="btn-evaluate" v-if="order.status === 45" @click="doConfirmOrder">确认收货</view>
        </view>
        <view class="action-bar" v-if="order.status === 50 && (!order.is_evaluated)">
            <view class="btn-tip" @click="showTipPopup = true">打赏接单员</view>
            <view class="btn-evaluate" @click="goToEvaluate">评价订单</view>
        </view>
        ...
        <view class="tip-popup" v-if="showTipPopup">
            ...
            <view class="tip-btns">
                <view class="tip-btn-cancel" @click="showTipPopup = false">取消</view>
                <view class="tip-btn-confirm" @click="submitTip">确认打赏</view>
            </view>
        </view>
...
import { getOrderDetail, cancelOrder as cancelOrderApi, confirmOrder, getRunnerLocation, tipOrder } from '../../api/xiaoyuan'
...
const canCancelOrder = computed(() => [0, 10, 20].includes(Number(order.value?.status)))
const showMainActions = computed(() => canCancelOrder.value || [0, 45].includes(Number(order.value?.status)))
...
const doConfirmOrder = () => {
    uni.showModal({
        title: '确认收货',
        content: '确认后订单将完成，是否继续？',
        success: async (res) => {
            if (!res.confirm) return
            try {
                const result: any = await confirmOrder({ id: order.value.id })
                if (result.code === 1) {
...
.status-section {
    ...
    &.status-bg-10, &.status-bg-20, &.status-bg-30, &.status-bg-40, &.status-bg-45 {
...
.action-bar {
    ...
    .btn-pay, .btn-evaluate {
        background: linear-gradient(135deg, #c0fe95, #88f78d);
        color: #333;
        box-shadow: 0 4rpx 16rpx rgba(192, 254, 149, 0.4);
        
        &:active {
            transform: scale(0.98);
            box-shadow: 0 2rpx 12rpx rgba(192, 254, 149, 0.4);
        }
    }
...
.tip-btns {
    ...
    .tip-btn-cancel, .tip-btn-confirm {
        flex: 1;
        height: 88rpx;
        line-height: 88rpx;
        font-size: 30rpx;
    ...
    .tip-btn-confirm {
        ...
        &:active {
            transform: scale(0.98);
            box-shadow: 0 2rpx 12rpx rgba(255, 149, 0, 0.4);
        }
    }
}
</style>
