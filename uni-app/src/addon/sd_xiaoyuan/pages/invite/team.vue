<template>
    <view class="team-page">
        ...
        <scroll-view scroll-y class="team-list" @scrolltolower="loadMore">
            <view class="team-item" v-for="item in teamList" :key="item.member_id">
                ...
                <view class="info">
                    ...
                    <text class="time">加入时间：{{ formatTime(item.create_time) }}</text>
                </view>
...
const loadTeam = async (refresh = false) => {
    ...
    loading.value = true
    const res: any = await getMyTeam({ level: currentTab.value, page: page.value, limit: 20 })
    loading.value = false
