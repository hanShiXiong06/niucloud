<template>
  <view class="order-container">
    ...
    <!-- 回收须知 -->
      <view class="content-card notice-card" @click="openNoticePopup">
      <view class="card-title">
          <u-icon name="info-circle" color="#4CAF50" size="32rpx"></u-icon>
        <text>回收须知</text>
      </view>
      <view class="notice-entry">
        <text class="notice-entry-text">点击查看回收须知</text>
        <u-icon name="arrow-right" color="#BBBBBB" size="28rpx"></u-icon>
      </view>
    </view>

    <u-popup :show="noticePopupShow" mode="center" :round="24" :closeable="true" @close="closeNoticePopup">
      <view class="notice-popup">
        <view class="notice-popup-header">
          <text class="notice-popup-title">回收须知</text>
        </view>
        <scroll-view class="notice-popup-scroll" scroll-y="true">
          <view class="notice-popup-list">
            <view class="notice-popup-item" v-for="(item, index) in noticeLines" :key="index">
              <view class="notice-dot"></view>
              <text class="notice-popup-text">{{ item }}</text>
            </view>
          </view>
        </scroll-view>
        <view class="notice-popup-footer">
          <view class="notice-popup-btn" @click="confirmNoticeAndContinue">我已阅读，继续下单</view>
        </view>
      </view>
    </u-popup>
    </view>
...
export default {
  data() {
    return {
      ...
      recycleNoticeContent: '',
      noticeConfirmed: false,
      noticeSubmitPending: false,
      loading: false,
  ...
  computed: {
    ...
    noticeLines() {
      return (this.recycleNoticeContent || '')
        .split(/\r?\n/)
        .map(item => item.trim())
        .filter(item => item);
    },
  ...
  methods: {
    ...
    async loadSystemConfig() {
      try {
        ...
        if (res.code === 1 && res.data) {
          ...
          this.recycleNoticeContent = res.data.recycle_notice_content || '';
    ...
    closeNoticePopup() {
      ...
      this.noticeSubmitPending = false;
    },
    confirmNoticeAndContinue() {
      this.noticeConfirmed = true;
      const shouldContinueSubmit = this.noticeSubmitPending;
      this.noticeSubmitPending = false;
      this.noticePopupShow = false;
