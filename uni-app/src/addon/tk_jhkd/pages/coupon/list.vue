<template>
	<view class="bg-[var(--page-bg-color)] min-h-[100vh] overflow-hidden" :style="themeColor()">
			...
			<view v-if="list.length" class="pb-[var(--top-m)] sidebar-margin">
				<template v-for="(item, index) in list">
					...
					<view v-else
						...
						<view v-if="item.btnType === 'using'" 
							class="pr-[20rpx] pl-[34rpx]">
							<button @click.stop="toLink(item.id)" v-if="item.use_member_count != 0" class="flex-center"
							...
							<button @click.stop="showTip()" v-else class="flex-center"
								:style="{ width: '150rpx', height: '60rpx', color: 'var(--primary-color)', fontSize: '24rpx', padding: '0', backgroundColor: 'transparent', border: '2rpx solid var(--primary-color)', borderRadius: '30rpx' }">不可领</button>
		...
		<up-popup :show="typePopup" mode="top" @close="typePopup = false" :customStyle="{ top: typePopupTopVal }"
...
:deep(.up-popup .up-popup__content) {
