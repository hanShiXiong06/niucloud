...
export function useGoodsDownload() {
    ...
    const requestAlbumPermission = (): Promise<boolean> => {
        return new Promise((resolve) => {
            ...
            // #ifdef MP
            // 小程序环境(uni.authorize 仅小程序/App 存在)
            uni.authorize({
                scope: 'scope.writePhotosAlbum',
                success: () => resolve(true),
                fail: () => {
            ...
            // #ifdef H5
            // H5(浏览器)无相册授权 API,直接放行(保存图片由浏览器另存处理),避免 uni.authorize 报错
            resolve(true)
            // #endif
        })
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            const downloadTask = uni.downloadFile({
                ...
                success: (res) => {
                    ...
                    console.log('返回的 tempFilePath:', res.tempFilePath)
                    console.log('指定的 filePath:', filePath)

                    if (res.statusCode === 200) {
                        // 当指定了 filePath 时，某些平台返回 tempFilePath，某些返回 filePath
                        // 优先使用 tempFilePath，如果不存在则使用我们指定的 filePath
