        ...
        <view class="container rounded-[var(--rounded-big)] px-[24rpx] py-[30rpx] mb-[20rpx]">
            ...
            <view class="space-y-4 mt-[30rpx]">
                <view v-for="(item, index) in list" :key="index" class="rounded-[20rpx] flex">
                    <up-image radius="var(--goods-rounded-big)" width="180rpx" height="180rpx" :src="img(item.goods.goods_cover)" model="aspectFill" @click="checkDetail(item)">
                        <template #error>
...
import { getShopImpulseBuyList, getShopImpulseBuyConfig } from '@/addon/phone_shop/api/impulse_buy'
import goodsDetail from '@/addon/phone_shop/pages/order/components/goods-detail/goods-detail.vue'
