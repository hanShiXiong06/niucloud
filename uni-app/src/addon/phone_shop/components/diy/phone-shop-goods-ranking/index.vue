                        ...
                        <view class="mt-[24rpx]">
                            <view class="flex bg-[rgba(255,255,255,0.94)] p-[10rpx] rounded-[16rpx] mb-[16rpx]"
                                ...
                                <view class="relative w-[130rpx] h-[130rpx] mr-[16rpx]">
                                    ...
                                    <up-image radius="var(--goods-rounded-big)" width="130rpx" height="130rpx" :src="img(goods.goods_cover_thumb_mid || '')" :mode="diyComponent.mode">
                                        <template #error>
...
import { getRankComponentsGoodsList } from '@/addon/phone_shop/api/rank'
...
const refresh = () => {
    // 装修模式下刷新
    if (diyStore.mode == 'decorate') {
        if (diyComponent.value && diyComponent.value.componentName == 'PhoneShopGoodsRanking') {
            const fakeGoods = {
                goods_name: '商品名称',
...
function getRankBadge(sort: any) {
    switch (sort) {
        case 1:
            return img('addon/phone_shop/rank/rank_first.png');
        case 2:
            return img('addon/phone_shop/rank/rank_second.png');
        case 3:
            return img('addon/phone_shop/rank/rank_third.png');
        default:
            return img('addon/phone_shop/rank/rank.png');
    }
}
