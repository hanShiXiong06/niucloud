        ...
        <view class="w-full relative overflow-hidden" :style="mediumCss">
            <view class="absolute top-0 left-0 w-full h-full transition-transform duration-300 ease-linear transform"
                ...
                <view class="swiper-box">
                    <u-swiper :list="swiperImageList"
                                @change="swiperChangeFn"
                                :indicator="(swiperImageList.length != 1 || diyStore.mode == 'decorate') && diyComponent.medium.indicator"
            ...
            <view class="media-mode bg-[rgb(0,0,0,.5)] flex items-center rounded-[50rpx] p-[4rpx] absolute bottom-[130rpx] right-[20rpx] text-center leading-[46rpx]" v-if="diyComponent.goods.goods_video != ''">
                ...
                <view class="tab-item flex items-center" :class="{ '!bg-[#fff] !text-[#666]': switchMedia == 'img' }" @click="switchToImage">
                    ...
                    <text v-if="switchMedia == 'img' && swiperImageList.length > 1">{{swiperCurrentIndex}}/{{swiperImageList.length}}</text>
                </view>
            </view>
        </view>
...
// 商品图片可能来自 Excel、外部平台或手工录入，统一走公共图片地址处理。
// 同时兼容 u-swiper 的字符串列表和对象列表，避免完整 URL 被拼到当前页面路径后。
const swiperImageList = computed(() => {
    const list = diyComponent.value?.goods?.goods_image
    if (!Array.isArray(list)) return []

    return list.map((item: any) => {
        if (typeof item == 'string') return img(item)
        if (!item || typeof item != 'object') return item

        const normalized: any = { ...item }
        ;['url', 'src', 'image', 'imageUrl'].some((field) => {
            if (typeof normalized[field] != 'string') return false
            normalized[field] = img(normalized[field])
            return true
        })
        return normalized
    }).filter(Boolean)
...
const swiperPreviewList = computed(() => swiperImageList.value.map((item: any) => {
    if (typeof item == 'string') return item
    return item?.url || item?.src || item?.image || item?.imageUrl || ''
}).filter(Boolean))
...
const swiperClick = (index: any) => {
    if (typeof index == 'number') imgListPreview(swiperPreviewList.value, index)
}
...
onMounted(() => {
    ...
    if (diyStore.mode == 'decorate') {
        watch(
            () => diyComponent.value,
            (newValue, oldValue) => {
                if (newValue && newValue.componentName == 'PhoneShopGoodsDetailBasicInfo') {
                    nextTick(() => {
                        saleInfo.value = (typeof diyComponent.value?.saleInfo == 'object') ? diyComponent.value?.saleInfo : diyComponent.value?.saleInfo.split(',');
                    })
                }
