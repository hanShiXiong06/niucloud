    ...
    <x-skeleton :type="skeleton.type" :loading="skeleton.loading" :config="skeleton.config" v-if="couponList && Object.keys(couponList).length > 0">
        <view :style="warpCss" class="overflow-hidden">
            ...
            <view v-if="diyComponent.style == 'style-1'" class="coupon-wrap style-1 relative">
                <scroll-view scroll-x="true" class="coupon-list" :style="{'background-image':'url(' + img('addon/phone_shop/diy/goods_coupon/style1_bg2.png') + ')','background-size':'100%','background-repeat':'no-repeat'}">
            ...
            <view v-else-if="diyComponent.style == 'style-2'" class="coupon-wrap style-2 relative">
                ...
                <view
                    ...
                    <text v-if="diyComponent.btnText" @click="toLink('/addon/phone_shop/pages/coupon/list')" class="bg-[#fff] text-[#FF4142] text-[22rpx] min-w-[100rpx] px-[24rpx] box-border h-[50rpx] leading-[50rpx] text-center coupon-buy-btn mt-auto">{{ diyComponent.btnText }}</text>
                </view>
            </view>

            <view v-else-if="diyComponent.style == 'style-3'" class="coupon-wrap style-3 relative"
                  :style="{'background-image':'url(' + img('addon/phone_shop/diy/goods_coupon/style3_bg.jpg') + ')','background-size':'100% 204rpx','background-repeat':'no-repeat'}">
            ...
            <view v-else-if="diyComponent.style == 'style-4'" class="coupon-wrap style-4 relative" :style="couponStyle4Css">
                <view class="desc flex items-center pt-[6rpx] pb-[26rpx]" @click="toLink('/addon/phone_shop/pages/coupon/list')">
...
import { getShopCouponComponents, getCoupon } from '@/addon/phone_shop/api/coupon';
...
const couponItemLink = (data: any) => {
    // redirect({ url: '/addon/phone_shop/pages/coupon/detail', param: { coupon_id: data.id } })
    collecting(data.id)
}
