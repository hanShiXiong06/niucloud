...
export function getEvaluateInfo(id: any) {
    return request.get(`phone_shop/goods/evaluate/${ id }`)
}
...
export function setOrderEvaluate(params: Record<string, any>) {
    return request.post('phone_shop/goods/evaluate', params, { showSuccessMessage: true })
}
...
export function getOrderEvaluate(id: any) {
    return request.get(`phone_shop/order/evaluate/${ id }`)
}
