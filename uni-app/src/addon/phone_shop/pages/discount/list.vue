<template>
    <view class="discount bg-[var(--page-bg-color)] min-h-[100vh]" :style="themeColor()">
        <view class="fixed top-0 left-0 w-full z-10 text-[0]">
            ...
            <image v-if="!bannerListLoading&&!bannerList.length" :src="img('addon/phone_shop/discount/discount_banner.png')" mode="scaleToFill" class="w-full" :style="{height: headStyle}" :show-menu-by-longpress="true" />
                        ...
                        <view class="w-[187.5rpx] h-[100rpx] relative flex-shrink-0" v-for="(item,index) in discountList" @click="navClick(item)">
                            ...
                            <template v-if="active==item.discount_id">
                                <image v-if="discountList.length<4" class="absolute bottom-0 z-5 h-[110rpx] z-5"
                                       ...
                                       :src="img(index==0?'addon/phone_shop/discount/nav-left.png':'addon/phone_shop/discount/nav-center.png')" />
                                <image v-if="discountList.length>=4" class="absolute bottom-0 z-5 h-[110rpx] z-5"
                                       ...
                                       :src="img(index==0?'addon/phone_shop/discount/nav-left.png':index==discountList.length-1?'addon/phone_shop/discount/nav-right.png':'addon/phone_shop/discount/nav-center.png')" />
                            </template>
                        </view>
                    </view>
                </scroll-view>
                ...
                <template v-for="(item,index) in list" :key="index">
                    ...
                    <view class="bg-[#fff] p-[20rpx] flex rounded-[var(--rounded-big)]" :class="{'mb-[var(--top-m)]':index<list.length-1}" @click="toLink(item)">
                        <view class="w-[240rpx] h-[240rpx] rounded-[var(--goods-rounded-big)] overflow-hidden">
                            <up-image width="240rpx" height="240rpx" :radius="'var(--goods-rounded-big)'" :src="img(item.goods_cover ? item.goods_cover_thumb_mid : '')" model="aspectFill">
                                <template #error>
        ...
        <view v-if="!discountList.length && !loading" class="h-[calc(100vh-550rpx)]" :style="{'padding-top':mescrollTop}">
            <mescroll-empty :option="{tip : '暂无商品，请看看其他商品吧！','btnText': '去逛逛'}" @emptyclick="redirect({ url: '/addon/phone_shop/pages/goods/list' })"></mescroll-empty>
        </view>
...
import { getActiveDiscountConfig, getActiveDiscountList, getActiveDiscountGoodsList } from '@/addon/phone_shop/api/discount'
...
const toLink = (item: any) => {
    ...
    redirect({ url: '/addon/phone_shop/pages/goods/detail', param: { sku_id: item.goodsSku.sku_id, type: 'discount' } })
}
