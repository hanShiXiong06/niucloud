<template>
    <view class="min-h-[100vh] bg-[#f6f6f6] overflow-hidden" :style="themeColor()">
        <template v-if="Object.keys(configInfo).length &&configInfo.active_status =='active'">
            <mescroll-body ref="mescrollRef" @init="mescrollInit" :down="{ use: false }" @up="getGoodsListFn">
                <view class="marketing-head" v-if="!pageLoading">
                    ...
                    <!-- #endif -->
                    <view class="time newcomer-time" v-if="newcomerTime > 0 && newcomerTime != undefined" :style="{'background-image': 'url('+ img('addon/phone_shop/newcomer/time_bg.png') +')'}">
                    ...
                    <view class="bg-[#fff] flex rounded-[var(--rounded-mid)] p-[20rpx]"
                        ...
                        <view class="flex flex-col flex-1 ml-[20rpx] pt-[4rpx]">
                            ...
                            <view class="w-[400rpx] flex items-center justify-center">
                                <image class="w-[400rpx] h-[106rpx] mt-[auto] mb-[10rpx]" :src="img('addon/phone_shop/newcomer/subsidy.png')" mode="aspectFit"/>
                            </view>
                            <view class="flex items-center justify-center btn-wrap">
                                ...
                                <image class="w-[26rpx] h-[54rpx]" :src="img('addon/phone_shop/newcomer/btn_02.png')" mode="heightFix"/>
                                <image class="w-[84rpx] h-[54rpx] relative ml-[-5rpx] z-2" :src="img('addon/phone_shop/newcomer/btn_03.png')" mode="aspectFit" />
                            </view>
                        </view>
                    </view>
                </view>
                <mescroll-empty v-else-if="!goodsList.length && loading" :option="{tip : '暂无商品，请看看其他商品吧！', btnText:'去逛逛'}" @emptyclick="redirect({ url: '/addon/phone_shop/pages/goods/list' })"></mescroll-empty>
            </mescroll-body>
        </template>
        <template v-if="!pageLoading && Object.keys(configInfo).length && configInfo.active_status !='active'">
            ...
            <mescroll-empty :option="{tip : '活动未开启,请看看其他商品吧！', btnText:'去逛逛'}" @emptyclick="redirect({ url: '/addon/phone_shop/pages/index' })"></mescroll-empty>
        </template>
...
import { getNewcomerGoodsList, getNewcomersConfig } from '@/addon/phone_shop/api/newcomer';
