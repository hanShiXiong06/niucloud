            ...
            <view class="py-[32rpx] relative" v-if="goodsDetail.detail" @touchmove.prevent.stop>
                <view class="flex px-[32rpx] mb-[40rpx]">
                    <view class="w-[180rpx] h-[180rpx]">
                        <up-image width="180rpx" height="180rpx" :radius="'var(--goods-rounded-big)'" :src="img(goodsDetail.detail.sku_image)" @click="imgListPreview(goodsDetail.detail.sku_image)" model="aspectFill">
                            <template #error>
...
const confirm = () => {
    ...
    // 检测是否登录
    if (!userInfo.value) {
        useLogin().setLoginBack({ url: '/addon/phone_shop/pages/point/detail', param: { id: goodsDetail.value.exchange_id } })
        return false
    }
    ...
    uni.setStorage({
        ...
        success: () => {
            const url = goodsDetail.value.is_join_exchange ? '/addon/phone_shop/pages/point/payment' : '/addon/phone_shop/pages/order/payment'
            redirect({ url })
        }
    });
