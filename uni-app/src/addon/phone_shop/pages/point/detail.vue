        ...
        <view class="bg-[var(--page-bg-color)] min-h-[100vh] relative" v-if="Object.keys(goodsDetail).length">
            ...
            <view class="bg-[var(--page-bg-color)] overflow-hidden rounded-[40rpx] -mt-[28rpx] relative">
                ...
                <view class="border-[0] border-t-[2rpx] border-solid border-[#f5f5f5] w-[100%] flex justify-between pl-[32rpx] pr-[4rpx] bg-[#fff] box-border fixed left-0 bottom-0 tab-bar z-1 items-center">
                    <view class="flex items-center">
                        <view class="flex flex-col justify-center items-center mr-[38rpx]" @click="redirect({ url: '/addon/phone_shop/pages/index', mode: 'reLaunch' })">
...
import { getExchangeGoodsDetail } from '@/addon/phone_shop/api/point';
import goodsSku from '@/addon/phone_shop/pages/point/components/goods-sku.vue';
...
onLoad((option: any) => {
    ...
    getExchangeGoodsDetail(option.id || '',).then((res: any) => {
        ...
        if (goodsDetail.value.goods.goods_video != '') {
            ...
            // 设置视频源
            currentVideoSrc.value = img(goodsDetail.value.goods.goods_video);
            currentVideoPoster.value = img(goodsDetail.value.goods.goods_cover_thumb_mid)
        }
        sendMessageTitle.value = goodsDetail.value.goods.goods_name
        sendMessagePath.value = '/addon/phone_shop/pages/point/detail?sku_id=' + goodsDetail.value.sku_id;
