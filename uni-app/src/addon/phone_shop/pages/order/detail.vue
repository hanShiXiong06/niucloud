<template>
    <view :style="themeColor()">
        <view class="bg-[var(--page-bg-color)] min-h-screen" v-if="!loading">
            <view v-if="!loading" class="pb-20rpx">
                <template v-if="isShowLocalDelivery">
                    <template v-if="systemStore.mapConfig.map_type == 'tencent'">
                        <view style="min-height: calc(50vh  + 150rpx);">
                            <!-- #ifdef MP-WEIXIN -->
                            <top-tabbar :data="topTabbarData" :scrollBool="topTabarObj.getScrollBool()" />
                            <!-- #endif -->
                            <map-show ref="mapShowRef" :tradeNo="detail.order_no" @confirm="handleComfirm" v-if="detail.order_no" />
                        </view>
                    </template>
                    <view v-else style="min-height: 150rpx;"></view>
                </template>
                <view v-if="detail.status_name  && !isShowLocalDelivery" class="pl-[40rpx] pr-[35rpx] bg-linear pb-[100rpx]">
                    <!-- #ifdef MP-WEIXIN || APP-PLUS -->
                    <top-tabbar :data="topTabbarData" :scrollBool="topTabarObj.getScrollBool()" />
                    <!-- #endif -->
                    <view class="flex justify-between items-center pt-[40rpx]">
                        <view class="flex flex-col">
                            <image class="w-[28rpx] h-[20rpx] mb-[10rpx] ml-[4rpx]" :src="img('addon/phone_shop/detail/head_001.png')" mode="aspectFit" />
                        ...
                        <view class="flex items-end relative -bottom-[6rpx]">
                            ...
                            <image class="w-[8rpx] h-[30rpx] mb-[20rpx]"
                                :src="img('addon/phone_shop/detail/head_002.png')" mode="aspectFit" />
                        </view>
                    </view>
                </view>
                <view class="sidebar-margin card-template mt-[-86rpx] relative" v-if="detail.delivery_type == 'local_delivery' && isShowLocalDelivery">
                    <view class="flex items-center" @click="localeDeliveryFn">
                        <view class="flex-1">
                            <view class="mb-[20rpx] flex items-baseline leading-[40rpx] min-h-[46rpx]">
                                <text class="text-[26rpx] font-500 mr-[10rpx]">预计</text>
                                <text class="text-[46rpx] font-600">{{ expectedFinishTime }}</text>
                            </view>
                            
                            <view class="flex">
                                <text class="text-[26rpx] font-500 mr-[10rpx] flex-shrink-0">送至</text>
                                <view class="flex flex-col">
                                    <text class="text-[26rpx] mb-[12rpx] leading-[26rpx]">{{ detail.taker_full_address }}</text>
                                    <view class="flex leading-[38rpx] overflow-hidden">
                                        <text class="text-[26rpx]">{{ detail.taker_name }}</text>
                                        <text class="text-[26rpx] ml-[15rpx]">{{ detail.taker_mobile }}</text>
                                    </view>
                                </view>
                            </view>
                        </view>
                        <view @click.stop="openMap" class="flex flex-col ml-3" v-if="detail.delivery_type == 'local_delivery' && ['3','5'].indexOf(detail.status) != -1 && systemStore.mapConfig.map_type == 'tianditu'">
                        ...
                        <text class="nc-iconfont nc-icon-youV6xx ml-[10rpx]" v-if="detail.delivery_type == 'local_delivery' && ['3','5'].indexOf(detail.status) != -1"></text>
                    </view>
                    
                </view>
                <view class="sidebar-margin mt-[-86rpx] card-template" v-if="detail.delivery_type != 'virtual'&& detail.taker_name && !isShowLocalDelivery">
                        ...
                        <view class="flex items-center mb-3">
                            <view>
                                <u--image class="overflow-hidden" radius="var(--goods-rounded-mid)" width="100rpx" height="100rpx" :src="img(detail.store.store_logo ? detail.store.store_logo : '')" model="aspectFill">
                                    <template #error>
                                        <image class="w-[100rpx] h-[100rpx] rounded-[var(--goods-rounded-mid)] overflow-hidden" :src="img('addon/phone_shop/store_default.png')" mode="aspectFill" />
                                    </template>
                                </u--image>
                            </view>
                    ...
                    <view v-for="(goodsItem, goodsIndex) in detail.goods" :key="goodsIndex"
                        ...
                        <view class="order-goods-item flex justify-between flex-wrap mb-[20rpx]">
                            <view class="w-[150rpx] h-[150rpx] rounded-[var(--goods-rounded-big)] overflow-hidden" @click="goodsEvent(goodsItem.goods_id)">
                                <up-image class="overflow-hidden" radius="var(--goods-rounded-big)" width="150rpx"
                            ...
                            <view class="ml-[20rpx] flex flex-1 flex-col justify-between">
                                ...
                                <view v-if="goodsItem.manjian_info &&goodsItem.manjian_info.length>0"
                                    ...
                                    <view class="text-[22rpx] text-[#999] truncate max-w-[320rpx]" >
                                        <text v-for="(cItem,cIndex) in goodsItem.manjian_info" :key="cIndex" class="whitespace-nowrap" :class="{'ml-[6rpx]':cIndex>0}">{{ cItem.manjian_name }}</text>
                                    </view>
                                </view>
                        ...
                        <view class="flex items-center box-border mt-[8rpx]"
                            ...
                            <image class="h-[24rpx] w-[56rpx]" :src="img('addon/phone_shop/impulse_buy.png')" mode="heightFix" />
                        ...
                        <view class="flex justify-end w-[100%] mt-[30rpx] mb-[20rpx]"
                            ...
                            <view v-if="goodsItem.status != '1'"
                                  ...
                                  @click="redirect({ url: '/addon/phone_shop/pages/refund/detail', param: { order_refund_no : goodsItem.order_refund_no } })">查看退款</view>
                    ...
                    <view class="pt-[20rpx] bg-[#f9f9f9] mt-[20rpx] mx-[var(--pad-sidebar-m)] rounded-[var(--rounded-big)]"
                        ...
                        <view class="order-goods-item flex justify-between flex-wrap px-[var(--pad-sidebar-m)] pb-[20rpx]"
                            ...
                            <view class="w-[120rpx] h-[120rpx] rounded-[var(--goods-rounded-big)] overflow-hidden" @click="goodsEvent(goodsItem.goods_id)">
                                <up-image class="overflow-hidden" radius="var(--goods-rounded-big)" width="120rpx" height="120rpx" :src="img(goodsItem.goods_image_thumb_small ? goodsItem.goods_image_thumb_small : '')" model="aspectFill">
                                    <template #error>
                ...
                <view class="flex z-2 justify-between items-center bg-[#fff] fixed left-0 right-0 bottom-0 min-h-[100rpx] pl-[30rpx] pr-[20rpx] flex-wrap  pb-ios">
                    ...
                    <view class="flex justify-end">
                        ...
                        <view class="order-grey-hollow-btn ml-[20rpx]" v-if="detail.status == -1" @click="orderBtnFn('delete')">{{ t('orderDelete') }}</view>
                    </view>
                </view>
            </view>
...
import { getShopOrderDetail, orderClose, orderFinish, orderDelete } from '@/addon/phone_shop/api/order';
...
import deliveryRecord from '@/addon/phone_shop/pages/order/components/delivery-record/delivery-record.vue'
...
const isShowLocalDelivery = ref(false)
...
const orderDetailFn = (id: any) => {
    ...
    getShopOrderDetail(id).then((res: any) => {
        ...
        isShowLocalDelivery.value = detail.value.show_local_delivery_info
        if (isShowLocalDelivery.value) {
            topTabbarData = topTabarObj.setTopTabbarParam({ title: '订单详情', topStatusBar: { textColor: '#333' } })

            if (systemStore.mapConfig.map_type == 'tianditu') {
                let site_id = detail.value.site_id
                let member_id = detail.value.member_id
                let trade_no = detail.value.order_no
                ...
                tianmapUrl.value = baseUrl + `/addon/phone_shop/tianmap/order_detail_track.html?site_id=${ site_id }&member_id=${ member_id }&trade_no=${ trade_no }&key=${ tiandituKey }`
            }
        }
        ...
        success: res => {
            if (res.confirm) {
                orderDelete(item.order_id).then((data) => {
                    goback({ url: '/addon/phone_shop/pages/order/list', title: '' })
                })
            }
        }
    })
...
const orderBtnFn = (type = '') => {
    if (type == 'pay')
        ...
        if (!detail.value.is_evaluate) {
            ...
            redirect({
                url: '/addon/phone_shop/pages/evaluate/order_evaluate_view',
...
// 是否展示地图
let expectedFinishTime = ref()
const handleComfirm = (val: any) => {
    if(val){
        isShowLocalDelivery.value = true
    } else {
        isShowLocalDelivery.value = false
    }
    expectedFinishTime.value = val?.expected_finish_time
...
const isShowVerify = computed(() => {
    let bool = false;
    if (detail.value.order_goods && detail.value.order_goods.length == 1) {
...
// 配送记录
const deliveryRecordRef = ref(null)
const localeDeliveryFn = () => {
    if(detail.value.delivery_type == 'local_delivery' && ['3','5'].indexOf(detail.value.status) != -1){
        deliveryRecordRef.value?.open(detail.value.local_delivery_order_log);
    }
}

const openMap = () => {
    if (!tianmapUrl.value) return
    if (systemStore.mapConfig.map_type == 'tianditu') {
        // 微信小程序不支持天地图，需要打开网页查看
        redirect({
            url: '/app/pages/webview/index',
            param: { src: encodeURIComponent(tianmapUrl.value) }
        });
    }
}
</script>
