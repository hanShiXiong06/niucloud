            ...
            <template v-for="(item, index)  in info" :key="index">
                <view class="card-template mb-[var(--top-m)]">
                    <view class="bg-[var(--temp-bg)] p-[20rpx] rounded-[var(--rounded-mid)] flex">
                        <up-image radius="var(--goods-rounded-mid)" width="150rpx" height="150rpx" :src="img(item.goods_image_thumb_small ? item.goods_image_thumb_small : '')" model="aspectFill">
                            <template #error>
...
import { getShopOrderDetail } from '@/addon/phone_shop/api/order';
import { setOrderEvaluate } from '@/addon/phone_shop/api/evaluate';
...
const toLink = (order_id: any) => {
    redirect({ url: '/addon/phone_shop/pages/evaluate/order_evaluate_view', param: { order_id }, mode: 'redirectTo' })
}
</script>
<style lang="scss">
.remove-border {
