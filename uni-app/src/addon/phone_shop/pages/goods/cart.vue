    ...
    <view :style="themeColor()">
        ...
        <view class="bg-page min-h-[100vh] overflow-hidden flex flex-col" v-if="!loading">
            ...
            <view v-else-if="!cartList.length&&!invalidList.length" class="pb-[100rpx]">
                <view class="empty-page">
                    <image class="img" :src="img('addon/phone_shop/cart-empty.png')" model="aspectFit" />
                    <view class="desc">赶紧去逛逛, 购买心仪的商品吧</view>
                    <button shape="circle" plain="true" class="btn" @click="redirect({ url: '/addon/phone_shop/pages/goods/list' })">去逛逛</button>
                </view>
                <ns-goods-recommend></ns-goods-recommend>
            </view>
            <template v-else>
...
import { getCartGoodsList, getCartCalculate } from '@/addon/phone_shop/api/cart'
...
import nsGoodsRecommend from '@/addon/phone_shop/components/ns-goods-recommend/ns-goods-recommend.vue';
...
const toLogin = () => {
    useLogin().setLoginBack({ url: '/addon/phone_shop/pages/goods/cart' })
}
