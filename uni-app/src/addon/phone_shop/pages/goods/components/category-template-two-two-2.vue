		...
		<view class="mescroll-box bg-[#F4F6F8]"
			...
			<mescroll-body ref="mescrollRef" :down="{ use: false }" @init="mescrollInit" @up="getListFn">
				<view v-if="config.search.control"
					...
					<input class="search-ipt pl-[32rpx] text-[24rpx] w-[430rpx]"
						:class="{ 'pr-[106rpx]': searchName, 'mr-[32rpx]': !searchName }" type="text"
						v-model="searchName" :placeholder="config.search.title" @confirm="searchNameFn">
					<view class="flex items-center h-[70rpx] absolute right-[16rpx] top-[18rpx]  z-2">
						...
						<view class="h-[70rpx] w-[40rpx] text-center leading-[70rpx]" @click.stop="searchNameFn"><text
								class="nc-iconfont nc-icon-sousuo-duanV6xx1 text-[32rpx]"></text>
						</view>
						<!-- 仓库筛选 Tab -->
						
						<!-- 胶囊式设计 -->
						<view class="flex ml-5 border-1 border-solid border-[var(--primary-color)] items-center ml-[6rpx] gap-[1rpx] bg-[#F5F5F5] rounded-[24rpx] px-[6rpx] py-[6rpx]">
							<view
								v-for="(item, index) in sourceOptions"
								:key="index"
								@click="handleSourceClick(item)"
								class="px-[10rpx] py-[10rpx] rounded-[20rpx] text-[24rpx] transition-all font-medium"
								:class="source === item.key ? 'bg-[#fff] text-color shadow-sm' : 'bg-transparent text-[#524d4d]'">
								{{ item.label }}
							</view>
						</view>
					</view>
...
import { img, redirect, getToken, getSiteId } from '@/utils/common';
...
const source = ref(''); // 来源供应商，空字符串表示"全部"
...
const sourceOptions = [
  { label: '本地仓', key: 1 },
  { label: '石家庄仓', key: '100005' }
];

// 处理仓库点击事件
const handleSourceClick = (item: any) => {
  // 如果点击的是已选中的项，则取消选中（恢复到"全部"）
  if (source.value === item.key) {
    source.value = '';
  } else {
    source.value = item.key;
  }
