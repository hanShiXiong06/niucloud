                ...
                <view class="mb-[var(--top-m)] card-template" v-for="(item,index) in list" :key="index">
                    <view @click="toLink(item)">
                        ...
                        <view class="flex mt-[20rpx]">
                            <up-image width="150rpx" height="150rpx" :radius="'var(--goods-rounded-big)'" :src="img(item.order_goods.goods_image_thumb_small ? item.order_goods.goods_image_thumb_small : '')" model="aspectFill">
                                <template #error>
...
import { getRefundList, closeRefund } from '@/addon/phone_shop/api/refund';
...
const toLink = (data) => {
    redirect({ url: '/addon/phone_shop/pages/refund/detail', param: { order_refund_no: data.order_refund_no } })
}
...
const refundBtnFn = (data, type) => {
    if (type == 'cancel') {
        ...
        redirect({ url: '/addon/phone_shop/pages/refund/edit_apply', param: { order_refund_no: data.order_refund_no } })
