<template>
    <view class="bg-[var(--page-bg-color)] min-h-[100vh] overflow-hidden" :style="themeColor()">
        <view class="coupon-header fixed  left-0 right-0 top-0 z-10080">
            ...
            <view :style="{height: headStyle, backgroundImage: 'url(' + img('addon/phone_shop/coupon/coupon_uniapp.png') + ')',backgroundSize: '100%', backgroundPosition: 'bottom', backgroundRepeat: 'no-repeat'}">
                <top-tabbar :data="param" class="top-header" />
            </view>
            <!-- #endif -->
            <!-- #ifdef H5 -->
            <view class="h-[364rpx]" :style="{ backgroundImage: 'url(' + img('addon/phone_shop/coupon/coupon_uniapp.png') + ')',backgroundSize: 'cover', backgroundPosition: 'bottom',backgroundRepeat: 'no-repeat'}"></view>
            <!-- #endif -->
        ...
        <mescroll-body ref="mescrollRef" @init="mescrollInit" :down="{ use: false }" height="auto" @up="getShopCouponListFn" :top="mescrollTop">
            <view v-if="list.length" class="pb-[var(--top-m)] sidebar-margin">
                <template v-for="(item, index) in list">
                    <view v-if="item.btnType === 'collected'"
                          ...
                          :style="{ backgroundImage: 'url(' + img('addon/phone_shop/coupon/coupn_loot.png') + ')'}"
            ...
            <!-- <view> -->
            <mescroll-empty v-if="!list.length && !loading" :option="{tip : '暂无优惠券',btnText:'去逛逛'}" @emptyclick="redirect({ url: '/addon/phone_shop/pages/goods/list' })"></mescroll-empty>
            <!-- </view> -->
        </mescroll-body>
...
import { getShopCouponList, getCoupon, getMyCouponType } from '@/addon/phone_shop/api/coupon'
...
const toDetail = (coupon_id: any) => {
    redirect({ url: '/addon/phone_shop/pages/coupon/detail', param: { coupon_id } })
}
