<template>
    <view :style="themeColor()">
        <view v-if="Object.keys(detail).length&&!loading" class="overflow-hidden min-h-screen bg-style relative" :style="{ 'background': 'url(' + img('addon/phone_shop/coupon/coupon_bg.png') + ') no-repeat' }">
            <!-- #ifdef MP || APP-PLUS -->
            <top-tabbar :data="topTabbarData" :scrollBool="topTabarObj.getScrollBool()" />
            <!-- #endif -->
            <view class="relative mt-[236rpx] w-[100%] h-[932rpx]" :style="{ 'background': 'url(' + img('addon/phone_shop/coupon/coupon_bg_02.png') + ') center / contain no-repeat' }">
                ...
                <view class="flex justify-center items-center mt-[20rpx]">
                    <text v-if="detail.btnType === 'collected'"
                          ...
                          :style="{ 'background': 'url(' + img('addon/phone_shop/coupon/coupon_btn_02.png') + ')  center / contain no-repeat' }">已领完</text>
...
const collecting = (coupon_id: any) => {
    if (!userInfo.value) {
        useLogin().setLoginBack({ url: '/addon/phone_shop/pages/coupon/detail', param: { coupon_id: detail.value.id } })
        return false
    }
