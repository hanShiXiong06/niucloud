	...
	<view class="bg-[#fff] min-h-screen overflow-hidden component-class" :style="themeColor()" v-if="!pageLoading">
		...
		<view
			...
			<view>
				...
				<view class="flex items-baseline">
					<text
						class="text-[50rpx] font-bold">{{ storeInfo.commission ? Number(storeInfo.commission).toFixed(2) : '0.00' }}</text>
					<text class="text-[32rpx] ml-[10rpx]">{{ t('yuan') }}</text>
				</view>
			</view>
			<view class="bg-[#fff] text-[var(--store-bg-one)] rounded-lg px-[30rpx]  text-[24rpx] leading-[1.8]" @click="handleCashOut">
				{{ t('withdraw') }}
			</view>
		</view>
		...
		<view class="today-bill bg-white mt-[30rpx] rounded-[24rpx] overflow-hidden">
			...
			<view class="bill-list-wrapper">
				<mescroll-body ref="billMescrollRef" :down="{ use: false }" @up="fetchBillList" @init="mescrollInit"  :height="600">
					...
					<mescroll-empty v-else-if="!loading" :option="{tip: t('noBillData')}"
...
<script setup lang="ts">
	...
	const navigateToIncomeExpenseSummary = () => {
		redirect({
			url: '/addon/home_service/store/pages/store/account/account_statement?type=2'
		})
	}
	...
	const fetchStoreInfo = () => {
		loading.value = true;
		return getStoreInfo({}).then((res : any) => {
			loading.value = false;
			pageLoading.value = false
			if (res.code === 1 && res.data) {
				// 确保commission是数字类型
				...
				pageLoading.value = false
				loading.value = false;
				console.error(t('fetchStoreDataFailed')); // 使用更合适的错误消息键名
			})
		}
</script>
