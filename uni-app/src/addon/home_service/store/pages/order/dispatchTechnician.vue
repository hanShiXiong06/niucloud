<template>
	...
	<!-- #endif -->
	<view class="fixed w-full bg-[#f6f6f6] z-index-99" :style={top:topStyle}>
		<u-tabs :list="categoryList" :current="currentValue" keyName="category_name" @change="changeTabs"
			lineColor="#FBD700"></u-tabs>
	</view>
	<u-picker :show="statusShow" :columns="statusList" keyName="label" @confirm="statusConfrim"
		@cancel="statusShow = false"></u-picker>
	<view class="bg-[var(--page-bg-color)] overflow-hidden component-class" :style="themeColor()">
		<mescroll-body ref="mescrollRef" :down="{ use: false }" top="88" @init="mescrollInit" @up="getTechnicianListFn">
			<view class="mt-2 mx-[25rpx] " v-if="technicianList.length">
				<!-- 技师项 -->
				<view class="  mb-[25rpx] " v-for="(technician, index) in technicianList" :key="index"
					@click.stop="goDetail(technician.id)">
					<view class="flex w-[100%]">
						<view class="flex items-center mr-[25rpx] w-[28rpx]" @click.stop="changeChoose(index)">
							<u-icon name="checkmark-circle-fill" v-if="technician.chooseStatus"
								color="#FBD700"></u-icon>
								...
								<u--image :src="img(technician.headimg)" width="160rpx" height="160rpx" radius="100rpx"
									...
									<template #error>
										<image :src="img('static/resource/images/default_headimg.png')"
											class="w-20 h-20 rounded-full  border-2 border-white" mode="aspectFill">
										</image>
									</template>
								</u--image>
								<text
	...
	<view
		...
		<button hover-class="none"
			...
			@click="distpachConfirm" :class="{'opacity-50': btnDisabled}">取消
		</button>
		<button hover-class="none"
			class="!bg-[var(--store-bg-one)] !text-[#fff] w-full !bg-[#00CB7A] h-[80rpx] leading-[80rpx] rounded-[10rpx] text-[26rpx] font-500"
			@click.stop="handleOrderAction({}, 'dispatch')" :class="{'opacity-50': btnDisabled}">确定
		</button>
	</view>
...
<script setup lang="ts">
	...
	let topTabbarData = topTabarObj.setTopTabbarParam({ title: '师傅派单', topStatusBar: { textColor: '#333' } })
	// 获取系统状态栏的高度
	...
	const topStyle = computed(() => {
		let style = pxToRpx(Number(menuButtonInfo.height) + menuButtonInfo.top) + 'rpx;'
		return style
	})
	...
	const handleOrderAction = (order : any, key : string) => {
	    ...
	    if (!technicianList.value.length) {
	        uni.showToast({ title: '暂无可派单师傅', icon: 'none' })
	        return
	    }
	
	    // 校验：是否选择了师傅
	    let selected : any = null
	    technicianList.value.forEach((item : any) => {
	        if (item.chooseStatus) selected = item
	    })
	    if (!selected) {
	        uni.showToast({ title: '请选择师傅', icon: 'none' })
	        return
	    }
