<template>
  <!-- 仅在小程序端显示导航栏 -->
  <!-- #ifdef MP-WEIXIN -->  
  <u-navbar title="公告信息" bgColor="#f6f6f6" leftIconSize="15px" @leftClick="leftClick">
  </u-navbar>
 <!-- #endif -->
  
  <!-- 固定在顶部的状态条 -->
  <view v-if="statusList.length" class="sticky-header" :style="{ zIndex: 9,paddingTop:navHeight + 'px' }">
    <view class="flex w-full p-[30rpx] py-[40rpx] pt-[25rpx] box-border bg-[#f6f6f6] text-[28rpx] leading-[1.8]">
      <view v-for="(item,index) in statusList" :key="index"
        ...
        <view class="mr-[10rpx] flex flex-col items-center relative">
          <view class="text-[26rpx]" :class="index == statusIndex ? 'text-[var(--technician-bg-one)] font-bold' : ''">
            {{item.value}}
          </view>
  ...
  <view class="bg-[#f6f6f6]" v-if="statusList.length">
    <view class="mescroll-box" :style="{ marginTop: 0 }">
      <mescroll-body ref="mescrollRef" :down="{ use: false }" @init="mescrollInit" 
        :top="getMescrollTop()" @up="getListFn">
            ...
            <view class="flex-1 flex flex-col justify-between pt-[10rpx]">
              ...
              <view class="flex justify-between item-center">
                <view class="text-[26rpx] text-[#666666] w-[450rpx] text-ell leading-4">
                  ...
                  <text
                  	class="text-[#EF000C] text-[28rpx] font-bold "
