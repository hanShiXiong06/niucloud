...
<script setup lang="ts">
	...
	import { submitTechnicianFeedback } from '@/addon/home_service/technician/api/member'
	...
	const submitForm = () => {
		...
		if (form.value.content.length < 10) {
			uni.showToast({ title: t('feedbackContentMinLength'), icon: 'none' })
			return
		}
		...
		submitTechnicianFeedback(submitData).then((res : any) => {
