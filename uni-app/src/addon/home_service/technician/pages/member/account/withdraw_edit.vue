<template>
    <view class="w-screen h-screen bg-[var(--page-bg-color)] overflow-hidden" :style="themeColor()">
                        ...
                        <u-form labelPosition="left" :model="formData" labelWidth="200rpx" errorType='toast' :rules="rules" ref="formRef">
                            ...
                            <view class="mt-[16rpx]">
                                <u-form-item label="收款码" prop="transfer_payment_code">
                                    ...
                                    <u-upload v-else @afterRead="collectionCodeAfterRead" @delete="collectionCodeDeleteFn" :maxCount="1" :maxSize="10485760" @oversize="oversizeTit"></u-upload>
                                </u-form-item>
                            </view>
                        </u-form>
                    </view>
                        ...
                        <u-form labelPosition="left" :model="formData" labelWidth="200rpx" errorType='toast' :rules="rules" ref="formRef">
                            <view>
                                <u-form-item :label="t('wechatCodeRealname')" prop="realname">
                                    <u-input v-model.trim="formData.realname" maxlength="30" border="none" fontSize="28rpx" clearable :placeholder="t('wechatCodeRealnamePlaceholder')" />
                                </u-form-item>
                            </view>
                            <view class="mt-[16rpx]">
                            ...
                            <view class="mt-[16rpx]">
                                <u-form-item label="收款码" prop="transfer_payment_code">
                                    <view class="relative w-[160rpx] h-[160rpx]" v-if="formData.transfer_payment_code">
                                        <image class="w-[160rpx] h-[160rpx] rounded-[8rpx]" :src="img(formData.transfer_payment_code)" mode="aspectFill" @click="previewImageFn(img(formData.transfer_payment_code))" />
                                        <view class="absolute top-0 right-0 bg-[#373737] flex justify-end h-[28rpx] w-[28rpx] rounded-bl-[40rpx]" @click="collectionCodeDeleteFn">
                                            <text class="nc-iconfont nc-icon-guanbiV6xx !text-[20rpx] mt-[2rpx] mr-[2rpx] text-[#fff]"></text>
                                        </view>
                                    </view>
        ...
        <u-modal :show="deleteConfirm" :content="t('deleteConfirm')" :confirmText="t('confirm')"
                 ...
                 @cancel="deleteConfirm = false" confirmColor="var(--technician-bg-one)"></u-modal>
    </view>
</template>
...
import { editCashoutAccount, addCashoutAccount,getCashoutAccountInfo,deleteCashoutAccount } from '@/addon/home_service/technician/api/account'
...
const handleDelete = () => {
    deleteCashoutAccount(formData.account_id).then(() => {
        redirect({ url: '/addon/home_service/home_service/pages/member/account/withdraw_list', mode: 'redirectTo' })
    })
}
...
.save-button {
    ...
    width: 100%;
    background: var(--technician-bg-one, #3E82FC);
    color: #FFFFFF;
    height: 80rpx;
    line-height: 80rpx;
    border-radius: 40rpx;
