		...
		<view class="content">
			...
			<template v-if="accountList.length > 0">
				<view class="account-list">
					<u-swipe-action-item v-for="(item, index) in accountList" :key="index" :options="accountOptions"
						@click="swipeClick(index)" class="mb-[24rpx]">
							...
							<view class="account-info">
								...
								<view class="account-detail">
									<text v-if="item.account_type == 'bank'">
										{{ t('endNumber') }}
										{{ item.account_no.substring(item.account_no.length - 4) }}{{ t('bankCard') }}
									</text>
									<text v-else>{{ formatAccountNumber(item.account_no) }}</text>
								</view>
							</view>
							<view class="account-actions">
								<text class="edit-icon nc-iconfont nc-icon-xiugaiV6xx"
									@click.stop="editAccount(item)"></text>
							</view>
						</view>
					</u-swipe-action-item>
				</view>
			...
			<view v-else class="empty-state">
				<image class="empty-icon" :src="img('static/resource/images/member/empty_account.png')"
					mode="widthFix" />
...
<script setup lang="ts">
	...
	import { getCashOutAccountList, deleteCashoutAccount } from '@/addon/home_service/technician/api/account'
	...
	const editAccount = (item : any) => {
		redirect({
			url: '/addon/home_service/technician/pages/member/account/withdraw_edit',
