	...
	<view class="min-h-[100vh] bg-[var(--page-bg-color)] overflow-hidden" :style="themeColor()">
		...
		<template v-if="!loading">
			<view class="sidebar-margin card-template mt-[20rpx] !pt-[60rpx] !pb-[40rpx]">
				...
				<view class="flex items-center flex-col mb-[80rpx]">
					...
					<text class="text-[28rpx] text-[#333]"
						:class="{'text-primary': cashOutInfo.status == 1, 'text-[#999]': cashOutInfo.status == 4 }">{{ cashOutInfo.status_name }}</text>
				</view>
				...
				<view>
					...
					<view class="flex justify-between text-[28rpx] mt-[34rpx] leading-[32rpx]"
						v-if="cashOutInfo.status && cashOutInfo.audit_time">
						<text class="text-[#333] w-[200rpx]">{{ t('auditTime') }}</text>
						<text class="text-[#333]">{{ cashOutInfo.audit_time }}</text>
					</view>
					<view class="flex justify-between text-[28rpx] mt-[34rpx] leading-[32rpx]"
						v-if="cashOutInfo.status == -1 && cashOutInfo.refuse_reason">
			...
			<view class="sidebar-margin card-template mt-[20rpx]"
				v-if="cashOutInfo.transfer_type == 'wechatpay' && (cashOutInfo.status == 2 || cashOutInfo.status == 3 || cashOutInfo.status == 4)">
				<u-steps :current="current" direction="column" activeColor="var(--primary-color)"
					activeIcon="checkmark-circle-fill" inactiveIcon="checkmark-circle-fill">
					<u-steps-item title="提现申请" :desc="cashOutInfo.create_time"></u-steps-item>
					<u-steps-item title="处理中"
						:desc="cashOutInfo.audit_time ? cashOutInfo.audit_time : ''"></u-steps-item>
					<u-steps-item title="提现成功"
						:desc="cashOutInfo.transfer_time ? cashOutInfo.transfer_time : ''"></u-steps-item>
				</u-steps>
			</view>
			<view class="sidebar-margin card-template mt-[20rpx]">
				...
				<view class="flex justify-between text-[28rpx] mt-[34rpx] leading-[32rpx]">
					...
					<text
						class="text-[#333] truncate">{{ cashOutInfo.transfer_type == 'wechatpay' ? '微信零钱' : cashOutInfo.transfer_account }}</text>
				</view>
				<template v-if="cashOutInfo.transfer_type == 'wechatpay'">
					...
					<view class="flex justify-between text-[28rpx] mt-[34rpx] leading-[32rpx]">
						...
						<u-avatar :src="img(cashOutInfo.headimg)" size="30" leftIcon="none"
				...
				<template v-if="cashOutInfo.transfer_type !== 'wechatpay'">
					<view class="flex justify-between text-[28rpx] mt-[34rpx] leading-[32rpx]">
						<text
							class="text-[#333] w-[200rpx]">{{ cashOutInfo.transfer_type == 'bank' ? t('bankRealname') : t('realname') }}</text>
						<text class="text-[#333]">{{ cashOutInfo.transfer_realname }}</text>
					</view>
					<view class="flex justify-between text-[28rpx] mt-[34rpx] leading-[32rpx]"
						v-if="cashOutInfo.transfer_bank">
						<text class="text-[#333] w-[200rpx]">{{ t('transferBank') }}</text>
						<text class="text-[#333] truncate">{{ cashOutInfo.transfer_bank }}</text>
					</view>
					<view class="flex justify-between text-[28rpx] mt-[34rpx] leading-[32rpx]"
						v-if="cashOutInfo.transfer_type == 'wechat_code' || cashOutInfo.transfer_type == 'alipay'">
						<text class="text-[#333] w-[200rpx]">{{ t('transferCode') }}</text>
						<view class="flex items-center" @click="previewImg()">
							<image :src="img(cashOutInfo.transfer_payment_code)"
								class="w-[140rpx] h-[140rpx] rounded-[var(--rounded-small)]" />
						</view>
					</view>
				</template>
			</view>
			<view class="sidebar-margin card-template my-[20rpx]" v-if="cashOutInfo.status == 2">
				...
				<view class="flex justify-between text-[28rpx] mt-[34rpx] leading-[32rpx]"
					v-if="cashOutInfo.transfer && cashOutInfo.transfer.transfer_voucher">
					<text class="text-[#333] w-[200rpx]">{{ t('transferVoucher') }}</text>
					<view class="flex items-center" @click="handleImg()">
						<image :src="img(cashOutInfo.transfer.transfer_voucher)"
							class="w-[140rpx] h-[140rpx] rounded-[var(--rounded-small)]" />
					</view>
				</view>
			</view>
			<view class="tab-bar-placeholder"
				v-if="cashOutInfo.status == 1 || cashOutInfo.status == 2 || cashOutInfo.status == 4"></view>
			<view
				class="fixed bottom-[0] tab-bar left-0 right-0 px-[var(--sidebar-m)] bg-[#fff] flex items-center justify-end min-h-[100rpx]"
				v-if="cashOutInfo.status == 1 ">
				<button
					class="min-w-[180rpx] box-border text-[26rpx] h-[70rpx] flex-center border-[2rpx] border-solid border-[#999] rounded-full !text-[var(--text-color-light6)] mr-0"
					@click="cashOutCancel" v-if="cashOutInfo.status == 1 ">{{ t('cashOutCancel') }}</button>
				<button
					class="min-w-[180rpx] h-[70rpx] !text-[#fff] leading-[70rpx] primary-btn-bg rounded-[50rpx] text-[26rpx] ml-[20rpx] mr-0"
					:loading="operateLoading" @click="cashOut"
					v-if="cashOutInfo.transfer_type == 'wechatpay' && cashOutInfo.status == 2">{{ t('cashOutNow') }}</button>
				<button
					class="min-w-[180rpx] h-[70rpx] !text-[#fff] leading-[70rpx] primary-btn-bg rounded-[50rpx] text-[26rpx] ml-[20rpx] mr-0"
...
<script lang="ts" setup>
	...
	import { getCashOutDetail, getCashOutTransfer, memberCancel } from '@/addon/home_service/technician/api/member';
	import wechat from '@/utils/wechat'
	import useConfigStore from "@/stores/config";
	import { topTabar } from '@/utils/topTabbar';
	// 系统状态管理
	const topTabarObj = topTabar()
	let topTabbarData = topTabarObj.setTopTabbarParam({ title: '提现详情', topStatusBar: { textColor: '#333' } })
	// #ifdef H5
