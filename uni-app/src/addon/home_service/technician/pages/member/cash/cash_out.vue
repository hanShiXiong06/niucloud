<template>
	...
	<view class="min-h-[100vh] bg-[var(--page-bg-color)] overflow-hidden" :style="themeColor()">
		<mescroll-body ref="mescrollRef" @init="mescrollInit" :down="{ use: false }" @up="getCashOutListFn">
			<view v-for="(item,index) in cashOutList" :key="item.id"
				class="sidebar-margin card-template mt-[var(--top-m)]" @click="toDetailFn(item)">
				<view class="flex items-center justify-between  mb-[20rpx]">
					<view class="text-[36rpx] font-500 price-font" :class="item.apply_money > 0 ? 'text-active' : ''">
						{{ item.apply_money }}</view>
					<view class="leading-[38rpx] text-[26rpx] ">{{ item.status_name }}</view>
				</view>
				<view class="text-[24rpx] text-[var(--text-color-light6)] mb-[10rpx] leading-[34rpx]">
					{{ t('rechargeType') }} {{ item.transfer_type_name }}</view>
				<view class="text-[24rpx] text-[var(--text-color-light6)] mb-[10rpx] leading-[34rpx]">
					{{ t('applyTime') }}: {{ item.create_time }}</view>
				<view class="text-[24rpx] text-[var(--text-color-light9)]  leading-[34rpx]">
					{{ item.status != -1 ? currentStatusDesc(item.status) : item.refuse_reason }}</view>
			</view>
...
<script setup lang="ts">
	...
	import { getCashOutList } from '@/addon/home_service/technician/api/member';
	...
	let topTabbarData = topTabarObj.setTopTabbarParam({ title: '提现列表', topStatusBar: { textColor: '#333' } })
	...
	const toDetailFn = (data : any) => {
		redirect({ url: '/addon/home_service/technician/pages/member/cash/cash_out_detail', param: { id: data.id } });
	}
</script>
