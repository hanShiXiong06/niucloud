  ...
  <view class="bg-[var(--page-bg-color)] min-h-screen overflow-hidden component-class" v-if="!loading">
        ...
        <text v-for="(day, index) in weekDays" :key="index" class="text-center text-gray-500 text-sm">
          {{ t(`${['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'][index]}`) }}
			...
			<view class="grid grid-cols-7 gap-2 ">
              ...
              '!bg-[var(--technician-bg-one)] !text-[#ffffff]': selectedDate === `${currentYear}-${currentMonth.toString().padStart(2, '0')}-${day.toString().padStart(2, '0')}`,
	...
	<view class="w-full footer bg-[#fff]">
		<view
			...
			<button class="flex-1 py-3 bg-blue-500 text-white rounded-lg bg-[var(--technician-bg-one)] !text-[26rpx]"
				@click="submitForm">
				{{ t('confirm') }}
			</button>
		</view>
...
<script setup lang="ts">
	...
	const cancelRestRecord = () : void => {
		...
		uni.showModal({
			...
			success: (resModal) => {
				if (resModal.confirm) {
					// 调用取消请假API
					cancelTechnicianRestRecord({ date: selectedDate.value }).then((res) => {
						if (res.code === 1) {
							// 重新加载休息记录
							loadRestRecords()
	...
	const loadRestRecords = () : void => {
		...
		getTechnicianRestRecords({ date: dateParam }).then((res) => {
			loading.value = false
			if (res.code === 1 && res.data) {
