        ...
        <view :style="warpCss" class="overflow-hidden">
            ...
            <view :class="{'diy-shop-goods-list relative flex flex-wrap justify-between': diyComponent.style != 'style-2', 'biserial-goods-list': diyComponent.style == 'style-2'}">
                <template v-if="diyComponent.style == 'style-2'">
                    <view>
						<view class="mb-[26rpx]">
							<up-image radius="10" :width="style2Width" :height="diyStore.mode != 'decorate' ? '248px' : '238px'" :src="img(diyComponent.bgUrl)" model="aspectFill">
							    <template #error>
                        ...
                        <template v-for="(item,index) in goodsList" v-if="diyStore.mode != 'decorate'">
                            <view v-if="(index%2) == 0" class="flex flex-col bg-[#fff] box-border rounded-lg overflow-hidden"
...
<!--                                <easy-image :image-src="item.goods_cover_thumb_small" :imageStyle="imageStyle2" />-->
                    ...
                    <view >
                        <template v-for="(item,index) in goodsList">
                            <view v-if="(index%2) == 1" class="flex flex-col bg-[#fff] rounded-lg box-border overflow-hidden" :class="{'mt-[24rpx]': index > 1}" :style="itemCss" @click="toLink(item)">
