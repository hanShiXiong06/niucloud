    ...
    <view :style="warpCss" class="goods-carousel-search-wrap">
        <view class="relative">
            ...
            <view class="fixed-wrap" :style="fixedStyle">
                <view class="diy-search-wrap style-2 relative z-10" :style="!props.global.topStatusBar.isShow ? navbarInnerStyle : ''" @click="diyStore.toRedirect(diyComponent.search.link)">
                    <view class="flex items-center w-full">
                        ...
                        <!-- #ifdef MP-WEIXIN -->
                        <view class="search-content" :style="{backgroundColor: diyComponent.search.bgColor,height:systemStore.menuButtonInfo?.height + 'px' }" @click.stop="diyStore.toRedirect(diyComponent.search.link)">
                        <!-- #endif -->
						<!-- #ifndef MP-WEIXIN -->
            ...
            <view class="relative" :class="{'mx-[20rpx]': swiperStyleBool && diyComponent.swiper.swiperStyle != 'style-3', 'swiper-style-3': diyComponent.swiper.swiperStyle == 'style-3'}" :style="carouselSwiperStyle()">
                <swiper v-if="diyComponent.swiper.control" class="swiper" :style="{ height: imgHeight,marginTop:'-54px'}" autoplay="true"
        ...
        <template v-if="currentSource == 'diy_page'">
            ...
            <view class="child-diy-template-wrap bg-index">
                ...
                <diy-group :data="diyPageData" ref="diyGroupRef" ></diy-group>
...
import { getDiyInfo } from '@/addon/home_service/api/diy';
...
const fixedStyle = computed(() => {
    let style = '';
        style += 'position: fixed;z-index: 99;left: 0;right: 0;';
		if(props.scrollBool == 1){
			style += 'background-color:' + '#15eb15' + ';';
		}
    // #ifdef H5
    ...
    if (diyComponent.value.swiper.swiperStyle == 'style-3') {
        // h5,上移的像素，采取的是平均值
		 const isIos = systemStore.systemInfo && systemStore.systemInfo.platform === 'ios';
		style += isIos ? 'margin-top: -55px;' : 'margin-top: -44.5px;';
    }
    // #endif
...
const carouselSwiperStyle = () => {
    ...
    if (diyComponent.value.swiper.swiperStyle == 'style-3') {
        ...
        // h5,上移的像素，采取的是平均值
        const isIos = systemStore.systemInfo && systemStore.systemInfo.platform === 'ios';
           style += isIos ? 'top: 55px;' : 'top: 44.5px;';
        // #endif
    }
    return style;
}
...
// #ifdef APP-PLUS
const statusBarHeight = systemStore.systemInfo?.statusBarHeight || 0;
navbarInnerStyle.value += 'padding-top:' + statusBarHeight + 'px;';
// #endif
...
onMounted(() => {
    ...
    if (diyStore.mode == 'decorate') {
        watch(
            () => diyComponent.value,
            (newValue, oldValue) => {
                if (newValue && newValue.componentName == 'HomeServiceCarouselSearch') {
...
if (componentsScrollVal && (typeof componentsScrollVal == "object")) {
    componentsScrollVal.HomeServiceCarouselSearch = 20
    uni.setStorageSync('componentsScrollValGroup', componentsScrollVal);
} else {
    let obj = {
        HomeServiceCarouselSearch: 20
    }
