    ...
    <view :style="themeColor()">
        <scroll-view scroll-y="true" class="w-screen h-screen bg-page" v-if="!loading">
            <view class="bg-[#fff] min-h-screen box-border overflow-hidden" v-if="Object.keys(technicianStat).length && Object.keys(technicianInfo).length">
                <view class="h-[552rpx] px-[30rpx] pt-[114rpx] box-border" :style="{ 'background-image': 'url(' + img('addon/home_service/technician/bg.png') + ')', 'background-size': '100%' }">
                    <view class="flex items-center">
                        <up-image
                            ...
                            <template #error>
                                <image class="w-[120rpx] h-[120rpx] border-[#fff] border-[4rpx] border-solid rounded-full" :src="img('addon/home_service/technician/user.png')" />
                            </template>
                        </up-image>
                            ...
                            <view class="flex items-center mt-[14rpx] flex-wrap">
                                <template v-for="(item, index) in technicianInfo.label" :key="index">
                                    <view class="flex items-center bg-[#fff] py-[4rpx] pl-[7rpx] pr-[17rpx] rounded-full border-[1rpx] border-solid border-[#E18547] mr-[10px]">
                                        <image style="width: 16rpx; height: 16rpx" mode="aspectFill" :src="img('addon/home_service/technician/label.png')" />
                    ...
                    <view class="flex items-center mt-[20rpx] bg-[#ffffff] rounded-[16rpx] opacity-90 box-border">
                        <view class="flex items-center justify-between py-[20rpx] pl-[30rpx] pr-[24rpx] flex-1">
                            ...
                            <image style="width: 84rpx; height: 84rpx" mode="aspectFill" :src="img('addon/home_service/technician/ordernum.png')" />
                        </view>
                        ...
                        <view class="flex items-center justify-between py-[20rpx] pl-[30rpx] pr-[24rpx] flex-1">
                            ...
                            <image style="width: 84rpx; height: 84rpx" mode="aspectFill" :src="img('addon/home_service/technician/ordermoney.png')"/>
                        </view>
                    </view>
                </view>
                <view class="p-[20rpx] bg-[#fff] mx-[30rpx] mt-[-150rpx] rounded-[10rpx]">
                    <view class="flex items-center justify-between mb-[30rpx]">
                        ...
                        <view class="flex items-center" @click="redirect({ url: '/addon/home_service/pages/master/task/list' })">
                    ...
                    <view class="flex flex-wrap justify-between">
                        <view class="p-[20rpx] w-[315rpx] box-border mb-[20rpx]"
                            :style="{ 'background-image': 'url(' + img('addon/home_service/technician/waitservice.png') + ')', 'background-size': '100%' }"
                            @click="redirect({ url: '/addon/home_service/pages/master/task/list', param: { order_status: 'wait_service' } })">
                        ...
                        <view class="p-[20rpx] w-[315rpx] box-border mb-[20rpx]"
                            :style="{ 'background-image': 'url(' + img('addon/home_service/technician/inservice.png') + ')', 'background-size': '100%' }"
                            @click="redirect({ url: '/addon/home_service/pages/master/task/list', param: { order_status: 'in_service' } })">
                        ...
                        <view class="p-[20rpx] w-[315rpx] box-border"
                            :style="{ 'background-image': 'url(' + img('addon/home_service/technician/finish.png') + ')', 'background-size': '100%' }"
                            @click="redirect({ url: '/addon/home_service/pages/master/task/list', param: { order_status: 'finish' } })">
                        ...
                        <view class="p-[20rpx] w-[315rpx] box-border"
                            :style="{ 'background-image': 'url(' + img('addon/home_service/technician/serviceitem.png') + ')', 'background-size': '100%' }"
                            @click="redirect({ url: '/addon/home_service/pages/technician/detail', param: { id: technicianInfo.id } })">
...
import { getStat } from '@/addon/home_service/user/api/o2o'
import { checkTechnician, getTechnicianDetail } from '@/addon/home_service/user/api/technician'
...
const checkTechnicianFn = () => {
    ...
    checkTechnician().then((res) => {
        if (JSON.stringify(res.data) == '[]') {
            redirect({ url: '/addon/home_service/pages/index', mode: 'reLaunch' })
        } else {
...
const toLink = (goods_id: any) => {
    redirect({ url: '/addon/home_service/pages/goods/detail', param: { goods_id } })
}
</script>
