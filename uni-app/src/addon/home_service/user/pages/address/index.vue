<template>
    <view class="address bg-[var(--page-bg-color)] min-h-[100vh]" v-if="!loading" :style="themeColor()">
		...
		<u-navbar title="地址管理" autoBack :fixed="true" placeholder>
		</u-navbar>
		<!-- #endif -->
        <scroll-view scroll-y="true">
            ...
            <view class="w-full footer">
                <view class="py-[var(--top-m)] px-[var(--sidebar-m)] footer w-full fixed bottom-0 left-0 right-0 box-border">
                    <button hover-class="none" class="!bg-[var(--primary-color)] !text-[#fff] !rounded-lg  text-[#fff] h-[80rpx] leading-[80rpx] rounded-[100rpx] text-[26rpx] font-500" @click="addAddress">{{ t('addHomeAddress') }}</button>
                </view>
            </view>
        </scroll-view>
    </view>
...
const addAddress = () => {
    redirect({ url: '/addon/home_service/user/pages/address/address_edit' })
}
