<template>
	<view class="bg-[#f5f5f5] min-h-screen" v-if="!loading" :style="themeColor()">
		...
		<!-- 门店选择器 -->
		<u-picker :show="showStorePicker" :columns="storeColumns" keyName="display_name" @confirm="onStoreConfirm"
			@cancel="onStoreCancel"></u-picker>
	</view>
...
<script setup lang="ts">
	...
	const submitForm = () => {
		...
		const submitData = {
			...
			certificate: form.value.certificate?.toString(), // 将数组转换为逗号分隔的字符串
