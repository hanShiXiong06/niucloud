		...
		<view class="fixed left-0 top-0 right-0 z-10">
			<scroll-view :scroll-x="true" class="scroll-Y box-border px-[var(--sidebar-m)] bg-white">
				<view class="flex whitespace-nowrap justify-around items-center h-[88rpx]">
					<view
						:class="['text-[28rpx] text-[#333] h-[88rpx] leading-[88rpx] font-400', { 'class-select !text-[#FF000D]': couponStatus === item.status }]"
						@click="statusClickFn(item.status)" v-for="(item, index) in statusList">
						{{ item.status_name }}({{ item.count }})</view>
				</view>
			</scroll-view>
			<scroll-view :scroll-x="true" scroll-with-animation
				:scroll-into-view="'id' + (subActive ? subActive - 1 : 0)"
				class="px-[var(--sidebar-m)]  box-border bg-white">
				<view class="items-center flex h-[88rpx]">
					<text
						class="flex-shrink-0 w-[126rpx] h-[54rpx] text-[24rpx] flex-center text-center text-[#333] bg-[var(--temp-bg)] rounded-[30rpx] box-border mr-[20rpx] border-[2rpx] border-solid border-[#F8F9FD]"
						:class="{'!text-[#FF000D] !border-[#FF000D] font-500 !bg-[#FFEFF0]':item.value == curType}"
				...
				<template v-for="(item, index) in list">
					...
					<view v-if="couponStatus != 1"
						...
						<view
							...
							:class="{'bg-[red]':couponStatus == 2 , 'bg-[#FFEFF0]' :couponStatus == 3 || couponStatus == 4}">
							<view class="price-font flex items-baseline"
								:class="{'text-[#fff]' : couponStatus == 2 , 'text-[#FFB4B1]' : couponStatus == 3 || couponStatus == 4}">
								<text
									class="text-[30rpx] leading-[34rpx] mr-[2rpx] text-center price-font font-500">￥</text>
								<text
									class="text-[54rpx] font-500 leading-[58rpx] price-font truncate">{{ item.coupon_price }}</text>
							</view>
							<text class="truncate max-w-[176rpx] mt-[6rpx] text-[24rpx] h-[32rpx] leading-[32rpx]"
								:class="{'text-[#fff]': couponStatus == 2 , 'text-[var(--primary-help-color4)]': couponStatus == 3 ||couponStatus == 4}">{{ item.title }}</text>
						</view>
						<view class="ml-[30rpx] flex-1 h-[100%] box-border py-[20rpx]">
							...
							<view class="mt-[10rpx] flex items-center">
								<text
									class="w-[80rpx] text-center bg-[#FFEFF0] whitespace-nowrap text-[#FF000D] text-[18rpx] h-[30rpx] leading-[30rpx] rounded-[15rpx] mr-[10rpx] flex-shrink-0">{{ item.type_name }}</text>
								<text
									class="truncate max-w-[226rpx] text-[24rpx] text-[var(--text-color-light6)] leading-[34rpx]">{{ item.title }}</text>
							</view>
							<view
								class="w-[100%] mt-[6rpx] text-[20rpx] leading-[34rpx] text-[var(--text-color-light6)]">
								<text>有效期至
						...
						<view class="px-[20rpx]">
							...
							<button class="flex-center rounded-full remove-border"
								:style="{width:'150rpx',height:'60rpx',color:'var(--primary-help-color4)', fontSize:'24rpx', padding:'0',border:'none',backgroundColor:'#FFEFF0'}"
					...
					<view v-else
						...
						:class="{'mt-[var(--top-m)]' : index}">
						<view
							class="coupon-bg w-[186rpx] h-[160rpx] flex  flex-col items-center justify-center rounded-[var(--rounded-small)] relative coupon-item">
							...
							<text
								class="truncate max-w-[176rpx] mt-[6rpx] text-[22rpx] text-[#fff] h-[32rpx] leading-[32rpx]">{{ item.title }}</text>
						</view>
						<view class="ml-[30rpx] flex-1 h-[100%] box-border py-[20rpx]">
							<view class="text-[26rpx] leading-[40rpx] text-left font-500">
								<text v-if="item.min_condition_money === '0.00'">无门槛</text>
							...
							<view class="text-[20rpx]  mt-[10rpx] flex items-center">
								<text
									class="w-[80rpx] text-center bg-[#FFEFF0] whitespace-nowrap text-[#FF000D] text-[18rpx] h-[30rpx] leading-[30rpx] rounded-[15rpx] mr-[10rpx] flex-shrink-0">{{ item.type_name }}</text>
								<text
									class="truncate max-w-[226rpx] text-[24rpx] text-[var(--text-color-light9)] leading-[34rpx]">{{ item.title }}</text>
							</view>
							<view
								class="w-[100%] mt-[6rpx] text-[20rpx] leading-[34rpx] text-[var(--text-color-light9)]">
								<text>有效期至
									<text>{{ item.expire_time ? item.expire_time.slice(0, 10) : '' }}</text>
								</text>
							</view>
						...
						<view class="px-[20rpx]" v-if="couponStatus === 1">
							<button hover-class="none" class="flex-center rounded-full remove-border bg-[#FF000D]"
...
<script setup lang="ts">
	...
	import { getMyCouponList, getMyCouponType, getMyCouponStatusCount } from '@/addon/home_service/user/api/coupon'
...
<style lang="scss" scoped>
	...
	.coupon-bg {
		background: linear-gradient(169deg, #FF000D 0%, #FF000D 86%);
	}
