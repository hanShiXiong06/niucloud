		...
		<view
			...
			<view class="flex flex-nowrap">
				...
				<view
					...
					:class="currentFilter === 'comprehensive' ? 'active-background active-border' : 'bg-transparent border-transparent'"
					@click="switchFilter('comprehensive')">
					<text class="!text-[26rpx]"
						:class="currentFilter === 'comprehensive' ? 'active-color' : 'text-gray-500'">
						综合
					</text>
				</view>
				<!-- 时间筛选按钮 -->
				<view class="flex items-center justify-center w-[90rpx] py-[8rpx] rounded-[16rpx] border-[2rpx] px-[10rpx] border-style"
					...
					<text class="!text-[26rpx]"
						:class="currentFilter === 'time' ? 'active-color' : 'text-gray-500'">时间</text>
					<view class="ml-[8rpx] text-sm transition-transform duration-300 flex flex-col">
						<text class="iconfont iconshangjiantou text-[20rpx] mb-[-5rpx]"
							:class="sortDirection == 'desc' && currentFilter === 'time' ? 'active-color' : 'text-[#999999]'"></text>
						<text class="iconfont iconxiajiantou text-[20rpx]"
							:class="sortDirection == 'asc' && currentFilter === 'time' ? 'active-color' : ' text-[#999999]'"></text>
					</view>
				</view>
			</view>
		</view>
...
<script setup lang="ts">
  ...
  const toDetail = (id: string | number) => {
	...
	const previewImage = (image : EvaluateItem, index : number) => {
		var urls = []
