<template>
    <view class="bg-gray-100 min-h-[100vh]" :style="themeColor()">
        ...
        <u-popup :show="labelPopup" mode="top" @close="labelPopup = false">
            ...
            <view class="flex flex-wrap pl-[30rpx]" :style="labelPopupStyle">
                ...
                <template v-for="(item, index) in articleList">
                    <template v-if="listType">
                        <view class="bg-white flex p-[20rpx] rounded-[16rpx]" :class="{ 'mt-[20rpx]': index }" @click="toDetail(item.goods_id)">
                            <up-image class="overflow-hidden" radius="10rpx" width="200rpx" height="200rpx" :src="img(item.goods_cover_thumb_small ? item.goods_cover_thumb_small : '')" mode="aspectFill">
                                <template #error>
                        ...
                        <view class="w-[342rpx] bg-[#fff] box-border rounded-[10rpx] overflow-hidden mt-[20rpx]" @click="toDetail(item.goods_id)">
                            ...
                            <view class="pl-[22rpx] pb-[20rpx] pr-[30rpx] flex justify-between items-end mt-[12rpx]">
                                <view class="flex justify-between items-end">
                                    <text class="text-[28rpx] text-[var(--price-text-color)] price-font">
                                        ￥{{ goodsPrice(item) }}
                                        <image v-if="item.member_discount !=''" class="h-[24rpx] ml-[4rpx] w-[60rpx]" :src="img('addon/home_service/VIP.png')" mode="heightFix" />
                                    </text>
                                </view>
...
import { getCategory, getGoodsList } from '@/addon/home_service/user/api/goods';
...
onLoad(async(option) => {
    ...
    if(option.goods_ids){
         goods_ids.value = option.goods_ids
         getAllAppListFn()
    }
})
const labelPopupStyle = computed(() => {
    let val = `padding-top: ${ systemStore.topTabbarInfo.height || 0 }px;`
    return val;
})
