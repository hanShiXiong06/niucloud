                ...
                <view class="bg-[#fff] mx-3 mt-3  p-3 rounded" @click="toLink(item.id)">
                    ...
                    <view class="flex">
                        <view class="min-w-[110rpx] text-center">
                            <text class="text-[20rpx] bg-[#333333] text-[#a9a089] px-[10rpx] py-[6rpx] rounded-full mt-[10rpx]">{{ item.status == 1 ? '正常' : item.status == -1 ? t('haveLeft') : t('takeBreak') }}</text>
                        </view>
...
import { getTechnicianList } from '@/addon/home_service/user/api/technician'
...
let topTabbarData = topTabarObj.setTopTabbarParam({ title: '师傅列表', topStatusBar: { textColor: '#333', bgColor: '#fff' }})
/********* 自定义头部 - end ***********/
...
// 跳转详情页
const toLink = (id: any) => {
    redirect({ url: '/addon/home_service/pages/technician/detail', param: { id } })
}
// 搜索师傅
const searchNameFn = () => {
