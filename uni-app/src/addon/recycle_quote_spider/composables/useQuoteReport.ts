...
import { getQuoteSpiderDetail, getQuoteSpiderCategoryTree, getQuoteSpiderItems } from '@/addon/recycle_quote_spider/api/quotation'
...
const watermark = ref<string>(readStore('qr_watermark', ''))
// 水印样式（lime-painter 不支持真旋转，angle 用斜向错位平铺近似）；本地持久化
const wm = reactive(
	Object.assign({ size: 34, opacity: 8, color: '#000000', angle: 30 }, readStore('qr_wm', {}) as Record<string, any>)
)
const themeKey = ref(readStore('qr_theme', 'midnight'))

// 改动即写本地，下次进来自动带出
watch(watermark, v => writeStore('qr_watermark', v))
watch(wm, () => writeStore('qr_wm', { size: wm.size, opacity: wm.opacity, color: wm.color, angle: wm.angle }), { deep: true })
watch(themeKey, v => writeStore('qr_theme', v))
...
function flattenTree(tree: any[], map: Record<number, { parent: number }> = {}) {
	;(tree || []).forEach(n => {
		map[Number(n.id)] = { parent: Number(n.parent_id || 0) }
		if (Array.isArray(n.children)) flattenTree(n.children, map)
	})
	return map
}
function topAncestorId(tree: any[], catId: number): number {
	const map = flattenTree(tree)
	let cur = Number(catId || 0)
	let guard = 0
	while (cur && map[cur] && map[cur].parent && guard < 30) {
		cur = map[cur].parent
		guard++
	}
...
async function load(id: number | string, src = 'spider') {
	entryItemId.value = id
	source.value = src
	loading.value = true
	try {
		const detailRes: any = await getQuoteSpiderDetail(id)
		const detail = detailRes?.data || {}
		const catId = Number(detail?.category_id || 0)
		...
		sheets.value = list.map((it: any) =>
			reactive({
				...
				adjustValue: 0,
				detail: String(it.id) === String(id) ? detail : null,
				overrides: {},
				marks: {},
				selectedModels: []
			})
...
async function ensureDetail(sheet: ReportSheet) {
	...
	const res: any = await getQuoteSpiderDetail(sheet.id)
