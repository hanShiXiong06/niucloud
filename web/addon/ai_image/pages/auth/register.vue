    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 flex items-center justify-center p-4">
        <div class="w-full max-w-5xl">
            ...
            <div class="text-center mb-8">
                ...
                <p class="text-gray-400">创建您的账户，开启AI创作之旅</p>
            </div>

            <!-- 注册卡片容器 -->
            <div class="flex flex-wrap justify-center gap-6">
                <!-- 扫码注册区域 -->
                <div v-if="config && config?.is_scan == 1"
                    ...
                    <div class="flex items-center mb-6">
                        ...
                        <h2 class="text-xl font-semibold text-white">微信扫码注册</h2>
                    </div>

                    <div class="flex flex-col items-center py-8">
                        <div class="text-gray-300 text-sm mb-4">打开手机微信，扫描二维码注册</div>
                        ...
                        <div class="text-gray-400 text-xs mt-4">扫码后请在手机上确认注册</div>
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="flex items-center mb-6">
                        ...
                        <h2 class="text-xl font-semibold text-white">账号注册</h2>
                    </div>

                    <!-- 注册方式切换 -->
                    <div class="flex gap-4 mb-6" v-if="registerType.length > 1">
                        <button v-for="item in registerType" :key="item.type" @click="type = item.type" :class="[
                    ...
                    <el-form :model="formData" ref="formRef" :rules="formRules" :validate-on-rule-change="false">
                        ...
                        <div v-show="type == 'username'" class="space-y-4">
                            ...
                            <el-form-item prop="confirm_password">
                                <el-input v-model="formData.confirm_password" placeholder="请确认密码" type="password"
                            ...
                            <el-form-item prop="captcha_code">
                                <el-input v-model="formData.captcha_code" placeholder="请输入验证码" size="large">
                                    <template #prefix>
                                        <svg class="w-5 h-5 text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd"
                                    ...
                                    <template #suffix>
                                        <div class="h-[32px] flex items-center">
                                            <el-image :src="captcha.image.value" class="h-[32px] cursor-pointer rounded"
                        ...
                        <!-- 手机号注册 -->
                        <div v-show="type == 'mobile' || configStore.login.is_bind_mobile" class="space-y-4">
                            <el-form-item prop="mobile">
                                <el-input v-model="formData.mobile" placeholder="请输入手机号" size="large" clearable>
                                    <template #prefix>
                                        <svg class="w-5 h-5 text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                                            <path
                            ...
                            <el-form-item prop="mobile_code">
                                <el-input v-model="formData.mobile_code" placeholder="请输入验证码" size="large">
                                    <template #prefix>
                                        <svg class="w-5 h-5 text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd"
                                                d="M18 8a6 6 0 01-7.743 5.743L10 14l-1 1-1 1H6v2H2v-4l4.257-4.257A6 6 0 1118 8zm-6-4a1 1 0 100 2 2 2 0 012 2 1 1 0 102 0 4 4 0 00-4-4z"
                                    ...
                                    <template #suffix>
                                        <sms-code :mobile="formData.mobile" type="register"
                                            v-model="formData.mobile_key" @click="sendSmsCode"
                                            ref="smsCodeRef"></sms-code>
                                    </template>
                                </el-input>
                            </el-form-item>
                        </div>
                        ...
                        <div class="flex justify-between items-center text-sm mt-4">
                            <NuxtLink to="/ai_image/auth/login"
                        ...
                        <div class="text-xs text-gray-400 text-center mt-6" v-if="configStore.login.agreement_show">
                            注册即表示同意
                            <NuxtLink to="/ai_image/auth/agreement?key=service"
                                class="text-purple-400 hover:text-purple-300">
                                《用户协议》
                            </NuxtLink>
                            和
                            <NuxtLink to="/ai_image/auth/agreement?key=privacy"
                                class="text-purple-400 hover:text-purple-300">
                                《隐私政策》
                            </NuxtLink>
                        </div>
...
import { usernameRegister, mobileRegister, scanlogin, checkscan } from '@/app/api/auth'
...
const registerType = computed(() => {
    const value = []
    configStore.login.is_username && (value.push({ type: 'username', title: '账号注册' }))
    configStore.login.is_mobile && !configStore.login.is_bind_mobile && (value.push({ type: 'mobile', title: '手机号注册' }))
