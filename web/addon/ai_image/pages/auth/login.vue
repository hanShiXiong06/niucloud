                    ...
                    <el-form :model="formData" ref="formRef" :rules="formRules" :validate-on-rule-change="false">
                        ...
                        <el-form-item class="mt-6">
                            <button @click="handleLogin" :disabled="loading"
                                ...
                                <span class="relative z-10">{{ loading ? '登录中...' : '🚀 立即登录' }}</span>
                            </button>
                        </el-form-item>

                        <!-- 底部链接 -->
                        <div class="flex justify-between items-center text-sm mt-4">
                            <NuxtLink to="/ai_image/auth/register"
...
const loginType = computed(() => {
    const value = []
    configStore.login.is_username && (value.push({ type: 'username', title: '账号登录' }))
    configStore.login.is_mobile && (value.push({ type: 'mobile', title: '手机号登录' }))
