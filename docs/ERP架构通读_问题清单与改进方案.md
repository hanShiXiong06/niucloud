...
### 6.1 员工绩效·薪酬模块（融入 ERP）
把回收插件里现有的"员工考核看板"融进 ERP，并补全为完整的**绩效→算薪→发薪**闭环。理由：ERP 是财务/数据中枢，全员操作留痕（操作事件脊柱）本就在此汇集，算薪发薪天然属于支出，放 ERP 最顺。

消费的数据源（需打通全链路）：
- 跨插件**操作事件**：回收 / ERP / 中台 / 商城 的"谁在何时干了什么"（已有操作事件脊柱）。
- 回收侧**签到**数据（次数）。
- 岗位**基础工资**配置。
- **KPI 评分**规则（岗位维度）。

产出：每人一张实时绩效卡 → 基础岗位工资 + 本月签到次数 + 考核分 + 岗位 KPI → 计算应发工资 → 发薪（计入经营支出/资金流水）。

> 状态：需求已记录，待主线（看板/报表）稳定后再设计实施。

### 6.2 财务中心检索语义化拆分（近期可做）
把"明细/结算"列表的笼统关键词搜索，拆成按语义的独立入口：**供货商/客户**（复用 `counterparty-select` 按人锚定）、**IMEI**（精确查某台账）、+ 已有的指定日期/结清状态/出库方式/操作人。架构上=复用现成组件 + 列表服务加筛选参数，改动小。
