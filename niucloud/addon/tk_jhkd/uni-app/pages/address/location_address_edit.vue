<template>
	<view :style="themeColor()">
		<scroll-view scroll-y="true" class="bg-page h-screen">
			<!-- <view class="h-[88rpx]">
                    <up-navbar title="添加地址" @rightClick="rightClick" :autoBack="true"></up-navbar>
            </view> -->
			...
			<view class="m-[30rpx] mt-0 p-[30rpx] pt-[10rpx] rounded-md bg-white">
				<up-form labelPosition="left" :model="formData" labelWidth="200rpx" errorType='toast' :rules="rules"
					ref="formRef">
					<view class="mt-[10rpx]">
						<up-form-item :label="t('name')" prop="name" :border-bottom="true">
							<up-input v-model.trim="formData.name" border="none" clearable maxlength="25"
								:placeholder="t('namePlaceholder')" />
						</up-form-item>
					</view>
					<view class="mt-[10rpx]">
						<up-form-item :label="t('mobile')" prop="mobile" :border-bottom="true">
							<up-input v-model="formData.mobile" border="none" clearable
								:placeholder="t('mobilePlaceholder')" />
						</up-form-item>
					</view>
					<view class="mt-[10rpx]">
						<up-form-item :label="t('deliveryAddress')" prop="address_name" :border-bottom="true">
							<view class="flex justify-between flex-1" @click="chooseLocation">
								...
								<up-icon name="arrow-right" color="#c3c4d5"></up-icon>
							</view>
						</up-form-item>
					</view>
					<view class="mt-[10rpx]">
						<up-form-item :label="t('address')" prop="address" :border-bottom="true">
							<up-input v-model.trim="formData.address" border="none" clearable maxlength="120"
								:placeholder="t('addressPlaceholder')" />
						</up-form-item>
					</view>
					<view class="mt-[10rpx]">
						<up-form-item :label="t('defaultAddress')" prop="name" :border-bottom="true">
							<up-switch v-model="formData.is_default" size="20" :activeValue="1" :inactiveValue="0"
								activeColor="var(--primary-color)" />
						</up-form-item>
					</view>
					<view class="mt-[40rpx]">
						<up-button type="primary" shape="circle" :text="t('save')" @click="save" :disabled="btnDisabled"
							:loading="operateLoading"></up-button>
					</view>
				</up-form>
			</view>
		</scroll-view>
