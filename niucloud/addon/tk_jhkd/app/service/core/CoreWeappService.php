...
class CoreWeappService extends BaseCoreService
{
    ...
    public function addPlugin($site_id,$plugin_appid){
        try {
            $response = self::appApiClient($site_id)->postJson('/wxa/plugin', [
                'action'=>'apply',
     ...
     * @throws InvalidArgumentException
     * @author: TK
     * @Time: 2024/7/31   下午11:04
     */
    public function getAccessToken(int $site_id)
    {
        return self::app($site_id)->getAccessToken();
    }
   public function getScheme($site_id,$path='',$query='')
   {
       $response = self::appApiClient($site_id)->postJson('/wxa/generatescheme', [
           'jump_wxa'=>[
               'path'=>$path,
               'query'=>$query,
           ],
