...
//                };
        ...
        foreach ($list['data'] as $k => &$v) {
            ...
            if ($member_info) {
                ...
                $use_member_count = $coupon_member->where([['site_id', '=', $this->site_id], ['member_id', '=', $this->member_id], ['coupon_id', '=', $v['id']], ['status', '=', CouponMemberDict::WAIT_USE]])->count();
                if ($coupon_member_count) {
                    $v['is_receive'] = 1;
                    $v['member_receive_count'] = $coupon_member_count;
                    $v['use_member_count'] = $use_member_count;
                } else {
                    $v['is_receive'] = 0;
                    $v['member_receive_count'] = 0;
                    $v['use_member_count'] = 0;
                }
            } else {
                $v['member_receive_count'] = 0;
                $v['is_receive'] = 0;
...
//        $where[] = [ 'expire_time', '>', time() ];
    ...
    {
        $data = event('JhkdCouponReceiveType', []);
        if (empty($data)) {
            return [];
        }
...
//        ];
    ...
    public function getQrcode($id)
    {
        ...
        $page = 'addon/tk_jhkd/pages/coupon/detail';
        ...
        $dir = 'upload/' . $this->site_id . '/tk_jhkd_coupon_qrcode';
        $channel = 'weapp';
    ...
    public function getUseMemberCouponList()
    {
        $where = array(
            ['member_id', '=', $this->member_id],
            ['status', '=', CouponMemberDict::WAIT_USE],
        ...
        return (new CouponMember())->where($where)->field($field)->select()->toArray();
    }
}
