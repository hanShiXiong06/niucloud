<template>
	<view class="bg-[#fcfcfa] min-h-screen overflow-hidden pl-2 pr-2">
		<mescroll-body ref="mescrollRef" top="12rpx" @init="mescrollInit" @down="downCallback" @up="getMemberListFn">
			<view class="flex tk-card rounded-[8rpx] justify-between items-center !bg-[#676a4c]">
				<view class="w-[80%]">
					<up-input clearable v-model="keyworld" placeholder="请输入会员昵称" @change="reload()"></up-input>
				</view>
				<view>
					<button @click="redirect({url:'/app/pages/verify/index'})" class="hover:bg-blue-700">
						<up-icon name="scan" color="#6b6b6b" size="38"></up-icon>
					</button>
				</view>
			</view>
			<view v-if="list.length > 0" class="tk-card mb-2" style="margin-top:30rpx;margin-bottom:12rpx"
					...
					<view class="flex">
						...
						<view class="ml-2">
							...
							<view class="flex items-center mt-1">
								<view>
									<up-icon @click="editReal(item)" :name="img('addon/tk_vip/card.png')"></up-icon>
								</view>
								<view class="text-xs ml-1">
									<up-tag v-if="item.real_info&&item.real_info.status==1" size="mini" bgColor="#f1ecda"
							...
							<view class="flex items-center">
								...
								<view class="ml-2">
									<up-icon @click="editPointEvent(item)" name="edit-pen"></up-icon>
								</view>
							</view>
						</view>
					</view>
					<view>
						<up-tag v-if="item.level_id>0" @click="openEditVip(item)" size="mini" bgColor="#494b33"
							borderColor="#b0a759" color="#E6DB74" plain :text="item.level_id_name">
						</up-tag>
						<up-tag v-if="item.level_id==0" @click="openEditVip(item)" size="mini" bgColor="#f1ecda"
				...
				<view @click="redirect({url:'/addon/tk_vip/pages/member?id='+item.id})"
					...
					<view class="mt-1">
						<up-tag @click="redirect({url:'/addon/tk_vip/pages/member?id='+item.id})" size="mini"
							borderColor="#b0a759" color="#b0a759" plain text="查看详情"></up-tag>
					</view>
				</view>
			</view>
	...
	<up-popup :show="editPointShow" mode="center" :round="10" :border-radius="20" :safe-area-inset-bottom="true">
		<view class="p-4 items-center ">
			...
			<view class="mt-4">
				<up-input v-model="point" placeholder="请输入变动积分"></up-input>
			</view>
