<template>
	<view v-if="detail" class="bg-[#fcfcfa] min-h-screen overflow-hidden pl-2 pr-2">
		<view class="tk-card mt-2">
			<view class="flex  justify-between">
				<view class="flex">
					...
					<view class="ml-2">
						...
						<view class="flex items-center mt-1">
							<view>
								<up-icon @click="editReal()" :name="img('addon/tk_vip/card.png')"></up-icon>
							</view>
							<view class="text-xs ml-1">
								<up-tag v-if="detail.real_info && detail.real_info.status == 1" size="mini"
									bgColor="#f1ecda" borderColor="#dcdcd3" color="#000000" plain text="已认证">
								</up-tag>
								<up-tag v-else size="mini" borderColor="#dcdcd3" color="#fc0004" plain text="未认证">
								</up-tag>
							</view>
						</view>
					</view>
				...
				<view>
					<up-tag v-if="detail.level_id > 0" @click="openEditVip()" size="mini" bgColor="#494b33"
						borderColor="#b0a759" color="#E6DB74" plain :text="detail.level_id_name">
					</up-tag>
					<up-tag v-if="detail.level_id == 0" @click="openEditVip()" size="mini" bgColor="#f1ecda"
		...
		<view class="tk-card">
			...
			<view class="flex items-center justify-between mt-1">
				...
				<view class="flex items-center">
					...
					<view class="ml-2">
						<up-icon @click="editBalanceEvent()" name="edit-pen"></up-icon>
					</view>
				</view>
			</view>
			<view class="flex items-center justify-between mt-2">
				...
				<view class="flex items-center">
					...
					<view class="ml-2">
						<up-icon @click="editPointEvent()" name="edit-pen"></up-icon>
					</view>
				</view>
			</view>
			<view class="flex items-center justify-between mt-2">
				...
				<view class="flex items-center">
					...
					<view class="ml-2">
						<up-icon @click="openEditVip()" name="edit-pen"></up-icon>
					</view>
				</view>
			</view>
			<view class="flex items-center justify-between mt-2">
				...
				<view class="flex items-center">
					...
					<view class="ml-2">
						<up-icon @click="telPhone(detail.memberInfo.mobile)" name="phone" size="20"></up-icon>
					</view>
				</view>
			</view>
			<view class="flex items-center justify-between mt-2">
				...
				<view v-if="detail.over_time == 0 && detail.level_id > 0" class="mt-1">
					<up-tag size="mini" borderColor="#b0a759" color="#b0a759" plain text="永久"></up-tag>
				</view>
			</view>
			<view class="flex items-center justify-between mt-2">
		...
		<view class="tk-card" v-if="detail.real_info && detail.real_info.real_name">
			...
			<view class="flex items-center justify-between mt-2">
				...
				<view @click="openEditReal()" class="flex items-center">
					...
					<view class="ml-2">
						<up-icon @click="openEditReal()" name="edit-pen"></up-icon>
					</view>
				</view>
			...
			<view class="flex items-center justify-between mt-2">
				...
				<view class="flex items-center">
					...
					<view class="ml-2">
						<up-icon @click="telPhone(detail.real_info.mobile)" name="phone" size="20"></up-icon>
					</view>
				</view>
		...
		<view class="tk-card !bg-[#494b33] !mt-4" v-else @click="addRealShow = true">
			<view class="flex items-center text-center justify-center">
				<view>
					<up-icon name="plus" color="#ffffff"></up-icon>
				</view>
				<view class="ml-2 text-white">新增认证</view>
			</view>
		</view>
	</view>
	<up-popup :show="editPointShow" mode="center" :round="10" :border-radius="20" :safe-area-inset-bottom="true">
		<view class="p-4 items-center ">
			<view class="text-xs text-slate-500 w-[420rpx]">请填写变动积分,小于0将减少变动积分，大于0将增加变动积分</view>
			<view class="mt-4">
				<up-input clearable v-model="point" placeholder="请输入变动积分"></up-input>
			</view>
			<view class="flex justify-center mt-4 mb-1">
				<view class="w-[200rpx]">
					<up-button color="#828282" shape="circle" @click="editPointShow = false">关闭</up-button>

				</view>
				<view class="ml-2 w-[200rpx]">
					<up-button color="#525548" shape="circle" @click="adjustPointEvent()">确认修改</up-button>
				</view>

			</view>
		</view>
	</up-popup>
	<up-popup :show="editBalanceShow" mode="center" :round="10" :border-radius="20" :safe-area-inset-bottom="true">
		<view class="p-4 items-center ">
			<view class="text-xs text-slate-500 w-[420rpx]">请填写变动余额,小于0将减少变动余额，大于0将增加变动余额</view>
			<view class="mt-4">
				<up-input clearable v-model="balance" placeholder="请输入变动余额"></up-input>
			</view>
			<view class="flex justify-center mt-4 mb-1">
				<view class=" w-[200rpx]">
					<up-button color="#828282" shape="circle" @click="editBalanceShow = false">关闭</up-button>
				</view>
				<view class="ml-2 w-[200rpx]">
					<up-button :loading="loading" color="#525548" shape="circle"
						@click="adjustBalanceEvent()">确认修改</up-button>
				</view>
			</view>
		</view>
	</up-popup>
	<up-popup :show="editVipShow" mode="center" :round="10" :border-radius="20" :safe-area-inset-bottom="true">
		<view class="p-4 items-center mt-2">
			...
			<view class="flex items-center justify-between">
				<view class="flex">
					<view>等级：</view>
					<view>
						<up-tag v-if="level_id == 0" size="mini" bgColor="#494b33" borderColor="#b0a759" color="#E6DB74"
							plain text="普通会员"></up-tag>
						<up-tag v-else size="mini" bgColor="#494b33" borderColor="#b0a759" color="#E6DB74" plain
							:text="level_name"></up-tag>
					</view>
				</view>
				<view class="ml-2"><up-tag @click="selectLevelShow = true" borderColor="#b0a759" color="#b0a759" plain
						text="更改等级"></up-tag></view>
			</view>
			<view class="flex items-center mt-4 justify-between">
				<view v-if="over_time == 0">永久会员</view>
				<view v-else>到期时间：{{ over_time }}</view>
				<view class="ml-6"><up-tag plain borderColor="#b0a759" color="#b0a759" @click="changeTime()"
						text="更改时间"></up-tag></view>
			</view>
			<view class="flex justify-center mt-8 mb-2">
				<view class="w-[240rpx]">
					<up-button color="#828282" shape="circle" @click="editVipShow = false">关闭</up-button>
				</view>
				<view class="ml-4 w-[240rpx]">
					<up-button color="#525548" shape="circle" @click="editVipEvent()">确认修改</up-button>
				</view>
			</view>
		</view>
	</up-popup>
	<up-popup :show="selectLevelShow" mode="bottom" :round="10" :border-radius="20" :safe-area-inset-bottom="true">
	...
	<up-popup zIndex="0" closeable @close="editRealShow = false" :show="editRealShow" mode="bottom" :round="10"
		...
		<view class="p-4 items-center mt-2">
			...
			<view class="flex justify-center mt-8 mb-2">
				<view class="w-[300rpx]">
					<up-button color="#828282" shape="circle" @click="editRealShow = false">关闭</up-button>
				</view>
				<view class="ml-2 w-[300rpx]">
					<up-button color="#525548" shape="circle" @click="editRealEvent()">确认修改</up-button>
				</view>

			</view>
		</view>
	</up-popup>
	<up-popup closeOnClickOverlay="true" :show="addRealShow" mode="bottom" :round="10" :border-radius="20"
		...
		<view class="p-4 items-center mt-2">
			...
			<view class="flex justify-center mt-8 mb-2">
				<view class="w-[300rpx]">
					<up-button color="#828282" shape="circle" @click="addRealShow = false">关闭</up-button>
				</view>
				<view class="ml-2 w-[300rpx]">
					<up-button color="#525548" shape="circle" @click="addRealEvent()">确认认证</up-button>
				</view>
			</view>
		</view>
	</up-popup>
	...
	<view class="fixed bottom-28 right-4 z-50 rounded-full p-2 text-white ">
		<up-icon @click="redirect({ url: '/app/pages/verify/index' })" name="scan" color="#6b6b6b" size="36"></up-icon>
	</view>
</template>
