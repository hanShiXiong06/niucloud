...
class MemberService extends BaseApiService
{
    ...
    public function getInfo(int $id)
    {
        ...
        $info = $this->model->field($field)->where([['id', "=", $id]])
            ->with(['memberLevel'])
            ->findOrEmpty();
        if ($info->isEmpty()) throw new CommonException('会员信息异常');
    ...
    {
        if (!is_numeric($data['account_data'])) throw new CommonException('调整积分必须为数字');
    ...
    {
        if (!is_numeric($data['account_data'])) throw new CommonException('调整余额必须为数字');
        $memberInfo = (new Member())->where(['site_id' => $this->site_id, 'member_id' => $this->member_id])->findOrEmpty();
