...
class RealService extends BaseApiService
{
    ...
    public function submitReal($data)
    {
        try {
            ...
            $config = $this->getRealConfig();
            if ($config['open_real'] == 0) throw new CommonException('未开启实名认证');
            $info = $this->model->where(['site_id' => $this->site_id, 'member_id' => $this->member_id])->findOrEmpty();
            if (!$info->isEmpty()) {
                $is_hasone = 1;
                if ($info['status'] == RealDict::AUDIT_SUCCESS) throw new CommonException('已实名认证成功');
                if ($info['status'] == RealDict::WAIT_AUDIT) throw new CommonException('请等待管理员审核');
            } else {
                $is_hasone = 0;
                //新添加审核  进行数量限制
                ...
                if ($max_num > 0) {
                    ...
                    if ($history_num >= $max_num) throw new CommonException('当前身份证认证账号已到上限');
                }
            }
            if ($config['is_auto'] == 0) {
    ...
    public function getRealConfig($type = '')
    {
        ...
        if (empty($info)) {
            if ($type == '') {
                throw new CommonException('实名认证配置未完成');
            }
