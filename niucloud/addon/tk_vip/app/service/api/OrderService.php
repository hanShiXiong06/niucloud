...
class OrderService extends BaseApiService
{
    ...
    public function add(array $data)
    {
        ...
        if (isset($level_list[$data['level_index']]) && isset($level_list[$data['level_index']]['level_benefits']['tk_vip_fee']) && isset($level_list[$data['level_index']]['level_benefits']['tk_vip_fee']['fee_info'])) {
            ...
            if (!isset($sku_data['id']) || ($sku_data['id'] != $data['sku_info']['id'])) throw new CommonException('会员等级信息已经变更，请刷新页面');
            ...
            if (isset($fee_info['is_real']) && $fee_info['is_real'] == 1) {
                ...
                if ($checkInfo['type'] != 'success') throw new CommonException('实名验证未通过');
            }
            $orderData = [
            ...
            throw new CommonException('会员等级信息已经变更，请刷新页面');
        }
    }
    ...
    public function paySuccess($pay_info)
    {
        ...
        try {
            ...
            //进行分销奖励
            (new MemberService())->changeFenxiao($order_info);
            (new FenxiaoService())->fenxiaoEvent($order_info);
            return true;
        } catch (Exception $e) {
            Log::write('VIP付费会员订单回调失败--' . date('Y-m-d H:i:s'));
