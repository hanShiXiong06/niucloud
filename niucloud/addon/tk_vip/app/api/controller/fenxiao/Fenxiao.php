...
namespace addon\tk_vip\app\api\controller\fenxiao;
...
class Fenxiao extends BaseController
{
    ...
    public function getSelfFenxiaoOrder()
    {
        $data = $this->request->params([
            ['type', 'self'],
        ]);
        if ($data['type'] == 'self') {
            $res = (new FenxiaoService())->getSelfFenxiaoOrder($data);
        } else {
            $res = (new FenxiaoService())->getSelfDirectFenxiaoOrder($data);
        }
        return success($res);
    }
