...
<template>
  <view class="chartsview" :id="'ChartBoxId'+cid">
    ...
    <!-- #endif -->
    <!-- APP 使用逻辑层 Canvas。兼容旧版 UniApp App renderjs 多入口 IIFE 限制。 -->
    <!-- #ifdef APP-VUE -->
      ...
      let chartdom = uni
        ...
        .boundingClientRect(data => {
          if (data.width > 0 && data.height > 0) {
            ...
            // H5 采用 renderjs；App 逻辑层 Canvas 与小程序共用稳定渲染路径。
            if (this.inH5) {
              if (this.echarts == true) {
...
// App 的 renderjs 是独立 IIFE 入口。使用隔离副本，避免旧版 UniApp/Rollup
// 将逻辑层与视图层的共享模块拆成 chunk 后触发 IIFE 不支持 code-splitting。
import uChartsRD from '../../js_sdk/u-charts/u-charts.min.js';
import cfu from '../../js_sdk/u-charts/config-ucharts-render.js';
import cfe from '../../js_sdk/u-charts/config-echarts-render.js';
