...
use addon\recycle_daheng_quote\app\model\quotation\RecycleQuotationConfig;
...
{
    const MASK_VALUE = '******';
    const LEGACY_CONFIG_KEY = 'recycle_quotation_crawler_config';
    ...
    private function writeConfig(int $siteId, array $config): void
    {
        $where = [
            ['site_id', '=', $siteId],
            ['config_key', '=', DahengQuoteConfigKeyDict::QUOTATION_CRAWLER],
        ];
        $model = new SysConfig();
    ...
    private function readStoredValue(int $siteId)
    {
        ...
        $legacyRow = (new SysConfig())->where([
            ['site_id', '=', $siteId],
            ['config_key', '=', self::LEGACY_CONFIG_KEY],
        ])->findOrEmpty()->toArray();
