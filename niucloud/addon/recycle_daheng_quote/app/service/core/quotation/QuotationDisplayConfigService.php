...
use addon\recycle_daheng_quote\app\dict\config\DahengQuoteConfigKeyDict;
...
class QuotationDisplayConfigService
{
    ...
    private const LEGACY_CONFIG_KEY = 'recycle_quotation_display_config';
    ...
    {
        $saved = $this->configService->getConfigValue($siteId, DahengQuoteConfigKeyDict::QUOTATION_DISPLAY);
        if (!is_array($saved) || empty($saved)) {
            $saved = $this->configService->getConfigValue($siteId, self::LEGACY_CONFIG_KEY);
        }
    ...
    public function setConfig(int $siteId, array $data): bool
    {
        return (bool)$this->configService->setConfig(
            $siteId,
            DahengQuoteConfigKeyDict::QUOTATION_DISPLAY,
