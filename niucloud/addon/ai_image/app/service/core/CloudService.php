...
class CloudService extends BaseAdminService
{
    public function __construct()
    {
        ...
        $this->addonInfo = (new CoreAddonService())->getAddonConfig('ai_image');
    }
    ...
    {
        $fromPath=root_path() . 'addon' . DIRECTORY_SEPARATOR . 'ai_image' . DIRECTORY_SEPARATOR . 'auth.crt';
    ...
    {
        $addonInfo = (new CoreAddonService())->getAddonConfig('tk_cps');
        $data = [
            'domain' => Request::domain(),
            'ip' => Request::ip(),
            'addon_key' => $addonInfo['key'],
    ...
    {
        $baseUrl = base64_decode("aHR0cDovL2Muc290dWkudG9wLw==");
        $url = $baseUrl . $method;
        $curl = curl_init();
        //设置header头
