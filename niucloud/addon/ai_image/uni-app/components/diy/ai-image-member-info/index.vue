    ...
    <view :style="warpCss">
        <view class="pt-[34rpx] member-info">
            <view v-if="info" class="flex ml-[32rpx] mr-[52rpx]  items-center relative">
                <!-- 唤起获取微信 -->
                <up-avatar v-if="diyComponent.show_member" :src="img(info.headimg)" size="45" leftIcon="none"
                    :default-url="img('static/resource/images/default_headimg.png')" @click="clickAvatar" />
                <view v-if="diyComponent.show_member" class="ml-[22rpx]">
                ...
                <view v-if="diyComponent.show_member" class="set-icon flex items-center absolute right-0 top-2">
            ...
            <view v-else class="flex ml-[32rpx] mr-[52rpx]  items-center relative">
                <up-avatar v-if="diyComponent.show_member" :src="img('static/resource/images/default_headimg.png')"
                    size="45" @click="toLogin" />
                <view v-if="diyComponent.show_member" class="ml-[22rpx]" @click="toLogin">
            ...
            <view class="flex m-[30rpx] mb-0 pb-4 py-[20rpx] items-center justify-between">
                <view class="flex-1 text-center">
                    <view class="font-bold">
                        <view @click="redirect({ url: account ? '/addon/ai_image/pages/list' : '' })"
                            :style="{ color: diyComponent.textColor }">{{ parseInt(account?.create_count) || 0 }}
                        </view>
                    </view>
                    <view class="text-[24rpx] mt-[10rpx]">
                        <view @click="redirect({ url: account ? '/addon/ai_image/pages/list' : '' })"
                            :style="{ color: diyComponent.textColor }">总任务</view>
                    </view>
                </view>
                ...
                <view class="flex-1 text-center">
                    <view class="font-bold">
                        <view @click="redirect({ url: account ? '/addon/ai_image/pages/list' : '' })"
                            :style="{ color: diyComponent.textColor }">{{ parseInt(account?.building_count) || 0 }}
                        </view>
                    </view>
                    <view class="text-[24rpx] mt-[10rpx]">
                        <view @click="redirect({ url: account ? '/addon/ai_image/pages/list' : '' })"
                            :style="{ color: diyComponent.textColor }">正在生成</view>
                    </view>
                </view>
                <view class="flex-1 text-center">
                    <view class="font-bold">
                        <view @click="redirect({ url: account ? '/addon/ai_image/pages/list' : '' })"
                            :style="{ color: diyComponent.textColor }">{{ parseInt(account?.success_count) || 0 }}
                        </view>
                    </view>
                    <view class="text-[24rpx] mt-[10rpx]">
                        <view @click="redirect({ url: account ? '/addon/ai_image/pages/list' : '' })"
                            :style="{ color: diyComponent.textColor }">生成成功</view>
                    </view>
                </view>
                <view class="flex-1 text-center">
                    <view class="font-bold">
                        <view @click="redirect({ url: account ? '/addon/ai_image/pages/list' : '' })"
                            :style="{ color: diyComponent.textColor }">{{ parseInt(account?.fail_count) || 0 }}
                        </view>
                    </view>
                    <view class="text-[24rpx] mt-[10rpx]">
                        <view @click="redirect({ url: account ? '/addon/ai_image/pages/list' : '' })"
                            :style="{ color: diyComponent.textColor }">生成失败</view>
                    </view>
                </view>
            </view>
        </view>
...
import { getStat } from '@/addon/ai_image/api/aiimage'
