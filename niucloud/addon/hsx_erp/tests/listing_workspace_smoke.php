...
$assert(str_contains($mobileDetail, 'handoffMobileStockListing') && str_contains($mobileDetail, '完成并交接') && str_contains($mobileDetail, 'submitProduct(showProductHandoff)') && str_contains($mobileDetail, '已交接，尚未上架') && str_contains($mobileDetail, "['published', 'duplicate'].includes(handoff?.data?.status)"), 'mobile handoff must distinguish actual publication from pending handoff');
