...
$intakeCore = (string)file_get_contents($repo . '/niucloud/addon/phone_shop/app/service/core/intake/CoreDeviceIntakeService.php');
$shopSchemaSync = (string)file_get_contents($repo . '/niucloud/addon/phone_shop/app/service/core/upgrade/SchemaSyncService.php');
...
$assert(strpos($directListing, 'saveFromEvent') < strpos($directListing, 'STATUS_BUILT'), '商城发布必须先校验货源身份，再判断是否重复建品');
$assert(str_contains($intakeCore, 'identityConflicts') && str_contains($intakeCore, 'archiveStaleBinding'), '货源幂等必须识别ERP重装后的资产主键复用并保留旧商品');
$assert(str_contains($shopSchemaSync, 'migrateDeviceIntakeIndexes') && str_contains($shopSchemaSync, 'uk_site_erp_asset'), '商城升级必须把货源唯一键迁移为站点级资产唯一键');
...
$assert(str_contains($mobileStockDetail, '<ErpListingWorkspaceForm') && str_contains($mobileStockDetail, '库存调拨') && str_contains($mobileStockDetail, '调整零售价'), '移动库存档案必须通过通用岗位表单闭环商城资料、调拨和零售价动作');
