...
$assert(str_contains($stock, 'if (!$shopCompletion && !$basicFirst) throw $throwable;') && str_contains($stock, '$categoryPath = [];'), '商城运营交接不得被缺失的ERP分类投影阻塞');
$assert(str_contains($stock, "if (!\$shopCompletion && !\$basicFirst && \$categoryPath === [])"), '原ERP直发模式仍须严格校验ERP商品目录');
