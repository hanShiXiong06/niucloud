...
final class ErpBusinessReportMetricsService
{
    ...
    public function collect(array $request): array
    {
        ...
        $saleCount = $saleIds === [] ? 0 : (int)Db::name('erp_sale_item')->where('site_id', '=', $siteId)->whereIn('sale_order_id', $saleIds)->where('status', '=', 'sold')->count();
        $saleAmount = (float)(clone $saleOrders)->sum('total_amount');
        $saleProfit = (float)(clone $saleOrders)->sum('profit');
        ...
        $openingStock = max(0, $stockCount - $periodInbound + $saleCount);
        $turnoverRate = $openingStock > 0 ? round($saleCount / $openingStock * 100, 2) : 0.0;
        ...
        if ($saleIds !== []) {
            $rows = Db::name('erp_sale_item')->alias('i')->leftJoin('erp_asset a', 'a.id=i.asset_id AND a.site_id=i.site_id')
                ...
                ->field("IF(a.category_name='', '未分类', a.category_name) as name,COUNT(*) as sale_count")->group('a.category_name')->select()->toArray();
            foreach ($rows as $row) {
                $name = (string)$row['name'];
                if (!isset($categories[$name])) $categories[$name] = ['key' => $name, 'name' => $name, 'stock_count' => 0, 'sale_count' => 0, 'in_count' => 0];
                $categories[$name]['sale_count'] = (int)$row['sale_count'];
            }
        }
        if ($purchaseIds !== []) {
        ...
        $staffRows = (clone $saleOrders)->where('salesman_uid', '>', 0)
            ->field('salesman_uid as uid,MAX(salesman_name) as name,COUNT(*) as sale_order_count,SUM(total_amount) as amount,SUM(profit) as profit')
            ->group('salesman_uid')->select()->toArray();
        $staff = array_map(static fn(array $row): array => [
            'uid' => (int)$row['uid'], 'name' => (string)$row['name'], 'role_key' => 'salesman', 'role_name' => '销售开单',
            'count' => (int)$row['sale_order_count'], 'amount' => round((float)$row['amount'], 2), 'profit' => round((float)$row['profit'], 2),
        ], $staffRows);
        ...
        return [
            ...
            'summary' => [
                ...
                'sale_order_count' => (int)(clone $saleOrders)->count(), 'sale_count' => $saleCount,
