...
**已完成并校验（前端 UI，续做部分）：**
- **调拨管理 UI**：ERP 资产列表加"调拨"按钮 + 对话框（选目标仓/库位；代卖设备调进商城仓时出现"上架代卖 / 我方买断"选择，买断显示买断价输入）→ 调 `erp/outbound/transfer`。`asset/list.vue` 编译通过。
- **出库 create 表单**：加每台"应付寄卖人"输入列（仅代卖设备显示）→ 传 `consignor_payable`。`outbound/list.vue` 编译通过。

**尚未做（留待续做）：**
1. **财务待办 Tab（P5）**：财务看板加"打款任务/追款任务"筛选（基于现成应付/应收 outstanding 接口）。
2. **主体关联内联编辑（P6）**：统一组件 + 端点，挂到回收/ERP/销售各处。
3. **中台回推可售**：当前去重已保证"不重拍"；"已拍过照自动回推可售"为可选优化，未做。
