...
const loading = ref(false), rows = ref<any[]>([]), projects = ref<any[]>([]), detail = ref<any>({}), reviewRemark = ref('')
...
async function openDetail(id:number) { drawer.visible = true; drawer.loading = true; reviewRemark.value = ''; Object.keys(issueMap).forEach(key => delete issueMap[key]); try { const res:any = await getProjectCenterApplication(id); detail.value = res.data || {}; await Promise.all([nextTick(), loadReviewReasons(Number(detail.value.project_id || 0))]); fields.value.forEach(item => { issueMap[fieldKey(item)] = { checked: false, reason_id: '', message: '', example: '' } }) } finally { drawer.loading = false } }
