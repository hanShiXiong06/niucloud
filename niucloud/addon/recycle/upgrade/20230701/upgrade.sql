-- 添加操作类型字段
ALTER TABLE `recycle_device_log` ADD COLUMN `operation_type` varchar(20) NOT NULL DEFAULT '' COMMENT '操作类型' AFTER `action`;

-- 为设备表添加分类字段
ALTER TABLE `recycle_device` ADD COLUMN `category_id` int NOT NULL DEFAULT '1' COMMENT '设备分类ID' AFTER `model`;

-- 添加分类字段索引
ALTER TABLE `recycle_device` ADD KEY `idx_category_id` (`category_id`);

-- 创建统计表
CREATE TABLE IF NOT EXISTS `recycle_stats_daily` (
...
-- 创建月统计表
CREATE TABLE IF NOT EXISTS `recycle_stats_monthly` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `site_id` int NOT NULL DEFAULT '0' COMMENT '站点ID',
  `user_id` int NOT NULL DEFAULT '0' COMMENT '用户ID',
  `user_type` varchar(20) NOT NULL DEFAULT '' COMMENT '用户类型：checker(质检员),pricer(估价员),admin(管理员)',
...
-- 创建年统计表
CREATE TABLE IF NOT EXISTS `recycle_stats_yearly` (
  `id` int NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `site_id` int NOT NULL DEFAULT '0' COMMENT '站点ID',
  `user_id` int NOT NULL DEFAULT '0' COMMENT '用户ID',
  `user_type` varchar(20) NOT NULL DEFAULT '' COMMENT '用户类型：checker(质检员),pricer(估价员),admin(管理员)',
