...
# 快速订单流转测试脚本 (修复版)
# 使用方法: ./quick_flow_test_fixed.sh [场景编号]
...
run_scenario() {
    ...
    case $scenario in
        1)
            ...
            # 先更新订单状态为已签收 (修复action参数)
            response=$(api_request "PUT" "$API_BASE/recycle_order/$order_id" '{
                "action": "order_sign", "devices": [], "remark": "订单签收确认"
            }')
            check_api "$response" "订单签收" || return 1
