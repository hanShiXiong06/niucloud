...
export function useDeviceQueryConfig() {
  ...
  const secretForm = reactive<any>(defaultSecretForm())
  ...
  const secretDialogTitle = computed(() => `配置密钥 - ${secretForm.name || secretForm.key || '查询渠道'}`)
  ...
  const getProviderPresetByKey = (providerKey: string) => providerPresets.find(item => item.key === providerKey) || null
  ...
  const isChannelMissingSecret = (channel: any) => {
    return !!channel && channel.auth_type !== 'none' && !String(channel.token || '').trim()
  }

  const getChannelSecretState = (row: any) => {
    const channel = row?.channel || getChannel(row?.key)
    if (!channel) return { type: 'warning', text: '未创建渠道' }
    if (isChannelMissingSecret(channel)) return { type: 'danger', text: '未配置密钥' }
    if (channel.auth_type === 'none') return { type: 'info', text: '无需密钥' }
    return { type: 'success', text: '已配置' }
  }

  const secretGuide = computed(() => {
    const preset = getProviderPresetByKey(secretForm.key)
    return {
      register: preset?.register_guide || '先到第三方服务商后台注册账号，并确认账号已开通对应设备查询接口。',
      document: preset?.document_guide || '在服务商后台查看接口文档，确认请求地址、鉴权字段、服务 ID 或接口路径。',
      recharge: preset?.recharge_guide || '在服务商后台充值余额或购买套餐。设备查询通常会按次扣费。',
      secret: preset?.secret_guide || '在服务商后台复制 API Key 或 token。系统不会内置客户密钥，需要客户自行填写。'
    }
  })
  ...
  const configDiagnostics = computed(() => {
    ...
    const isRunnableMapping = (mapping: any) => {
      ...
      return Number(mapping.enabled) === 1
        ...
        && !isChannelMissingSecret(channel)
    ...
    const secretProblems = channels.value
      .filter(channel => Number(channel.enabled) === 1 && isChannelMissingSecret(channel))
      .map(channel => {
        const affectedMappings = mappings.value.filter(mapping => (
          mapping.channel_key === channel.key
          && Number(mapping.enabled) === 1
      ...
      .map(item => ({
        type: 'danger',
        title: `${item.channel.name || item.channel.key} 未配置密钥`,
        desc: `该服务商下 ${item.service_total} 个查询项、${item.mapping_total} 条映射无法执行查询。请填写客户自己的 API Key 或 token。`,
        action: '配密钥'
      }))
  ...
  const queryChannelBalance = async (row: any) => {
    ...
    if (isChannelMissingSecret(row)) {
      ElMessage.warning('请先点击“配置密钥”，填写客户自己的 API Key 后再查询余额')
      return
    }
    try {
  ...
  const openSecretDialog = (row: any) => {
    const preset = getProviderPresetByKey(row.key)
    const channel = row.channel || getChannel(row.key)
    const source = channel || preset || row
    Object.assign(secretForm, defaultSecretForm(), {
      key: source.key || row.key,
      name: source.name || row.name,
      provider: source.provider || preset?.provider || 'path_query',
      base_url: source.base_url || preset?.base_url || '',
      token: source.token || '',
      auth_type: source.auth_type || preset?.auth_type || 'header',
      auth_key: source.auth_key || preset?.auth_key || 'key',
      service_id_key: source.service_id_key || preset?.service_id_key || 'key',
      _editing: !!channel
    })
    secretDialogVisible.value = true
  }

  const saveSecretConfig = async () => {
    if (!secretForm.key || !secretForm.name || !secretForm.base_url) {
      ElMessage.warning('请补全渠道名称和请求地址')
      return
    }
    if (secretForm.auth_type !== 'none' && !String(secretForm.token || '').trim()) {
      ElMessage.warning('请填写客户从服务商后台获取的 API Key 或 token')
      return
    }

    const oldChannel = getChannel(secretForm.key)
    const preset = getProviderPresetByKey(secretForm.key)
    const record = {
      key: secretForm.key,
      name: secretForm.name,
      provider: secretForm.provider || oldChannel?.provider || preset?.provider || 'path_query',
      enabled: Number(oldChannel?.enabled ?? preset?.enabled ?? 1),
      priority: Number(oldChannel?.priority ?? preset?.priority ?? 100),
      base_url: secretForm.base_url,
      method: oldChannel?.method || 'GET',
      token: secretForm.token,
      auth_type: secretForm.auth_type,
      auth_key: secretForm.auth_key,
      service_id_key: secretForm.service_id_key,
      timeout: oldChannel?.timeout || 300,
      connect_timeout: oldChannel?.connect_timeout || 10,
      balance_warning: oldChannel?.balance_warning || 20
    }
