<?php

namespace addon\sd_xiaoyuan\app\service\core;

use addon\sd_xiaoyuan\app\model\Sign;
use core\base\BaseApiService;
use core\exception\CommonException;

/**
 * 签到服务
 */
class SignService extends BaseApiService
{
    public function __construct()
    {
        parent::__construct();
    }

    /**
     * 签到
     */
    public function sign()
    {
        $today = date('Y-m-d');
        
        // 检查今天是否已签到
        $todaySign = (new Sign())->where([
            ['member_id', '=', $this->member_id],
            ['site_id', '=', $this->site_id],
            ['sign_date', '=', $today]
        ])->find();

        if (!empty($todaySign)) {
            throw new CommonException('今天已经签到过了');
        }

        // 获取昨天的签到记录，计算连续签到天数
        $yesterday = date('Y-m-d', strtotime('-1 day'));
        $yesterdaySign = (new Sign())->where([
            ['member_id', '=', $this->member_id],
            ['site_id', '=', $this->site_id],
            ['sign_date', '=', $yesterday]
        ])->find();

        $continuousDays = 1;
        if (!empty($yesterdaySign)) {
            $continuousDays = $yesterdaySign['continuous_days'] + 1;
            // 最多7天循环
            if ($continuousDays > 7) {
                $continuousDays = 1;
            }
        }

        // 获取签到奖励配置
        $reward = $this->getSignReward($continuousDays);

        // 创建签到记录
        $signData = [
            'site_id' => $this->site_id,
            'member_id' => $this->member_id,
            'sign_date' => $today,
            'continuous_days' => $continuousDays,
            'reward_points' => $reward['points'] ?? 0,
            'reward_coupon_id' => $reward['coupon_id'] ?? 0,
            'create_time' => time()
        ];

        (new Sign())->save($signData);

        // 发放奖励
        if (!empty($reward['points'])) {
            $this->sendPointsReward($reward['points']);
        }

        return [
            'continuous_days' => $continuousDays,
            'reward_points' => $reward['points'] ?? 0,
            'reward_coupon_id' => $reward['coupon_id'] ?? 0
        ];
    }

    /**
     * 获取签到状态
     */
    public function getSignStatus()
    {
        $today = date('Y-m-d');
        
        // 今天是否签到
        $todaySign = (new Sign())->where([
            ['member_id', '=', $this->member_id],
            ['site_id', '=', $this->site_id],
            ['sign_date', '=', $today]
        ])->find();

        $isSigned = !empty($todaySign);
        $continuousDays = $todaySign['continuous_days'] ?? 0;

        // 如果今天没签到，获取昨天的连续天数
        if (!$isSigned) {
            $yesterday = date('Y-m-d', strtotime('-1 day'));
            $yesterdaySign = (new Sign())->where([
                ['member_id', '=', $this->member_id],
                ['site_id', '=', $this->site_id],
                ['sign_date', '=', $yesterday]
            ])->find();
            
            $continuousDays = $yesterdaySign['continuous_days'] ?? 0;
        }

        // 获取本周签到记录
        $weekStart = date('Y-m-d', strtotime('monday this week'));
        $weekSigns = (new Sign())->where([
            ['member_id', '=', $this->member_id],
            ['site_id', '=', $this->site_id],
            ['sign_date', '>=', $weekStart]
        ])->column('sign_date');

        // 获取7天奖励配置
        $rewards = $this->getWeekRewards();

        // 获取用户总积分
        $member = \app\model\member\Member::where([
            ['member_id', '=', $this->member_id],
            ['site_id', '=', $this->site_id]
        ])->field('point')->find();
        $totalPoints = (int)($member['point'] ?? 0);

        return [
            'is_signed' => $isSigned,
            'continuous_days' => $continuousDays,
            'week_signs' => $weekSigns,
            'rewards' => $rewards,
            'total_points' => $totalPoints
        ];
    }

    /**
     * 获取签到奖励
     */
    private function getSignReward($day)
    {
        // 默认奖励配置
        $defaultRewards = [
            1 => ['points' => 10, 'coupon_id' => 0],
            2 => ['points' => 20, 'coupon_id' => 0],
            3 => ['points' => 30, 'coupon_id' => 0],
            4 => ['points' => 40, 'coupon_id' => 0],
            5 => ['points' => 50, 'coupon_id' => 0],
            6 => ['points' => 60, 'coupon_id' => 0],
            7 => ['points' => 100, 'coupon_id' => 0], // 第7天额外奖励
        ];

        return $defaultRewards[$day] ?? ['points' => 10, 'coupon_id' => 0];
    }

    /**
     * 获取一周奖励配置
     */
    private function getWeekRewards()
    {
        return [
            ['day' => 1, 'points' => 10, 'coupon_id' => 0],
            ['day' => 2, 'points' => 20, 'coupon_id' => 0],
            ['day' => 3, 'points' => 30, 'coupon_id' => 0],
            ['day' => 4, 'points' => 40, 'coupon_id' => 0],
            ['day' => 5, 'points' => 50, 'coupon_id' => 0],
            ['day' => 6, 'points' => 60, 'coupon_id' => 0],
            ['day' => 7, 'points' => 100, 'coupon_id' => 0],
        ];
    }

    /**
     * 发放积分奖励
     */
    private function sendPointsReward($points)
    {
        try {
            $accountService = new \app\service\core\member\CoreMemberAccountService();
            $accountService->addLog(
                $this->site_id,
                $this->member_id,
                'point',
                $points,
                'sd_xiaoyuan_sign',
                '签到奖励',
                0
            );
        } catch (\Exception $e) {
            trace('签到积分发放失败：' . $e->getMessage(), 'error');
        }
    }

    /**
     * 获取签到历史
     */
    public function getSignHistory($page = 1, $limit = 20)
    {
        $model = new Sign();
        
        $count = $model->where([
            ['member_id', '=', $this->member_id],
            ['site_id', '=', $this->site_id]
        ])->count();

        $list = $model->where([
            ['member_id', '=', $this->member_id],
            ['site_id', '=', $this->site_id]
        ])->page($page, $limit)->order('sign_date desc')->select()->toArray();

        return [
            'count' => $count,
            'list' => $list
        ];
    }
}
