...
class CommunityService extends BaseApiService
{
    ...
    public function getList($params)
    {
        ...
        if ($school_id > 0) {
            $where[] = ['school_id', 'in', [(int)$school_id, 0]];
        }
    ...
    public function publish($data)
    {
        ...
        $autoApprove = $config['community_auto_approve'] ?? 1;
        
        $schoolId = (int)($data['school_id'] ?? 0);
        $campus = (string)($data['campus'] ?? '');
        if ($schoolId <= 0 && $this->member_id > 0) {
            $authRow = (new \addon\sd_xiaoyuan\app\model\CampusAuth())->where([
                ['member_id', '=', $this->member_id],
                ['site_id', '=', $this->site_id],
                ['status', '=', 1],
            ])->order('id', 'desc')->find();
