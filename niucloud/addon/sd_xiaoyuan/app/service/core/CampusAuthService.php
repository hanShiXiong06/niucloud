...
class CampusAuthService extends BaseApiService
{
    ...
    public function apply($data)
    {
        ...
        $cfg = (new ConfigService())->getConfig();
        $authAuto = !empty($cfg['campus_auth_auto_approve']);
        $passStatus = $authAuto ? 1 : 0;
        $auditTime = $authAuto ? time() : 0;
        ...
        if (!$exists->isEmpty()) {
            ...
            if ($authAuto) {
                $this->initCreditIfNeeded($this->member_id);
            }
    ...
    protected function initCreditIfNeeded($memberId)
    {
