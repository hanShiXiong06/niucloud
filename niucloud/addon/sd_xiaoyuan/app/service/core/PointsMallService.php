...
class PointsMallService extends BaseApiService
{
    ...
    public function exchange(int $goods_id, string $receiver_name, string $receiver_phone, string $receiver_address, string $remark = '')
    {
        ...
        // 获取用户积分（从member表获取）
        $memberQuery = Db::name('member')->where('member_id', '=', $this->member_id);
        if ($memberQuery->getConnection()->getTableFields('member') && in_array('site_id', $memberQuery->getConnection()->getTableFields('member'), true)) {
            $memberQuery->where('site_id', '=', $this->site_id);
        }
        $member = $memberQuery->field('point')->find();
        ...
        try {
            ...
            Db::rollback();
            trace('sd_xiaoyuan points exchange failed: ' . $e->getMessage(), 'error');
            throw new CommonException($e->getMessage() ?: '兑换失败');
        }
    }
    ...
    public function getMyPoints()
    {
        ...
        $points = 0;
        if ($member) {
            $points = (int)(is_array($member) ? ($member['point'] ?? 0) : ($member->point ?? 0));
        }
        return ['points' => $points];
    }

    public function getPointsRecord(string $type = '', int $page = 1, int $limit = 20)
    {
        $where = [
            ['site_id', '=', $this->site_id],
            ['member_id', '=', $this->member_id],
        ...
        $pointTypes = MemberAccountChangeTypeDict::getType(MemberAccountTypeDict::POINT);
        $list = [];
        foreach ($rows as $row) {
            $accountData = (float)($row['account_data'] ?? 0);
            $fromType = $row['from_type'] ?? '';
            $title = $row['memo'] ?? '';
            if ($title === '' && is_array($pointTypes) && !empty($pointTypes[$fromType]['name'])) {
                $title = $pointTypes[$fromType]['name'];
            }
            $createTime = (int)($row['create_time'] ?? 0);
            $list[] = [
                'id' => (int)($row['id'] ?? 0),
                'title' => $title,
                'create_time' => $createTime > 0 ? date('Y-m-d H:i', $createTime) : '',
                'type' => $accountData >= 0 ? 'earn' : 'spend',
                'points' => abs((int)$accountData),
            ];
        }
        return ['list' => $list, 'total' => $count];
    }
