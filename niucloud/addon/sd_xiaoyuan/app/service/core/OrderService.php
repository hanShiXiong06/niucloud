...
class OrderService extends BaseApiService
{
    ...
    public function canViewFullOrderDetail(array $order): bool
    {
        if (empty($order)) {
            return false;
        }
        $viewerMemberId = intval($this->member_id ?? 0);
        $viewerRunnerId = $this->getViewerRunnerRecordId();
        if ($viewerMemberId > 0 && (int)($order['member_id'] ?? 0) === $viewerMemberId) {
            return true;
        }
        $runnerOid = (int)($order['runner_id'] ?? 0);
        if ($runnerOid > 0 && $viewerRunnerId > 0 && $runnerOid === $viewerRunnerId) {
            return true;
        }
        $status = (int)($order['status'] ?? 0);
        if ($status === OrderDict::STATUS_WAIT_ACCEPT && $runnerOid <= 0) {
            return true;
        }
        return false;
    }
    ...
    public function create($data)
    {
        ...
        $useCard = false;
        if (!empty($data['ext'])) {
            $ext = is_string($data['ext']) ? json_decode($data['ext'], true) : $data['ext'];
            if (is_array($ext) && !empty($ext['use_card'])) {
                $useCard = true;
            }
        }
        if (!isset($data['actual_fee']) || ($data['actual_fee'] <= 0 && !$useCard)) {
    ...
    {
        $cfg = (new ConfigService())->getConfig($this->site_id);
        $distance = floatval($params['distance'] ?? 0);
        $weight = floatval($params['weight'] ?? 0);
        $isUrgent = intval($params['is_urgent'] ?? 0);

        $baseFee = floatval($cfg['base_fee'] ?? 3);
        $distanceThreshold = floatval($cfg['distance_threshold'] ?? 2);
        $distanceFeeRate = floatval($cfg['distance_fee'] ?? 1.5);
        $weightThreshold = floatval($cfg['weight_threshold'] ?? 5);
        $weightFeeRate = floatval($cfg['weight_fee'] ?? 0.5);
        $urgentFeeRate = floatval($cfg['urgent_fee'] ?? 5);

        $distanceFee = $distance > $distanceThreshold ? ($distance - $distanceThreshold) * $distanceFeeRate : 0;
        $weightFee = $weight > $weightThreshold ? ($weight - $weightThreshold) * $weightFeeRate : 0;
        $urgentFee = $isUrgent ? $urgentFeeRate : 0;

        $totalFee = round($baseFee + $distanceFee + $weightFee + $urgentFee, 2);

        return [
            'base_fee' => $baseFee,
            'distance_fee' => round($distanceFee, 2),
            'weight_fee' => round($weightFee, 2),
            'urgent_fee' => round($urgentFee, 2),
            'total_fee' => $totalFee
        ];
    }
    ...
    public function getOrderList($params)
    {
        ...
        $credits = [];
        if (!empty($member_ids)) {
            $member_list = (new Member())->where([['member_id', 'in', $member_ids]])->field('member_id,member_no,nickname,headimg')->select()->toArray();
        ...
        foreach ($list as &$item) {
            ...
            $memberRow = (isset($members[$mid]) && is_array($members[$mid])) ? $members[$mid] : [];
            $item['member_nickname'] = $memberRow['nickname'] ?? '';
            $item['member_no'] = $memberRow['member_no'] ?? '';
            $item['member_headimg'] = $memberRow['headimg'] ?? '';
            $item['credit_score'] = $credits[$mid] ?? Credit::INIT_SCORE;

            // 附加接单员信息（列表与 sd-order-item 展示一致）
            $rid = $item['runner_id'] ?? 0;
            $runnerRow = (isset($runners[$rid]) && is_array($runners[$rid])) ? $runners[$rid] : [];
            $item['runner_name'] = $runnerRow['real_name'] ?? '';
            $item['runner_mobile'] = $runnerRow['mobile'] ?? '';
            $item['runner_avatar'] = $runnerRow['avatar'] ?? '';
            $item['runner_score'] = $runnerRow['score'] ?? 0;
    ...
    public function getOrderDetail($orderId, $siteId = 0, $maskYinsi = true)
    {
        ...
        if (!empty($item)) {
            ...
            if (!empty($item['member_id'])) {
                $member = (new Member())->where('member_id', $item['member_id'])->field('member_id,member_no,nickname,headimg')->findOrEmpty()->toArray();
            ...
            $viewerMemberId = (int)($this->member_id ?? 0);
            $item['is_order_owner'] = ($viewerMemberId > 0 && (int)($item['member_id'] ?? 0) === $viewerMemberId) ? 1 : 0;
        }
     ...
     * 校验当前会员是否为下单人
     */
    public function checkOrderOwner(array $order, int $member_id, string $msg = '无权操作此订单')
    {
        if ($member_id <= 0) {
            throw new CommonException('请先登录');
        }
        if ((int)($order['member_id'] ?? 0) !== $member_id) {
    ...
    public function updateStatus($orderId, $status, $data = [])
    {
        ...
        if ($status == 20 && $result) {
            ...
            if ($order) {
                ...
                (new NoticeWechatService())->send((int)$order['site_id'], 'sd_xiaoyuan_order_accept_user', [
                    'order_id' => $orderId,
                ]);
                (new NoticeWeappService())->send((int)$order['site_id'], 'sd_xiaoyuan_order_accept_user', [
                    'order_id' => $orderId,
                ]);
            }
        }
    ...
    public function completeToUserConfirm(int $orderId, array $proofImages = [], int $siteId = 0): bool
    {
        ...
        $config = (new ConfigService())->getConfig((int)$sid);
    ...
    protected function finalizeCompleted(array $order, string $source = 'USER'): bool
    {
        ...
        try {
            ...
            if ($runnerId > 0) {
                ...
                try {
                    (new \addon\sd_xiaoyuan\app\service\core\RunnerInviteService())->onOrderCompleted($runnerId);
                } catch (\Throwable $e) {
                    trace('sd_xiaoyuan finalize invite: ' . $e->getMessage(), 'error');
                }

                $config = (new ConfigService())->getConfig((int)$order['site_id']);
    ...
    public function cancel($orderId, $siteId = 0, string $cancelReason = '', string $cancelRole = 'USER')
    {
        ...
        try {
            if ((int)$order['pay_status'] === 1) {
                ...
                if ($refundAmount > 0) {
                    ...
                    $this->restoreOrderCardIfNeed($order->toArray());
                    $this->model->where('id', $orderId)->update([
                        'status' => 90,
                        'cancel_time' => time(),
                        'cancel_reason' => $cancelReason,
        ...
        if (!empty($order['member_id'])) {
            ...
            (new NoticeWeappService())->send((int)$order['site_id'], 'sd_xiaoyuan_order_cancel_user', [
                'order_id' => $orderId,
                'cancel_reason' => $cancelReason,
                'cancel_role' => $cancelRole,
            ]);
    ...
    protected function restoreOrderCardIfNeed(array $order): void
    {
        $ext = [];
        if (!empty($order['ext'])) {
        ...
        if (empty($ext['card_consumed']) || empty($ext['card_type'])) {
            return;
        }
        (new \addon\sd_xiaoyuan\app\service\core\CardService())->restoreCardForOrder(
            (int)$order['site_id'],
            (int)$order['member_id'],
            (string)$ext['card_type']
        );
        $ext['card_consumed'] = 0;
        $this->model->where('id', (int)$order['id'])->update([
