...
class SchoolService extends BaseAdminService
{
    ...
    public function getPage(array $where = [])
    {
        ...
        $page = $this->pageQuery($search_model);
        if (!empty($page['data']) && is_array($page['data'])) {
            foreach ($page['data'] as $k => $row) {
                $page['data'][$k] = $this->formatSchoolItem($row, false);
            }
        }
        return $page;
    }
    ...
    public function getList(array $where = [])
    {
        ...
        foreach ($result as $k => $row) {
            $result[$k] = $this->formatSchoolItem($row);
        }
    ...
    /**
     * 格式化学校展示字段(logo转完整地址，区域合并)
     */
    private function formatSchoolItem(array $row, bool $format_logo = true): array
    {
        $logo = trim((string)($row['logo'] ?? ''));
        if ($format_logo && $logo !== '') {
            $parts = array_values(array_filter(array_map('trim', explode(',', $logo))));
            $first = $parts[0] ?? '';
            if ($first !== '') {
                $row['logo'] = get_file_url($first);
            }
        }
    ...
    public function getInfo(int $id)
    {
        ...
        $row = $info->toArray();
        return $this->formatSchoolItem($row);
    }
    ...
    public function add(array $data)
    {
        ...
        $data['logo'] = $this->normLogo($data['logo'] ?? '');
     ...
     * logo存相对路径
     */
    private function normLogo($logo): string
    {
