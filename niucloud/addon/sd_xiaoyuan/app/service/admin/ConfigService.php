...
class ConfigService extends BaseAdminService
{
    ...
    {
        $sid = (int)($site_id > 0 ? $site_id : $this->site_id);
        $info = (new CoreConfigService())->getConfig($sid, ConfigDict::getConfigType());
        if (!is_array($info)) {
            $info = [];
        }
        $valueBroken = array_key_exists('value', $info) && !is_array($info['value']);
        if ($valueBroken) {
            $info['value'] = [];
            Cache::tag(CoreConfigService::$cache_tag_name . $sid)->clear();
        ...
        $info['value']['buy_fee'] = $info['value']['buy_fee'] ?? 2.00;
        ...
        $info['value']['commission_rate_class'] = $info['value']['commission_rate_class'] ?? '';
        ...
        $info['value']['single_school'] = $info['value']['single_school'] ?? 0;
        $info['value']['default_school_id'] = (int)($info['value']['default_school_id'] ?? 0);
        ...
        $info['value']['enable_queue'] = $info['value']['enable_queue'] ?? 1;
        $info['value']['enable_class'] = $info['value']['enable_class'] ?? 1;
        $info['value']['enable_seat'] = $info['value']['enable_seat'] ?? 1;
        ...
        $info['value']['community_auto_approve'] = $info['value']['community_auto_approve'] ?? 1;
        ...
        $info['value']['secondhand_auto_approve'] = $info['value']['secondhand_auto_approve'] ?? 1;
        $info['value']['lost_found_auto_approve'] = $info['value']['lost_found_auto_approve'] ?? 1;
        $info['value']['campus_auth_auto_approve'] = $info['value']['campus_auth_auto_approve'] ?? 0;
        $info['value']['runner_apply_fee'] = $info['value']['runner_apply_fee'] ?? 0;
        $info['value']['runner_apply_pay_enable'] = $info['value']['runner_apply_pay_enable'] ?? 0;
        $info['value']['wechat_runner_apply_admin_ids'] = $info['value']['wechat_runner_apply_admin_ids'] ?? '';
        $info['value']['enable_wechat_notice'] = $info['value']['enable_wechat_notice'] ?? 0;
        $info['value']['wechat_tpl_order_pay_runner'] = $info['value']['wechat_tpl_order_pay_runner'] ?? '';
        $info['value']['wechat_tpl_runner_apply'] = $info['value']['wechat_tpl_runner_apply'] ?? '';
        $info['value']['wechat_tpl_order_accept_user'] = $info['value']['wechat_tpl_order_accept_user'] ?? '';
        $info['value']['enable_weapp_notice'] = $info['value']['enable_weapp_notice'] ?? 0;
        $info['value']['weapp_tpl_order_pay_runner'] = $info['value']['weapp_tpl_order_pay_runner'] ?? '';
        $info['value']['weapp_tpl_order_accept_user'] = $info['value']['weapp_tpl_order_accept_user'] ?? '';
        $info['value']['weapp_tpl_order_cancel_user'] = $info['value']['weapp_tpl_order_cancel_user'] ?? '';
