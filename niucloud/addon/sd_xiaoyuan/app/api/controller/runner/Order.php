...
class Order extends BaseApiController
{
    ...
    public function delivery()
    {
        ...
        if (!empty($proofImages)) {
            $updateData['proof_images'] = is_array($proofImages) ? json_encode($proofImages) : $proofImages;
        }
    ...
    public function complete()
    {
        ...
        $service->updateStatus($orderId, 50, [
            'complete_time' => time(),
        ...
        $runnerService->addIncome($runnerInfo['id'], $runnerIncome, $orderId);

        // 检查并升级接单员等级
        $levelService = new \addon\sd_xiaoyuan\app\service\core\RunnerLevelService();
        $levelService->checkAndUpgrade($runnerInfo['id']);

        // 完成订单加信誉分 +1
        if (!empty($order['member_id'])) {
            try {
                (new \addon\sd_xiaoyuan\app\service\core\CreditService())->onOrderComplete((int)$order['member_id'], 'ORDER', (int)$orderId);
            } catch (\Exception $e) {}
        }
