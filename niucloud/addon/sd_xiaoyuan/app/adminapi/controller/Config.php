...
class Config extends BaseAdminController
{
    ...
    public function get()
    {
        ...
        $result = [
            'base' => [
                ...
                'single_school' => $config['single_school'] ?? 0,
                'default_school_id' => (int)($config['default_school_id'] ?? 0),
                ...
                'runner_apply_fee' => $config['runner_apply_fee'] ?? 0,
                'runner_apply_pay_enable' => $config['runner_apply_pay_enable'] ?? 0,
            ],
            'fee' => [
                'base_fee' => $config['base_fee'] ?? 3.00,
                'buy_fee' => $config['buy_fee'] ?? 2.00,
                ...
                'commission_rate_class' => $config['commission_rate_class'] ?? null,
            ...
            'features' => [
                ...
                'enable_class' => $config['enable_class'] ?? 1,
                ...
                'campus_auth_auto_approve' => $config['campus_auth_auto_approve'] ?? 0,
            ],
            'notice' => [
                'enable_wechat_notice' => (int)($config['enable_wechat_notice'] ?? 0),
                'wechat_tpl_order_pay_runner' => $config['wechat_tpl_order_pay_runner'] ?? '',
                'wechat_tpl_runner_apply' => $config['wechat_tpl_runner_apply'] ?? '',
                'wechat_tpl_order_accept_user' => $config['wechat_tpl_order_accept_user'] ?? '',
                'wechat_runner_apply_admin_ids' => $config['wechat_runner_apply_admin_ids'] ?? '',
                'tpl_guide' => (new NoticeWechatService())->getTplGuide(),
                'enable_weapp_notice' => (int)($config['enable_weapp_notice'] ?? 0),
                'weapp_tpl_order_pay_runner' => $config['weapp_tpl_order_pay_runner'] ?? '',
                'weapp_tpl_order_accept_user' => $config['weapp_tpl_order_accept_user'] ?? '',
                'weapp_tpl_order_cancel_user' => $config['weapp_tpl_order_cancel_user'] ?? '',
                'weapp_tpl_guide' => (new NoticeWeappService())->getTplGuide(),
            ],
        ];
    ...
    public function set()
    {
        ...
        if ($type === 'notice') {
            (new NoticeWechatService())->syncToSysNotice((int)$this->request->siteId());
            (new NoticeWeappService())->syncToSysNotice((int)$this->request->siteId());
        }
