    ...
    <div class="confession-list">
        <el-card class="search-card">
            <el-form :inline="true" :model="searchForm">
                ...
                <el-form-item label="关键词">
                    <el-input v-model="searchForm.keyword" placeholder="内容/表白对象" clearable />
                </el-form-item>
        ...
        <el-card class="table-card">
            <el-table :data="tableData" v-loading="loading">
                ...
                <el-table-column prop="content" label="表白内容" min-width="200" show-overflow-tooltip />
                <el-table-column label="表白对象" width="150">
                    <template #default="{ row }">
                        <span v-if="row.target_name">{{ row.target_name }}<span v-if="row.target_info" style="color:#999;">（{{ row.target_info }}）</span></span>
...
import { getConfessionList, auditConfession, setTopConfession, deleteConfession, getAllSchools } from '../../api/admin'
...
const loadData = async () => {
    loading.value = true
    try {
        const res: any = await getConfessionList({ ...searchForm.value, page: page.value, limit: limit.value })
        if (res.code === 1) {
...
const handleAudit = async (row: any, status: number) => {
    await ElMessageBox.confirm(`确定${status === 1 ? '通过' : '拒绝'}该表白？`, '提示')
    const res: any = await auditConfession({ id: row.id, status })
    if (res.code === 1) {
...
const handleTop = async (row: any) => {
    const res: any = await setTopConfession({ id: row.id, is_top: row.is_top ? 0 : 1 })
    if (res.code === 1) {
        ElMessage.success('操作成功')
        loadData()
    }
...
const handleDelete = async (row: any) => {
    await ElMessageBox.confirm('确定删除该表白？', '提示')
    const res: any = await deleteConfession({ id: row.id })
    if (res.code === 1) {
