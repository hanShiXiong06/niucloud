        ...
        <el-card class="!border-none" shadow="never">
            <template #header>
                <div class="card-header">
                    <span class="text-lg">二手分类管理</span>
...
const loadData = async () => {
    loading.value = true
    try {
        const res: any = await request.get('sd_xiaoyuan/secondhand/category_list')
        if (res.code === 1) {
...
const handleSubmit = async () => {
    ...
    try {
        const api = isEdit.value ? 'sd_xiaoyuan/secondhand/edit_category' : 'sd_xiaoyuan/secondhand/add_category'
...
const handleDelete = async (row: any) => {
    ...
    const res: any = await request.post('sd_xiaoyuan/secondhand/del_category', { id: row.id })
    if (res.code === 1) {
