        ...
        <el-card class="!border-none" shadow="never">
            <template #header>
                <div class="card-header">
                    <span class="text-lg">失物招领分类管理</span>
...
const loadData = async () => {
    loading.value = true
    try {
        const res: any = await request.get('sd_xiaoyuan/lost_found_category/lists')
        if (res.code === 1) {
...
const handleSubmit = async () => {
    ...
    try {
        const api = isEdit.value ? 'sd_xiaoyuan/lost_found_category/edit' : 'sd_xiaoyuan/lost_found_category/add'
...
const handleDelete = async (row: any) => {
    ...
    const res: any = await request.post('sd_xiaoyuan/lost_found_category/delete', { id: row.id })
    if (res.code === 1) {
