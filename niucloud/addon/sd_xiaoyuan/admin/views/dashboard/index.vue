<template>
    <div class="dashboard-container">
        <div class="school-filter-bar">
            ...
            <el-select v-model="schoolId" placeholder="全部学校" clearable filterable style="width: 260px" @change="onSchoolFilterChange">
...
import { getDashboardOverview, getDashboardStat, getAllSchools } from '@/addon/sd_xiaoyuan/api/admin'
...
const loadSchoolOptions = async () => {
    ...
    if (res.code === 1) {
        schoolOptions.value = Array.isArray(res.data) ? res.data : []
    }
}
...
const loadOverview = async () => {
    try {
        const res = await getDashboardOverview({ school_id: schoolId.value || 0 })
        if (res.code === 1) {
...
const loadChartData = async () => {
    try {
        const res = await getDashboardStat({ type: chartType.value, school_id: schoolId.value || 0 })
        if (res.code === 1) {
...
.school-filter-bar .filter-label {
    color: #606266;
    font-size: 14px;
}
