    ...
    <div class="comment-manage">
        <el-card>
            ...
            <el-table :data="tableData" v-loading="loading">
                ...
                <el-table-column prop="confession_content" label="表白内容" min-width="150" show-overflow-tooltip />
...
import { getConfessionCommentList, auditComment, deleteComment } from '../../api/admin'
...
const loadData = async () => {
    loading.value = true
    try {
        const res: any = await getConfessionCommentList({ ...searchForm.value, page: page.value, limit: limit.value })
        if (res.code === 1) {
...
const handleAudit = async (row: any, status: number) => {
    ...
    const res: any = await auditComment({ id: row.id, type: 'confession', status })
    if (res.code === 1) {
...
const handleDelete = async (row: any) => {
    ...
    const res: any = await deleteComment({ id: row.id, type: 'confession' })
    if (res.code === 1) {
