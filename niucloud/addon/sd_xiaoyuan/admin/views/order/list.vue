    ...
    <div class="order-list-container">
        ...
        <el-card>
            <el-table :data="orderList" v-loading="loading" stripe>
                ...
                <el-table-column label="服务信息" min-width="250">
                    <template #default="{ row }">
                        <div v-if="row.receive_name || row.receive_address">
                            <span v-if="row.receive_name">{{ row.receive_name }} {{ row.receive_mobile }}</span>
                            <div class="text-gray" v-if="row.receive_address">{{ row.receive_address }}</div>
                        </div>
                        <div v-if="row.ext_data && Object.keys(row.ext_data).length">
                            <template v-for="(val, key) in row.ext_data" :key="key">
                                <div v-if="key !== 'images' && key !== 'files' && key !== 'gender_limit' && val" class="text-gray" style="margin-top:2px;">
        ...
        <el-dialog v-model="detailVisible" title="订单详情" width="800px">
            <el-descriptions :column="2" border v-if="currentOrder">
                ...
                <el-descriptions-item label="取件人">{{ currentOrder.pickup_name }} {{ currentOrder.pickup_mobile }}</el-descriptions-item>
                <el-descriptions-item label="取件地址">{{ currentOrder.pickup_address }}</el-descriptions-item>
                <el-descriptions-item label="服务信息" :span="2">
                    <div v-if="currentOrder.receive_name">{{ currentOrder.receive_name }} {{ currentOrder.receive_mobile }}</div>
                    <div v-if="currentOrder.receive_address" class="text-gray">{{ currentOrder.receive_address }}</div>
                    <div v-if="currentOrder.ext_data && Object.keys(currentOrder.ext_data).length" style="margin-top:4px;">
                        <template v-for="(val, key) in currentOrder.ext_data" :key="key">
                            <div v-if="key !== 'images' && key !== 'files' && key !== 'gender_limit' && val" style="margin-bottom:2px;">
                                <span style="font-weight:500;">{{ extLabelMap[key] || key }}：</span>{{ formatExtValue(key, val) }}
                            </div>
                        </template>
                    </div>
