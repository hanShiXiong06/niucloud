...
import { requestUserOrderSubscribe } from '../utils/subscribeMsg'
...
export function getCardList() {
    return request.get('sd_xiaoyuan/card/list')
}

export function getCardDetail(card_type: string) {
    return request.get('sd_xiaoyuan/card/detail', { card_type })
}

export function createCardOrder(card_type: string) {
    return request.post('sd_xiaoyuan/card/create_order', { card_type })
}

export function getMyCards() {
    return request.get('sd_xiaoyuan/card/my')
}

export function getCardUseLogs(card_type: string, params: any = {}) {
    return request.get('sd_xiaoyuan/card/use_logs', { card_type, ...params }, { showErrorMessage: false })
        .catch((err: any) => {
            if (err && typeof err === 'object' && err.msg !== undefined) {
                return err
            }
            return { code: 0, msg: err?.errMsg || '加载失败', data: { list: [], count: 0 } }
        })
}
...
export function createOrder(data: any) {
    return request.post('sd_xiaoyuan/order/create', data, { showErrorMessage: false })
        .then((res: any) => {
            if (res && res.code === 1) {
                requestUserOrderSubscribe()
            }
            return res
        })
        .catch((err: any) => {
            if (err && typeof err === 'object' && (err.msg !== undefined || err.code !== undefined)) {
                return err
            }
            return { code: 0, msg: err?.errMsg || '网络错误', data: null }
        })
}
