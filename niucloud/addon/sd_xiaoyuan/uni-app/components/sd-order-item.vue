<template>
    ...
    <view class="sd-order-item" :class="{ 'no-card': noCard }" @click="handleCardClick">
        ...
        <view class="order-header">
            ...
            <view class="right-info" :class="{ 'income-right': runnerIncomeMode }">
                <template v-if="runnerIncomeMode">
                    <text class="income-label">预计收益</text>
                    <text class="price">¥{{ runnerExpectedIncome }}</text>
                </template>
                <text v-else class="price">¥{{ order.actual_fee || order.total_fee }}</text>
            </view>
        </view>
        ...
        <view class="order-footer" :class="{ 'footer-actions-only': hideFooterOrderTime }">
            ...
            <view class="actions" @click.stop>
                ...
                <template v-else-if="enableDefaultActions">
                    ...
                    <view class="btn-tip" v-if="order.status === 50 && !order.is_evaluated" @click.stop="emitAction('tip', order.id)">给小费</view>
...
const runnerExpectedIncome = computed(() => {
    const o = props.order || {}
...
const extTags = computed(() => {
    ...
    if (taskType === 'GAME') {
        ...
        if (ext.class_subject) tags.push(ext.class_subject)
        if (ext.class_location) tags.push(ext.class_location)
        if (ext.class_duty_text) tags.push(ext.class_duty_text)
        if (ext.class_time) tags.push(ext.class_time)
        if (ext.class_duration) tags.push(ext.class_duration + '分钟')
