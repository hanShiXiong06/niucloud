        ...
        <view class="price-section">
            <view class="price-block">
                <view class="type-tags">
                    <text class="type-tag free" v-if="Number(goods.price) === 0">免费送</text>
                    <text class="type-tag urgent" v-else-if="Number(goods.is_top) === 1">急售捡漏</text>
                    <text class="type-tag normal" v-else>正常出售</text>
                </view>
                <view class="price-row">
                    <template v-if="Number(goods.price) > 0">
                        <text class="price">¥{{ goods.price }}</text>
                        <text class="original-price" v-if="goods.original_price">原价 ¥{{ goods.original_price }}</text>
                    </template>
                    <text v-else class="price-free">免费赠送</text>
                </view>
            </view>
...
const callSeller = () => {
    showLostFoundContactSheet({
        contact_mobile: goods.value.contact_mobile,
        contact_wechat: goods.value.contact_wechat
    })
}
