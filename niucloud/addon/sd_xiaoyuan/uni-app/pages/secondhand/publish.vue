        ...
        <view class="form-section">
            ...
            <view class="form-item">
                ...
                <view class="trade-list">
                    <view
                        class="trade-item"
                        :class="{ active: publishMode === 'normal' }"
                        @click="setPublishMode('normal')"
                    >正常出售</view>
                    <view
                        class="trade-item"
                        :class="{ active: publishMode === 'urgent' }"
                        @click="setPublishMode('urgent')"
                    >急售捡漏</view>
                    <view
                        class="trade-item"
                        :class="{ active: publishMode === 'free' }"
                        @click="setPublishMode('free')"
                    >免费送</view>
                </view>
            </view>
            ...
            <view class="form-item">
                ...
                <view class="price-input" :class="{ disabled: publishMode === 'free' }">
                    <text class="unit">¥</text>
                    <input v-model="formData.price" type="digit" placeholder="0.00" :disabled="publishMode === 'free'" />
                </view>
            </view>
            ...
            <view class="form-item">
                ...
                <input v-model="formData.contact_mobile" type="number" maxlength="11" placeholder="选填，与微信至少填一项" />
            </view>
            <view class="form-item">
                <text class="label">联系微信</text>
                <input v-model="formData.contact_wechat" placeholder="选填，与电话至少填一项" />
            </view>
        </view>
...
const publishMode = ref<'normal' | 'urgent' | 'free'>('normal')
...
onLoad((options: any) => {
    if (options?.free === '1') {
        publishMode.value = 'free'
        formData.value.price = '0'
        formData.value.is_urgent = 0
    } else if (options?.urgent === '1') {
        publishMode.value = 'urgent'
        formData.value.is_urgent = 1
    }
    if (options?.id) {
        isEdit.value = true
...
const loadDetail = async () => {
    ...
    if (res.code === 1 && res.data) {
        ...
        formData.value.price = d.price !== undefined && d.price !== null ? String(d.price) : ''
        ...
        formData.value.contact_wechat = d.contact_wechat || ''
        const pr = d.price !== undefined && d.price !== null ? Number(d.price) : NaN
        if (pr === 0) {
            publishMode.value = 'free'
        } else if (d.is_top) {
            publishMode.value = 'urgent'
        ...
        formData.value.is_urgent = publishMode.value === 'urgent' ? 1 : 0
        if (d.images) {
...
const setPublishMode = (m: 'normal' | 'urgent' | 'free') => {
    publishMode.value = m
    if (m === 'free') {
        formData.value.price = '0'
        formData.value.is_urgent = 0
        ...
        formData.value.is_urgent = 1
        if (String(formData.value.price).trim() === '0') {
...
const submit = async () => {
    ...
    const pStr = publishMode.value === 'free' ? '0' : String(formData.value.price ?? '').trim()
    if (publishMode.value !== 'free' && (pStr === '' || isNaN(Number(pStr)) || Number(pStr) < 0)) {
    ...
    if (!formData.value.contact_mobile && !formData.value.contact_wechat) {
        uni.showToast({ title: '请填写电话或微信', icon: 'none' })
        return
    }
    ...
    try {
        ...
        const data: any = {
            ...
            is_urgent: publishMode.value === 'urgent' ? 1 : 0,
