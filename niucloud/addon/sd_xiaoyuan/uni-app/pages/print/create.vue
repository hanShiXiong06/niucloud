<template>
    <view class="print-page">
        ...
        <scroll-view scroll-y class="form-content">
            ...
            <view class="form-card card-row" v-if="cardRemain > 0" @click="useCard = !useCard">
                <view class="card-content">
                    <text class="value">打印次卡（剩余{{ cardRemain }}次）</text>
                    <text class="card-tip" v-if="useCard">本单使用次卡抵扣，实付¥0.00</text>
                </view>
                <u-icon :name="useCard ? 'checkbox-mark' : 'circle'" size="20" :color="useCard ? '#ff9800' : '#ccc'"></u-icon>
            </view>
        ...
        <view class="submit-bar">
            <view class="price-info">
                ...
                <text class="price">¥{{ payPrice.toFixed(2) }}</text>
            </view>
...
import { createOrder, uploadDocument, getCardDetail } from '../../api/xiaoyuan'
...
const yinsiText = ref('')
const cardRemain = ref(0)
const useCard = ref(false)

const payPrice = computed(() => {
    const fee = parseFloat(String(totalFee.value)) || 0
    if (useCard.value && cardRemain.value > 0) {
        return 0
    }
    return fee
})
...
const loadMyCard = async () => {
    const res: any = await getCardDetail('PRINT').catch((e: any) => e)
    if (res?.code === 1 && res.data) {
        cardRemain.value = Number(res.data.remain_times || 0)
        if (cardRemain.value > 0) {
            useCard.value = true
        }
    }
}
...
const submitOrder = async () => {
    if (!receiveAddress.value) {
        uni.showToast({ title: '请选择送达宿舍', icon: 'none' })
        return
    }
    ...
    const feeText = String(totalFee.value ?? '').trim()
    if (!feeText) {
    ...
    if (isNaN(fee) || fee <= 0) {
        uni.showToast({ title: '请输入正确的服务费用', icon: 'none' })
        return
    }
    ...
    const res: any = await createOrder({
        ...
        total_fee: fee,
        use_card: useCard.value && cardRemain.value > 0 ? 1 : 0,
        remark: remark.value || '',
    ...
    if (res.code === 1) {
        ...
        if (Number(res.data.need_pay) === 0) {
            uni.showToast({ title: '下单成功', icon: 'success' })
            setTimeout(() => {
                uni.redirectTo({
                    url: `/addon/sd_xiaoyuan/pages/order/detail?id=${res.data.id}`
                })
            }, 1200)
            return
        }
