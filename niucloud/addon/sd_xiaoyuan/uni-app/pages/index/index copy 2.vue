<template>
    <view class="index-page">
        ...
        <view class="main-content" :style="{ paddingTop: (statusBarHeight + navBarHeight + 50) + 'px' }">
            ...
            <view class="menu-section" v-if="config">
                <view class="menu-item" v-for="(item, index) in menuList" :key="index" @click="handleMenuClick(item)">
                    <view class="menu-icon" :style="{ background: item.bgColor, border: item.border }">
                        <u-icon :name="item.icon" size="24" :color="item.iconColor || '#333'"></u-icon>
                    </view>
            ...
            <view class="feature-banners feature-banners-three" v-if="config">
                <view v-if="config.enable_house !== 0" class="feature-banner banner-house" @click="handleMenuClick({ url: '/addon/sd_xiaoyuan/pages/house/index' })">
                    ...
                    <view class="banner-icon">
                        <u-icon name="home-fill" size="28" color="rgba(255,255,255,0.85)"></u-icon>
                    </view>
                </view>
                <view v-if="config.enable_schedule !== 0" class="feature-banner banner-schedule" @click="handleMenuClick({ url: '/addon/sd_xiaoyuan/pages/schedule/index' })">
                    ...
                    <view class="banner-icon">
                        <u-icon name="calendar-fill" size="28" color="rgba(255,255,255,0.85)"></u-icon>
                    </view>
                </view>
                <view v-if="config.enable_group !== 0" class="feature-banner banner-group" @click="handleMenuClick({ url: '/addon/sd_xiaoyuan/pages/group/index' })">
                    ...
                    <view class="banner-icon">
                        <u-icon name="coupon-fill" size="28" color="rgba(255,255,255,0.85)"></u-icon>
                    </view>
                </view>
            </view>
            ...
            <view class="feature-banners feature-banners-three" v-if="config">
                <view v-if="config.enable_secondhand !== 0" class="feature-banner banner-secondhand" @click="handleMenuClick({ url: '/addon/sd_xiaoyuan/pages/secondhand/index' })">
                    ...
                    <view class="banner-icon">
                        <u-icon name="bag-fill" size="28" color="rgba(255,255,255,0.85)"></u-icon>
                    </view>
                </view>
                <view v-if="config.enable_lost_found !== 0" class="feature-banner banner-lost" @click="handleMenuClick({ url: '/addon/sd_xiaoyuan/pages/lost_found/index' })">
                    ...
                    <view class="banner-icon">
                        <u-icon name="search" size="28" color="rgba(255,255,255,0.85)"></u-icon>
                    </view>
                </view>
                <view v-if="config.enable_community !== 0" class="feature-banner banner-community" @click="handleMenuClick({ url: '/addon/sd_xiaoyuan/pages/community/index' })">
                    ...
                    <view class="banner-icon">
                        <u-icon name="chat-fill" size="28" color="rgba(255,255,255,0.85)"></u-icon>
                    </view>
                </view>
            </view>
        ...
        <custom-tabbar v-if="config?.enable_custom_tabbar !== 0" current="home" :isAuthed="isAuthed" ref="tabbarRef" />
...
const requireAuth = computed(() => {
    const result = config.value?.require_auth_publish !== 0
    console.log('requireAuth 计算:', config.value?.require_auth_publish, '结果:', result)
    return result
})
...
onMounted(async () => {
    ...
    try {
        ...
        if (res.code === 1 && res.data) {
            // 如果后台配置启用了装修首页，跳转到diy页面
            if (res.data.enable_diy_index === 1 || res.data.use_diy_index === 1) {
                uni.reLaunch({ url: '/addon/sd_xiaoyuan/pages/index/diy' })
                return
            }
        }
    } catch (e) {
        console.error('检查装修配置失败:', e)
    }
    ...
    loadFeatureConfig()
    // 只有启用自定义导航时才隐藏框架导航
    if (config.value?.enable_custom_tabbar !== 0) {
        uni.hideTabBar()
    }
})
...
const loadFeatureConfig = async () => {
    ...
    if (cachedConfig) {
        ...
        // 设置标题
        if (cachedConfig.site_name || cachedConfig.name) {
            uni.setNavigationBarTitle({
                title: cachedConfig.site_name || cachedConfig.name || '校园帮'
            })
        }
        console.log('从缓存加载配置:', cachedConfig)
    }
    ...
    try {
        ...
        if (res.code === 1 && res.data) {
            ...
            // 设置标题
            if (res.data.site_name || res.data.name) {
                uni.setNavigationBarTitle({
                    title: res.data.site_name || res.data.name || '校园帮'
                })
            }
            // 缓存到本地
            uni.setStorageSync('xiaoyuan_config', res.data)
            console.log('从接口加载配置:', res.data)
            console.log('require_auth_publish:', res.data.require_auth_publish)
        }
    } catch (e) {
...
const needsCertCheck = (url: string) => {
    // 如果后台配置关闭了认证要求，则不需要检查
    if (!requireAuth.value) {
        return false
    }
    // 需要认证的页面列表
    const certRequiredPages = [
        '/create',      // 所有创建/发布页面
        '/publish',     // 所有发布页面
        '/pickup',      // 快递代取
        '/order/list',  // 我的订单
    ...
    return url && certRequiredPages.some(page => url.includes(page))
}
