<template>
    <view class="settings-page">
        <!-- 接单服务类型（后端暂无 accept_types 字段，暂不使用）
        <view class="section">
...
import { getRunnerInfo, updateRunnerInfo } from '../../api/runner'
import xyUpload from '../../components/xy-upload.vue'

const runnerInfo = ref<any>({})
/* 接单类型（与 setRange accept_types 一并停用）
const settings = ref({
...
const saveSettings = async () => {
    try {
        ...
        uni.hideLoading()
        const msg = e?.msg || (typeof e?.errMsg === 'string' ? e.errMsg : '') || '保存失败，请稍后重试'
