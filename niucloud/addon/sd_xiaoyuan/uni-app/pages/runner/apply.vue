<template>
    <view class="runner-apply">
        ...
        <view class="page-state pending-panel" v-else-if="pageMode === 'pending'">
            <view class="pending-badge" :class="{ 'pay-badge': pendingNeedPay }">{{ pendingNeedPay ? '待支付' : '审核中' }}</view>
            <view class="pending-icon-wrap">
                <u-icon :name="pendingNeedPay ? 'rmb-circle' : 'clock'" size="64" :color="pendingNeedPay ? '#52c41a' : '#ff9500'"></u-icon>
            </view>
            <text class="pending-title">{{ pendingNeedPay ? '请完成入驻缴费' : '资料审核中' }}</text>
            <text class="pending-desc">{{ pendingNeedPay ? '您已提交申请，支付入驻费用后将自动通过审核。' : '您已提交接单员申请，请耐心等待审核，无需重复填写。审核通过后将自动进入接单员主页。' }}</text>
            <button class="pending-back" v-if="pendingNeedPay" @click="openPay">立即支付</button>
            <button class="pending-back" v-else @click="goBackSafe">返回</button>
        </view>
...
const pendingNeedPay = ref(false)
...
const initRunnerGate = async () => {
    ...
    try {
        ...
        if (res.code === 1 && res.data && hasRunnerRecord(res.data)) {
            ...
            if (st === 0) {
                pendingNeedPay.value = Number(d.need_pay) === 1
                if (pendingNeedPay.value && d.pay_order_id) {
                    payOrderId.value = Number(d.pay_order_id)
                }
...
const submitApply = async () => {
    ...
    try {
        ...
        if (res.code === 1) {
            if (res.data?.need_pay && res.data?.order_id) {
                payOrderId.value = res.data.order_id
                payRef.value?.open('sd_xiaoyuan_card', res.data.order_id, '/addon/sd_xiaoyuan/pages/runner/index')
                return
            }
...
const openPay = () => {
    ...
    payRef.value?.open('sd_xiaoyuan_card', payOrderId.value, '/addon/sd_xiaoyuan/pages/runner/index')
}
</script>
