<template>
    <view class="order-detail">
        ...
        <view class="info-section">
            ...
            <view class="info-item" v-if="order.status >= 20 && order.pickup_code">
            ...
            <view class="info-item" v-if="order.task_type === 'EXPRESS' && order.status >= 20 && extData.pickup_code">
                <text class="label">取件码</text>
            ...
            <view class="info-item" v-if="order.task_type === 'CLASS' && extData.class_subject">
                <text class="label">课程名称</text>
                <text class="value">{{ extData.class_subject }}</text>
            </view>
            <view class="info-item" v-if="order.task_type === 'CLASS' && extData.building_type">
                <text class="label">教学楼类型</text>
                <text class="value">{{ getClassBuildingTypeName(extData.building_type) }}</text>
            </view>
            <view class="info-item" v-if="order.task_type === 'CLASS' && extData.class_location">
                <text class="label">上课地点</text>
                <text class="value">{{ extData.class_location }}</text>
            </view>
            <view class="info-item" v-if="order.task_type === 'CLASS' && extData.class_duty_text">
                <text class="label">服务内容</text>
                <text class="value">{{ extData.class_duty_text }}</text>
            </view>
            <view class="info-item" v-if="order.task_type === 'CLASS' && extData.class_time">
                <text class="label">上课时间</text>
                <text class="value">{{ extData.class_time }}</text>
            </view>
            <view class="info-item" v-if="order.task_type === 'CLASS' && extData.class_duration">
                <text class="label">课程时长</text>
                <text class="value">{{ extData.class_duration }}分钟</text>
            </view>
        ...
        <view class="income-section">
            ...
            <view class="income-total">
                ...
                <text class="value">¥{{ formatExpectedIncome(order) }}</text>
            </view>
        </view>

        <!-- 底部操作 -->
        <view class="action-bar" v-if="order.status === 10 || (order.status >= 20 && order.status < 50)">
            <!-- 待接单状态 -->
            <button class="btn-action" v-if="order.status === 10" @click="acceptOrder">立即接单</button>
...
import { getOrderDetail, acceptOrder as acceptOrderApi, pickupOrder as pickupOrderApi, deliveryOrder as deliveryOrderApi, completeOrder as completeOrderApi, rejectOrder as rejectOrderApi } from '../../api/runner'
...
const classBuildingTypeMap: Record<string, string> = {
    'TEACHING': '教学楼', 'LIBRARY': '图书馆', 'LAB': '实验楼', 'OTHER': '其他'
}
...
const loadOrderDetail = async () => {
    ...
    try {
        ...
        if (res.code === 1 && res.data) {
            ...
            // 允许查看待接单订单（状态10），不再自动跳转
            // 接单员可以在详情页查看订单信息并决定是否接单
...
const getClassBuildingTypeName = (type: string) => classBuildingTypeMap[type] || type || '其他'
...
const formatExpectedIncome = (o: any) => {
    if (!o || typeof o !== 'object') return '0.00'
    const ri = parseFloat(String(o.runner_income ?? '').replace(/,/g, ''))
    if (Number.isFinite(ri) && ri > 0) return ri.toFixed(2)
    const fee = parseFloat(String(o.actual_fee ?? o.total_fee ?? 0).replace(/,/g, '')) || 0
    let plat = parseFloat(String(o.commission_rate ?? ''))
    if (!Number.isFinite(plat) || plat <= 0) plat = 20
    plat = Math.min(100, Math.max(0, plat))
    return (fee * (100 - plat) / 100).toFixed(2)
}
...
const acceptOrder = () => {
    uni.showModal({
        title: '确认接单',
        content: `确定要接取这个${getTaskTypeName(order.value.task_type)}订单吗？`,
        success: async (res) => {
            if (res.confirm) {
                try {
                    uni.showLoading({ title: '接单中...' })
                    const result: any = await acceptOrderApi({ id: order.value.id })
                    uni.hideLoading()
