    ...
    <view class="runner-index">
        ...
        <view class="apply-section" v-if="!runnerInfo.id || runnerInfo.status === 0 || runnerInfo.status === 2">
            ...
            <view class="apply-content pending-review-card" v-else-if="runnerInfo.status === 0">
                ...
                <button class="back-btn" @click="goBackPrev">返回</button>
            </view>
        ...
        <view class="runner-main" v-else-if="runnerInfo.status === 1">
            ...
            <view class="top-header">
                <view class="back-btn" @click="goBackPrev">
            ...
            <view class="subscribe-card" @click="onOrderPushSubscribe">
                <view class="sc-left">
                    <view class="sc-icon">
                        <u-icon name="bell-fill" size="22" color="#fff"></u-icon>
                    </view>
                    <view class="sc-text">
                        <text class="sc-title">小程序订单提醒</text>
                        <text class="sc-desc">授权后接收同校新订单推送，点一次授权一次</text>
                    </view>
                </view>
                <view class="sc-right">
                    <text class="sc-num">剩余 {{ runnerInfo.weapp_subscribe_num || 0 }} 次</text>
                    <text class="sc-btn">去授权</text>
                </view>
            </view>
...
import { requestRunnerOrderSubscribe } from '../../utils/subscribeMsg'
...
const onOrderPushSubscribe = async () => {
    const platform = uni.getSystemInfoSync().uniPlatform
    if (platform !== 'web' && platform !== 'mp-weixin') {
        uni.showToast({ title: '仅H5和小程序端可用', icon: 'none' })
        return
    }
    const left = await requestRunnerOrderSubscribe()
    if (left > 0) {
        runnerInfo.value.weapp_subscribe_num = left
        uni.showToast({ title: '授权成功，剩余' + left + '次', icon: 'none' })
        return
    }
    uni.showToast({ title: '未授权或授权失败', icon: 'none' })
}
const goBackPrev = () => {
    ...
    uni.redirectTo({ url: '/addon/sd_xiaoyuan/pages/user/index' })
}
