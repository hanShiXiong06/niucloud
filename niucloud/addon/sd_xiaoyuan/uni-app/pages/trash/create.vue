<template>
    ...
    <view class="trash-page" v-if="isFeatureEnabled">
        <!-- 顶部背景 -->
...
const { config, isFeatureEnabled, loadConfig } = useFeatureCheck('enable_trash')
...
const loadDefaultFee = async () => {
    ...
    if (Number.isFinite(fee) && fee > 0 && !tipFee.value) {
        tipFee.value = String(fee)
    }
}
const yinsiText = ref('')
...
const onAddrSelect = (address: any) => {
    if (address && (address.type === 'pickup' || !address.type)) {
...
const submitOrder = async () => {
    if (!pickupAddress.value) {
        uni.showToast({ title: '请选择取垃圾地点', icon: 'none' })
        return
    }
    if (!trashDesc.value) {
