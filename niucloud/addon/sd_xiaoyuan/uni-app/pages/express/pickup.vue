<template>
    ...
    <view class="express-page" v-if="isFeatureEnabled">
            ...
            <view class="package-list">
                ...
                >
                    <view class="package-info">
                        ...
                        <text class="price">¥{{ parseFloat(item.price || 0).toFixed(2) }}</text>
                    </view>
        ...
        <view class="form-card card-row" v-if="cardRemain > 0" @click="useCard = !useCard">
            <view class="card-content">
                <text class="value">快递次卡（剩余{{ cardRemain }}次）</text>
                <text class="card-tip" v-if="useCard">本单使用次卡抵扣，实付¥0.00</text>
            </view>
            <u-icon :name="useCard ? 'checkbox-mark' : 'circle'" size="20" :color="useCard ? '#52c41a' : '#ccc'"></u-icon>
        </view>
...
import { getExpressStations, getPackagePrices, createOrder, getCardDetail } from '../../api/xiaoyuan'
...
const loadMyCard = async () => {
    const res: any = await getCardDetail('EXPRESS').catch((e: any) => e)
