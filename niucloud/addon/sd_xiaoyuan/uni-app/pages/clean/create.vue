...
const loadDefaultFee = async () => {
    ...
    if (Number.isFinite(fee) && fee > 0 && !formData.value.fee) {
        formData.value.fee = String(fee)
    }
}
...
const submitOrder = async () => {
    ...
    try {
        ...
        const res: any = await createOrder({
            ...
            receive_lng: selectedAddress.value?.lng || '',
            receive_lat: selectedAddress.value?.lat || '',
            appointment_time: formData.value.appointment_time,
            goods_name: '清洁服务',
            task_desc: formData.value.task_desc,
