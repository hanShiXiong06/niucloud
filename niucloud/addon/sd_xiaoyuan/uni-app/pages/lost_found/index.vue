<template>
    <view class="lostfound-page">
        ...
        <!-- 正常内容 -->
        <view v-if="isFeatureEnabled" class="lostfound-body">
        <!-- 顶部统计与搜索 -->
            ...
            <swiper class="urgent-swiper" circular autoplay interval="4000" next-margin="40rpx">
                <swiper-item v-for="item in urgentList" :key="item.id">
                    <view class="urgent-card" @click="goToDetail(item.id)">
                        <image class="thumb" :src="firstMediaImageSrc(item.images)" mode="aspectFill"></image>
                        <view class="info">
                            <text class="name text-ellipsis">{{ item.title }}</text>
                            <view class="loc text-ellipsis">
                                <u-icon name="map-fill" size="12" color="#999"></u-icon>
                                <text class="loc-txt">{{ item.lost_address }}</text>
                            </view>
                            <view class="reward-badge" v-if="item.reward > 0">跑腿费 ¥{{ item.reward }}</view>
                        </view>
                    </view>
                </swiper-item>
            </swiper>
            ...
            <view class="item-list">
                <view class="item-card" v-for="item in itemList" :key="item.id" @click="goToDetail(item.id)">
                    ...
                    <view class="card-main">
                        <image class="card-img" :src="firstMediaImageSrc(item.images)" mode="aspectFill"></image>
                        <view class="card-info">
                            ...
                            <view class="meta-row">
                                <view class="location text-ellipsis">
                                    ...
                                    <text class="loc-txt">{{ item.lost_address || '地点未知' }}</text>
                                </view>
                            </view>
                            <view class="tags-row">
                                <view class="tag">{{ getCategoryName(item.category) }}</view>
                                <view class="tag reward" v-if="item.reward > 0 && currentType === 'LOST'">跑腿费¥{{ item.reward }}</view>
                            </view>
                        </view>
                    </view>
...
const getSchoolIdFromStorage = () => {
    ...
    const sidRaw = s.id ?? s.school_id
    const n = parseInt(String(sidRaw ?? 0), 10)
    return Number.isFinite(n) && n > 0 ? n : 0
}

const loadStats = async () => {
    const params: any = { type: currentType.value }
    const sid = getSchoolIdFromStorage()
    if (sid > 0) {
...
const loadUrgent = async () => {
    const sid = getSchoolIdFromStorage()
    const urgentParams: any = { page: 1, limit: 5, type: 'LOST', is_urgent: 1, status: 1 }
    if (sid > 0) {
        urgentParams.school_id = sid
    }
    const res: any = await getLostFoundList(urgentParams)
    if (res.code === 1) {
...
const handleContact = (item: any) => {
    showLostFoundContactSheet(item)
}
</script>
