<template>
    <view class="confession-page">
        ...
        <!-- 正常内容 -->
        <view v-if="isFeatureEnabled" class="confession-main">
        <!-- 头部背景与搜索 -->
...
const loadConfessions = async (refresh = false) => {
    ...
    try {
        ...
        // 只有当选择了具体类型时才传递type参数，"全部"不传type
        ...
        if (res.code === 1) {
            ...
            if (refresh) {
                confessionList.value = list
            } else {
                confessionList.value = [...confessionList.value, ...list]
            }
            ...
            console.error('获取表白墙列表失败:', res.msg)
            uni.showToast({ title: res.msg || '加载失败', icon: 'none' })
        }
    } catch (e) {
        console.error('加载表白墙列表异常:', e)
...
const loadMore = () => {
    if (!loading.value && hasMore.value) loadConfessions()
}
