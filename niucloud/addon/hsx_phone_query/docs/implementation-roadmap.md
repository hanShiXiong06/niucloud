...
- 已形成“应用服务 → 查机领域网关 → 服务商适配器”的调用链：
  - 应用服务只负责订单、收费/退款和接口审计日志。
  - `PhoneQueryGatewayService` 负责解析并调用适配器。
  - `GkdtPhoneQueryProvider` 独立处理爱查签名、服务 ID、`code` 参数及 `code=200` 成功协议。
  - `PathPhoneQueryProvider` 独立处理 3023 路径、Header `key`、动态查询参数及 `code=0` 成功协议。
- 3023 不再把所有接口写死为 `sn`：
  - `/apple/*` 等设备接口默认 `sn`。
  - `/imei/*` 使用 `imei`。
  - `/item/*` 使用 `barcode`。
  - `/ip/*` 使用 `ip`。
  - `/phone/*` 使用 `phone`。
- 3023 默认接口升级为 HTTPS，并保持严格 SSL 校验。
- 爱查旧配置 `service_id_query` 自动迁移为 `gkdt_query`，无需重填密钥。
- 爱查文档中带 `?key=` 的地址会去除固定查询串，服务 ID 始终由查询项目映射注入，避免重复参数。
- 接口映射页已允许明确选择 `sn / imei / code / barcode / ip / phone`，不再隐藏真实协议。
- 10101、10104、10109 的下划线字段已补齐中文标签和关键摘要展示。
- 已新增 `tests/provider_contract_smoke.php`，覆盖爱查签名、10101 服务 ID、3023 Header、动态参数推断和旧配置迁移。
