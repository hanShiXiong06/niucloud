<template>
    <view class="page">
        ...
        <template v-else-if="report">
            ...
            <view class="metric-grid"><view v-for="item in metrics" :key="item.label" class="metric"><text>{{ item.label }}</text><strong>{{ item.value }}{{ item.unit }}</strong></view></view>
...
const metrics = computed(() => { const s = snapshot.value.summary || {}; return [
    { label: '回收入库', value: s.recycle_in_count || 0, unit: ' 台' }, { label: '销售出库', value: s.sale_count || 0, unit: ' 台' },
    { label: '当前库存', value: s.stock_count || 0, unit: ' 台' }, { label: '动销率', value: Number(s.turnover_rate || 0).toFixed(2), unit: '%' },
    { label: '销售额', value: `¥${Number(s.sale_amount || 0).toFixed(2)}`, unit: '' }, { label: '销售毛利', value: `¥${Number(s.sale_profit || 0).toFixed(2)}`, unit: '' }
] })
