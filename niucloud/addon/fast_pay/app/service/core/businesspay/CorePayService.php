...
use addon\fast_pay\app\service\core\businesspay\CorePayEventService;
use addon\fast_pay\app\service\core\businesspay\CoreRefundService;
...
class CorePayService extends BaseCoreService
{
     ...
     */
    public function findPayInfoByTrade(int $site_id, $trade_type, $trade_id)
    {
        $where = array(
            ['site_id', '=', $site_id],
            ['trade_type', '=', $trade_type],
     ...
     * @throws ModelNotFoundException
     */
    public function getInfoByTrade(int $site_id, $trade_type, $trade_id, string $channel)
    {
     ...
     * @throws ModelNotFoundException
     */
    public function pay($site_id, $trade_type, $trade_id, $type, $channel, string $openid, string $return_url = '', string $quit_url = '', string $buyer_id = '', string $voucher = '',string $business_id='')
    {
        ...
        $pay_result = $this->pay_event->init($site_id, $channel, $type,$business_id)->pay($out_trade_no, $money, $body, $return_url, $quit_url, $buyer_id, $openid ?? '', $voucher);
    ...
    {
        $pay = $this->findPayInfoByTradeId($site_id, $trade_type, $trade_id);
        if ($pay->isEmpty()) {
            //创建新的支付单据
            $pay = $this->createByTrade($site_id, $trade_type, $trade_id);
        }
        if ($pay['status'] == PayDict::STATUS_FINISH) throw new PayException('PAY_SUCCESS');
//        if ($pay['status'] == PayDict::STATUS_CANCEL) throw new PayException('PAY_IS_REMOVE');
        if ($pay['status'] == PayDict::STATUS_ING || $pay['status'] == PayDict::STATUS_CANCEL) {
    ...
    public function findPayInfoByTradeId(int $site_id, string $trade_type, int $trade_id)
    {
        $where = array(
            ['site_id', '=', $site_id],
            ['trade_type', '=', $trade_type],
...
//        if ($pay['status'] == PayDict::STATUS_FINISH) throw new PayException('DOCUMENT_IS_PAID');//当前单据已支付
    ...
    public function check(int $site_id, array $data)
    {
        ...
        if ($pay['status'] == PayDict::STATUS_CANCEL) throw new PayException('PAY_IS_REMOVE');//单据已取消
        //查询第三方支付单据
