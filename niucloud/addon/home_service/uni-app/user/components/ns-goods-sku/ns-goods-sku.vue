			...
			<view class="rounded-t-[20rpx] overflow-hidden bg-[#fff] p-[32rpx] relative">
				...
				<view class="flex mb-[58rpx]">
					...
					<view class="rounded-[8rpx] overflow-hidden">
						<up-image width="204rpx" height="204rpx" :src="img(goodsDetail.detail?.sku_image)" @click="imgListPreview(goodsDetail.detail?.sku_image)" model="aspectFill">
							<template #error>
					...
					<view class="mb-[20rpx]">
						<view class="flex flex-wrap">
							<template v-for="(item,index) in goodsDetail.skuList" :key="index">
								<view class="box-border max-w-[680px] truncate text-[24rpx] px-[15rpx] text-center  leading-[42rpx] mr-[20rpx] mb-[20rpx] border-1 border-solid rounded-[8rpx] border-[#888]"
...
const confirm = ()=> {
	// 检测是否登录
	if (!userInfo.value) {
		useLogin().setLoginBack({ url: '/addon/home_service/user/pages/goods/detail', param: { sku_id: goodsDetail.value.sku_id } })
		return false
	}
    ...
    uni.setStorage({
		...
		success: () => {
			redirect({ url: '/addon/home_service/user/pages/order/payment', param: { id: goodsDetail.value.goods_id } })
		}
	});
	closeFn();
}
...
const goodsPrice = computed(() =>{
	let price = "0.00";
	if(Object.keys(goodsDetail.value).length && Object.keys(goodsDetail.value.goods).length  && getToken() && goodsDetail.value.member_price != goodsDetail.value.price){
