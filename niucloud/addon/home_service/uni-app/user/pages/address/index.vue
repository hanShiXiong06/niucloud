					...
					<view class="flex flex-col card-template">
						<view class="flex-1 line-feed mr-[20rpx]" @click="selectAddress(item)">
							...
							<view
								...
								{{ item.full_address }}</view>
						</view>
						<view class="flex justify-between pt-[26rpx]">
							<view class="flex items-center text-[26rpx] leading-none" @click.stop="setDefault(index)">
								<text class="iconfont !text-[26rpx] mr-[10rpx]"
									:class="{ 'iconduigou text-primary': item.is_default, 'iconcheckbox_nol': !item.is_default }"></text>
								设为默认
							</view>
							<view class="flex">
								...
								<view @click.stop="deleteAddressFn(index)" class="ml-[40rpx] text-[26rpx]">
									<text
										class="nc-iconfont nc-icon-shanchu-yuangaizhiV6xx shrink-0 text-[26rpx] mr-[4rpx]"></text>
									删除
								</view>
							</view>
						</view>
			...
			<view class="w-full footer">
				<view
					...
					<button hover-class="none"
						class="!bg-[var(--primary-color)] !text-[#fff] !rounded-lg  text-[#fff] h-[80rpx] leading-[80rpx] rounded-[100rpx] text-[26rpx] font-500"
						@click="addAddress">{{ t('addHomeAddress') }}</button>
				</view>
			</view>
		</scroll-view>
	</view>
...
<script setup lang="ts">
	...
	let topTabbarData = topTabarObj.setTopTabbarParam({ title: '地址列表', topStatusBar: { textColor: '#333', rollBgColor: "#ffffff" } })
	...
	const is_order_address = ref(0)
	onLoad((option : any) => {
		is_order_address.value = option.is_order_address
	})
	onShow(() => {
		getAddressListFn()
	})
	...
	const addAddress = () => {
		redirect({ url: '/addon/home_service/user/pages/address/address_edit', param: { is_order_address: is_order_address.value } })
	}

	const editAddressFn = (id : number) => {
		redirect({ url: '/addon/home_service/user/pages/address/address_edit', param: { id } })
	}
