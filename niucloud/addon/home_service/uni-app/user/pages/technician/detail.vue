        ...
        <template v-for="(subItem,index) in technicianDetail.goods" :key="index">
          <view class="flex" :class="{'mt-[20rpx]':index}">
            <view class="w-[160rpx] h-[160rpx]" @click="toLink(subItem.goods_info.goods_id)">
              <up-image class="rounded-[10rpx] overflow-hidden" width="160rpx" height="160rpx" :src="img(subItem.goods_info.cover_thumb_small ? subItem.goods_info.cover_thumb_small : '')" model="aspectFill">
                <template #error>
            ...
            <view class="flex-1 ml-[20rpx]  flex flex-col justify-between">
              ...
              <text class="text-[var(--primary-color)] text-[28rpx] price-font">
                ...
                <image v-if="priceType(subItem.goods_info) == 'member_price'" class="h-[24rpx] ml-[4rpx] w-[60rpx]" :src="img('addon/home_service/VIP.png')" mode="heightFix" />
              </text>
            </view>
          </view>
        </template>
...
import { getTechnicianDetail } from '@/addon/home_service/user/api/technician'
