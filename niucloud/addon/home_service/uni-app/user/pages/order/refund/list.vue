				...
				<view class="mb-[30rpx] bg-[#fff] p-[24rpx] rounded" v-for="(item,index) in list" :key="index">
					<view @click="toLink(item)">
						<view class="text-[#222] flex justify-between items-center  text-[28rpx] pb-[24rpx] border-0 border-b border-slate-200 border-solid">
							<view class="text-gray-500">{{item.orderMain?.order_no}}</view>
							<view class="text-[var(--primary-color)]" :style="{color: item.status == 'refund_completed' ?'!text-[var(--primary-color)]' :item.status == 'refund_refuse' ? '#ff0000' : '#ff9513' }">{{item.status_name}}</view>
						</view>
						<view class="order-goods-item flex mt-[30rpx]" v-if="item.order_item && item.order_item[0]">
							<view class="w-[160rpx] h-[160rpx] flex-2">
								<up-image class="rounded-[10rpx] overflow-hidden" width="160rpx" height="160rpx" :src="img(item.order_item[0].item_image_thumb_small ? item.order_item[0].item_image_thumb_small : '')" model="aspectFill">
									<template #error>
							...
							<view class="ml-[20rpx] flex flex-1 flex-col justify-between">
								<view >
									<text class="text-[28rpx] text-item h-[80rpx] leading-[40rpx]">{{ item.order_item[0].item_name }}</text>
								</view>
								<view class="text-[28rpx]">
									<text class="text-[20rpx] price-font text-[var(--price-text-color)]  font-bold">￥</text>
									 
									<text class="text-[40rpx] price-font text-[var(--price-text-color)]  font-bold">{{ Number(( item.order_item[0].price)).toString().split('.')[0] }}</text>
									<text
										class="price-font text-[22rpx] text-[var(--price-text-color)]  font-bold">.{{ Number(( item.order_item[0].price)).toFixed(2).split('.')[1] }}</text>
					...
					<view class="flex  mt-[30rpx]">
						<text class="ml-auto mr-[20rpx] text-xs" v-if="item.order_item && item.order_item[0] && item.order_item[0].price">{{ t('actualPayment') }}：<text class="text-sm text-[var(--price-text-color)] price-font">￥{{ item.order_item[0].price }}</text></text>
...
<script setup lang="ts">
	...
	import { getRefundList, cancelRefund, getRefundStatus} from '@/addon/home_service/user/api/refund';
	...
	const toLink = (data) => {
		redirect({ url: '/addon/home_service/user/pages/order/refund/detail', param: { refund_no : data.refund_id } })
	}
