<template>
	<view class="bg-[#ffffff] min-h-screen flex flex-col items-center justify-center px-[40rpx]" :style="themeColor()">
		...
		<top-tabbar :data="topTabbarData" scrollBool="1" :isBack="true" :customBack="()=>{ redirect({ url: '/addon/home_service/user/pages/index'}) }" />
		<!-- #endif -->
		<!-- 成功图标 -->
		...
		<view class="text-[24rpx] text-[#999] text-center mb-[60rpx] leading-[1.5]" v-if="submitStatus == -1">
		...
		<view class="w-full footer bg-[#fff]">
			<view
				...
				<button hover-class="none"
					...
					@click="goBack">{{submitStatus == -1 ? '重新申请' : '返回'}}
				</button>
			</view>
		</view>
	</view>
...
<script setup lang="ts">
	...
	let topTabbarData = topTabarObj.setTopTabbarParam({ title: '审核状态', topStatusBar: { textColor: '#333' } })
