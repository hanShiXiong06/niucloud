<template>
	<view :style="themeColor()" class="withdraw-page">
			...
			<view class="sidebar-margin pt-[var(--top-m)]">
				...
				<view class="card-template money-card">
					...
					<view class="flex items-end justify-between mt-[16rpx]">
						...
						<view class="text-[24rpx] leading-[36rpx] withdrawal-btn" @click="allMoney">{{ t('allMoney') }}
						</view>
					</view>
					...
					<view class="amount-input-container">
						...
						<input type="digit"
							class="h-[76rpx] leading-[76rpx] pl-[10rpx] flex-1 font-500 text-[54rpx] bg-[#fff] amount-input"
							v-model="applyData.apply_money" maxlength="7"
							:placeholder="applyData.apply_money?'':(t('enterAmount'))" placeholder-class="apply-price"
							:adjust-position="false" @blur="onAmountBlur" />
						<text v-if="Number(serviceMoney)"
							class="text-[24rpx] text-[var(--text-color-light6)] mr-[20rpx]">{{ t('fee') }}{{ serviceMoney }}</text>
				...
				<view class="mt-[20rpx] card-template payment-methods-card">
					...
					<view class="payment-method-item mb-[20rpx]"
						...
						<view>
							<image class="h-[60rpx] w-[60rpx] align-middle"
								:src="img('static/resource/images/member/apply_withdrawal/wechat.png')"
								mode="widthFix" />
						</view>
					...
					<!-- 提现到银行卡 -->
					<view class="payment-method-item" v-if="config.transfer_type.includes('bank')"
				...
				<view class="fixed bottom-[0] tab-bar left-0 right-0 px-[var(--sidebar-m)] bg-[var(--page-bg-color)]">
					<view
						class="fixed bottom-[0] tab-bar left-0 right-0 px-[var(--sidebar-m)] bg-[var(--page-bg-color)]">
						<button
		...
		<view class="h-[100vh] w-[100vw] bg-[var(--page-bg-color)] overflow-hidden"
			v-if="config.is_open == 0 && !pageLoading">
