<template>
	<view :style="themeColor()">
		<template v-if="!loading">
			...
			<view v-if="detail" class="bg-[#f7f7f7] min-h-screen overflow-hidden">
				...
				<view class="flex items-center m-[25rpx] justify-between p-[25rpx] bg-[#fff] rounded-lg"
									v-if="detail.member?.nickname">
									<view class="flex items-center">
										<view>
											<u--image :src="img(detail.member?.headimg)" width="50rpx" height="50rpx"
				...
				<view class="text-[26rpx] mb-[30rpx] mt-[15rpx]" v-if="detail?.order_status == 'in_service'">
					<view class="text-[#fff] flex justify-center">
						<view v-for="(item,index) in formattedElapsedTime.split(':')" :key="index">
							<text
								class="bg-[#FF000D] text-[45rpx] font-bold mx-[15rpx] px-[14rpx] py-[8rpx] rounded-lg">
								{{item}}
							</text>
							<text v-if="index != formattedElapsedTime.split(':').length - 1"
								class="text-[#FF000D] text-[45rpx] font-bold">:</text>
						</view>
					</view>
				...
				<view class="bg-[#fff] my-[30rpx] mx-[30rpx] rounded-[16rpx] p-[30rpx] mt-[0rpx]"
					...
					<view class="flex justify-between mb-[10rpx]">
						<view class="flex items-center">
							<view class="flex items-center rounded-l-[50rpx] rounded-r-[6rpx] mr-[20rpx]"
								>
					...
					<view class="flex mt-[20rpx]">
						...
						<view class="flex items-center justify-between flex-1">
							...
							<view>
								<view
									class="flex-1 flex items-center justify-end text-[26rpx] text-[var(--store-bg-one)] w-[100rpx]"
				...
				<view class="mt-[30rpx]">
					<view class="bg-[#fff] mx-[30rpx] p-[30rpx] mt-[30rpx] rounded-lg">
						...
						<view class="bg-[#F6F6F6] p-[30rpx] text-[24rpx] my-[24rpx] rounded-[10rpx]"
							v-if="detail.member_message">
							{{ t('remarkPrefix') }}{{detail.member_message}}
						</view>
				...
				<view class="mt-[30rpx]">
					<view class="bg-[#fff] mx-[30rpx] p-[30rpx] mt-[30rpx] rounded-lg">
						<view class="flex justify-between">
								...
								<view class="flex justify-between items-end text-[26rpx]">
									<view class="text-[32rpx] font-bold leading-[35rpx]  text-[var(--price-text-color)] price-font"><text
											class="!text-[22rpx]">￥</text>{{ detail.store_sum_commission }}</view>
								</view>
							</view>
						<view v-if="detail?.order_status == 'in_service'"
							...
							<view class="price-font text-[24rpx] text-[#999999] font-bold"><text
						...
						<view v-if="detail?.order_status == 'in_service'"
							...
							<view class=" price-font"><text class="!text-[24rpx]">￥</text>{{ detail.technician_additional_commission }}</view>
						</view>
				...
				<view v-if="detail.order_status_info?.action && detail.order_status_info?.action?.length"
					class="flex z-2 justify-end items-center bg-[#fff] fixed left-0 right-0 bottom-0 min-h-[100rpx] px-1 flex-wrap pb-ios py-[15rpx]">
					<button v-for="(btnItem,btnIndex) in detail.order_status_info.action" :key="btnIndex"
						@click.stop="handleOrderAction(detail, btnItem.key)"
						:class="btnItem.key == 'action_save_check'  ? 'text-white bg-[var(--store-bg-one)]' : (btnItem.key == 'action_dispatch' ? 'bg-[#00CB7A] text-[#fff]' : 'bg-[var(--store-bg-one)] border-1 border-solid border-[var(--store-bg-one)]')  "
		...
		<!-- <order-popup :show="systemStore.popup.visible" :order="systemStore.popup.order"
			:action-key="systemStore.popup.actionKey" @close="systemStore.closePopup()"
...
<script setup lang="ts">
	...
	import { getOrderDetail } from '@/addon/home_service/store/api/order'
