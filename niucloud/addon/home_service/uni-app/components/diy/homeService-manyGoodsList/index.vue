			...
			<template v-if="diyComponent.headStyle == 'style-3'">
				<template v-if="diyComponent.source == 'custom'">
					<view v-for="(item,index) in diyComponent.list" :key="index"
						...
						<text
							:class="['text-[28rpx] mt-[16rpx]', {'font-500 text-[var(--primary-color)]' : index == cateIndex  }]">{{ item.title }}</text>
					</view>
				</template>
				<template v-else-if="diyComponent.source == 'goods_category'">
					<view class="pr-[40rpx] inline-flex flex-col items-center justify-center"
						@click="changeGoodsCategory({ category_id : 0 })">
						<text
							:class="['text-[28rpx] mt-[16rpx]', {'font-500 text-[var(--primary-color)]': currentCategoryId == 0}]">全部</text>
					</view>
					<view v-for="(item,index) in goodsCategoryListData" :key="index"
						...
						<text
							:class="['text-[28rpx] mt-[16rpx]', {'font-500 text-[var(--primary-color)]' : currentCategoryId == item.category_id}]">{{ item.category_name }}</text>
					</view>
				</template>
			</template>
			<template v-else>
				<view v-for="(item, index) in diyComponent.list" class="scroll-item"
					...
					@click="changeCateIndex(item, index)">
					<view class=" flex justify-center flex-col items-center relative px-[15rpx]"
						v-if="diyComponent.headStyle == 'style-1'">
						<view class="name">{{ item.title }}</view>
						<image v-if="index == cateIndex"
							class="block w-[50rpx] h-[50rpx] absolute z-index-99 top-[22rpx]"
							:src="img('/addon/home_service/diy/index/manygoodslist_active.png')" mode=""></image>
					</view>
				</view>
			</template>
		</scroll-view>
		<diy-home-goods-list class="many-goods-list-body" :swiperList="swiperList"
			v-if="goodsValue && diyComponent.list.length" :value="goodsValue" :cateIndex="cateIndex"></diy-home-goods-list>
	</view>
</template>
...
<script setup lang="ts">
	...
	import diyHomeGoodsList from '@/addon/home_service/components/diy/homeService-goods-list/index.vue';
	import { getGoodsCategoryList } from '@/addon/home_service/api/diy';
	...
	if(diyComponent.value?.list && diyComponent.value.list.length && diyComponent.value?.list[cateIndex.value].bgUrl){
		diyComponent.value?.list[cateIndex.value]?.bgUrl.split(',').forEach((item, index) => {
			swiperList.value.push(img(item))
		})
	}
	...
	const changeCateIndex = (item : any, index : any, isFirst : any = false) => {
		...
		if(diyComponent.value.list[cateIndex.value].bgUrl){
			swiperList.value = []
			diyComponent.value?.list[cateIndex.value]?.bgUrl.split(',').forEach((item, index) => {
				swiperList.value.push(img(item))
			})
			...
			swiperList.value = []
			swiperList.value.push(img('static/resource/images/diy/figure.png'))
		}
	...
	const refreshGoodsList = (obj : any) => {
		goodsValue.value = {
			...
			source: diyComponent.value.list[cateIndex.value].source,
			bgUrl: diyComponent.value.bgUrl,
			num: diyComponent.value.list[cateIndex.value].num,
