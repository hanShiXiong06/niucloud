			...
			<mescroll-body ref="mescrollRef" :down="{ use: false }" @init="mescrollInit" @up="getListFn">
				<view v-if="!loading && filteredOrderList.length > 0">
					<view class="order-list">
						<view v-for="(item, index) in filteredOrderList" :key="index" @click="redirect({url:'/addon/home_service/technician/pages/order/detail',param:{order_id:item.order_id}})"
...
<script setup lang="ts">
	...
	import { getDayOrderStat, getMonthOrderStat, getMonthorderPage } from '@/addon/home_service/technician/api/account';
