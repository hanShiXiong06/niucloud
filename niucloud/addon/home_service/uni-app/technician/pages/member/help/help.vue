<template>
  <view class="bg-[var(--page-bg-color)] min-h-screen overflow-hidden component-class" :style="themeColor()"  v-if="!loading">
    <!-- 顶部导航栏 -->
    <u-navbar :title="t('helpCenter')" :autoBack="true" bgColor="#ffffff" :placeholder="true" :left-arrow="true">
    </u-navbar>
    ...
    <view class="px-[30rpx] py-[20rpx]">
        ...
        <view class="bg-[#ffffff] rounded-[20rpx] overflow-hidden">
          ...
          <template v-if="defaultCategory">
            ...
            <view v-for="(item, index) in defaultCategory.help_list" :key="item.help_id" 
              ...
              <image 
                :src="img('addon/home_service/technician/arrow-right.png')" 
                class="w-[24rpx] h-[24rpx]" 
                mode="aspectFit"
                @error="handleImageError($event, 'arrow-right')"
              ></image>
            </view>
          </template>
        ...
        <view class="bg-[#ffffff] rounded-[20rpx] overflow-hidden">
          ...
          <template v-if="nonDefaultCategories.length > 0">
            ...
            <view class="flex border-b border-[#f0f0f0] overflow-x-auto whitespace-nowrap">
              <view 
                v-for="category in nonDefaultCategories" 
              ...
              >
                ...
                <!-- 选中状态下划线 -->
                <view 
                  v-if="selectedCategoryId === category.category_id" 
                  class="absolute bottom-0 left-1/4 right-1/4 h-[4rpx] bg-[var(--technician-bg-one)] rounded-full"
                ></view>
              </view>
            </view>
            ...
            <template v-if="selectedCategoryData">
              <template v-if="selectedCategoryData.help_list && selectedCategoryData.help_list.length > 0">
      ...
      <view class="mt-[40rpx] mb-[60rpx]">
        ...
        >
            ...
            :src="img('addon/home_service/technician/phone.png')"
...
import { getTechnicianHelpData } from '@/addon/home_service/technician/api/member'
...
const redirectToProblemDetail = (helpId: number) => {
 redirect({
        url: '/addon/home_service/technician/pages/member/help/detail',
        param: { helpId: helpId }
    })
}
