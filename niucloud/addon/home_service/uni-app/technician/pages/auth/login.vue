<template>
    <view class="w-screen h-screen flex flex-col  " :style="themeColor()" v-if="type">
        ...
        <view class="mx-[60rpx]">
            ...
            <!-- <view class="h-[34rpx] text-right text-[24rpx] text-[var(--technician-bg-one)] leading-[34rpx] mt-[20rpx] " @click="toResetpwd">{{ type == 'username' ? t('resetpwd') : '' }}</view> -->
            <view class="mt-[106rpx]">
                <view v-if="configStore.login.agreement_show" class="flex items-center mt-[20rpx] py-[14rpx]  mb-[20rpx]" @click.stop="agreeChange">
                    <u-checkbox-group @change="agreeChange">
                        <u-checkbox activeColor="var(--technician-bg-one)" :checked="isAgree" shape="circle" size="24rpx" />
                    </u-checkbox-group>
                    <view class="text-[24rpx] flex items-center flex-wrap leading-[30rpx]">
                        <text>{{ t('agreeTips') }}</text>
                        <text @click.stop="redirect({ url: '/app/pages/auth/agreement?key=technician_privacy' })" class="text-[var(--technician-bg-one)]">《{{ t('privacyAgreement') }}》</text>
                        <text>{{ t('and') }}</text>
                        <text @click.stop="redirect({ url: '/app/pages/auth/agreement?key=technician_register' })" class="text-[var(--technician-bg-one)]">《{{ t('userAgreement') }}》</text>
                    </view>
                </view>
        ...
        <uni-popup ref="popupRef" type="dialog">
            <view class="bg-[#fff] flex flex-col justify-between w-[600rpx] min-h-[280rpx] rounded-[var(--rounded-big)] box-border px-[35rpx] pt-[35rpx] pb-[8rpx] relative">
                ...
                <view class="flex items-center mb-[20rpx] mt-[20rpx] py-[20rpx]" @click.stop="agreeChange">
                    <view class="text-[26rpx] flex items-center flex-wrap">
                        <text>{{ t('agreeTips') }}</text>
                        <text @click.stop="redirect({ url: '/app/pages/auth/agreement?key=technician_privacy' })" class="text-[var(--technician-bg-one)]">《{{ t('privacyAgreement') }}》</text>
                        <text>{{ t('and') }}</text>
                        <text @click.stop="redirect({ url: '/app/pages/auth/agreement?key=technician_register' })" class="text-[var(--technician-bg-one)]">《{{ t('userAgreement') }}》</text>
                ...
                <view>
                    <view class="w-[100%] flex justify-center bg-[var(--technician-bg-one)] h-[70rpx] leading-[70rpx] text-[#fff] text-[26rpx] border-[0] font-500 rounded-[50rpx]" @click="dialogConfirm">同意并登录</view>
...
import smsCode from '@/addon/home_service/technician/components/sms-code/sms-code.vue'
