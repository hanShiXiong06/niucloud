	...
	<div class="main-container min-h-[60vh]" ref="mainContainerRef" v-loading="loading">
		<div v-if="!loading">
			...
			<el-card class="box-card !border-none mt-[15px]" shadow="never" v-if="orderData.buyer">
				...
				<div
					class="box-border px-[12px] py-[10px] rounded-[4px] border-solid border-[#ebebeb] border-[1px] inline-flex">
					<div class="mr-[10px]  w-[65px] h-[65px] flex items-center justify-center">
						<img class="max-w-[65px] max-h-[65px]" v-if="orderData.buyer.headimg"
							:src="img(orderData.buyer.headimg)" alt="">
			...
			<el-card class="box-card !border-none mt-[15px]" shadow="never">
				...
				<el-form label-width="120px" ref="basicFormRef" :model="createData" :rules="formRules"
					class="page-form">
								...
								<template #default="{ row }">
									<div class="flex items-inherit">
										<div class="min-w-[70px] h-[70px] flex items-center justify-center">
											<el-image v-if="row.sku_image" class="w-[70px] h-[70px]"
												:src="img(row.sku_image)" fit="contain">
												<template #error>
										...
										<div class="ml-2 flex-1 flex flex-col items-baseline">
											<div v-if="row.goods && row.goods.goods_name"
												class="truncate text-[14px] w-[450px] leading-[1.4]">
												{{ row.goods.goods_name }}</div>
											<div class="text-[#999] text-[12px] leading-[1] mt-[2px]">{{ row.sku_name }}
											</div>
											<div v-if="row.not_support_delivery"
												class="bg-[#fef0f0] mt-[5px] text-[red] px-[5px] rounded-[3px] text-[11px] flex items-center justify-center h-[20px]">
												该服务不支持当前所选配送方式</div>
											<div v-if="row.manjian_info && Object.keys(row.manjian_info).length"
												class="mt-[5px] flex items-center cursor-pointer"
												@click.stop="manjianOpenFn(row.manjian_info)">
												<div
													class="bg-[var(--el-color-primary-light-9)] text-[var(--el-color-primary)] rounded-[3px] text-[10px] flex items-center justify-center w-[44px] h-[18px] mr-[3px]">
													满减送</div>
												<span
													class="text-[11px] text-[#999]">{{row.manjian_info.manjian_name}}</span>
											</div>
										</div>
									</div>
								</template>
					...
					<el-form-item label="配送方式" v-if="false">
						<el-radio-group v-model="deliveryType" @change="deliveryTypeChange"
							v-if="delivery_type_list.length">
							<el-radio v-for="(item,index) in delivery_type_list" :key="index"
					...
					<el-form-item label="上门时间" prop="reserve_service_time">
						<div v-if="selectedServiceTime" class="flex items-center cursor-pointer"
							@click="selectServiceTime()">
					...
					<el-form-item label="上门地址">
						<div class="flex">
							<div v-if="Object.keys(addressData).length"
								class="temp-wrap overflow-hidden relative flex flex-col mr-[10px] w-[240px] px-[12px] py-[15px] border-[1px] border-[#eee] border-solid rounded-[5px] cursor-pointer">
								...
								<span
									class="truncate text-[13px] leading-[1] mt-[5px]">{{addressData.full_address}}</span>
								<div class="sub-temp bg-[rgba(0,0,0,.4)] absolute left-[0] right-[0] top-[0] bottom-[0] items-center justify-center text-[#fff] hidden"
									@click="replaceAddress">
									更换地址
								</div>
							</div>
							<div @click="addAddressFn"
								class="flex flex-col items-center justify-center mr-[10px] w-[200px] px-[12px] py-[15px] border-[1px] border-[#eee] border-solid rounded-[5px] cursor-pointer">
					...
					<el-form-item label="选择自提点" v-if="false">
						<div v-if="Object.keys(storeData).length"
							class="temp-wrap w-[350px] overflow-hidden relative flex flex-col px-[12px] py-[15px] border-[1px] border-[#eee] border-solid rounded-[5px] cursor-pointer">
							<div class="flex item-center">
								<span
									class="text-[15px] leading-[1] truncate max-w-[150px]">{{storeData.store_name}}</span>
						...
						<div v-else @click="selectStoreFn"
							class="flex flex-col items-center justify-center mr-[10px] w-[200px] px-[12px] py-[15px] border-[1px] border-[#eee] border-solid rounded-[5px] cursor-pointer">
							<span class="iconfont iconVector-3 text-[36px]"></span>
					...
					<el-form-item label="选择地址" v-if="false">
						<div class="flex">
							<div v-if="addressListData.length && Object.keys(localData).length"
								class="temp-wrap overflow-hidden relative flex flex-col mr-[10px] w-[240px] px-[12px] py-[15px] border-[1px] border-[#eee] border-solid rounded-[5px] cursor-pointer">
								...
								<span
									class="truncate text-[13px] leading-[1] mt-[5px]">{{localData.full_address}}</span>
								<div class="sub-temp bg-[rgba(0,0,0,.4)] absolute left-[0] right-[0] top-[0] bottom-[0] items-center justify-center text-[#fff] hidden"
									@click="replaceAddress">
									更换地址
								</div>
							</div>
							<div v-else-if="addressListData.length" @click="replaceAddress"
								class="flex flex-col items-center justify-center mr-[10px] w-[200px] px-[12px] py-[15px] border-[1px] border-[#eee] border-solid rounded-[5px] cursor-pointer">
								<span class="iconfont iconVector-3 text-[36px]"></span>
					...
					<el-form-item label="优惠券" v-if="couponNum">
						<div v-if="Object.keys(couponData).length" class="flex items-baseline cursor-pointer"
							@click="selectCoupon()">
							<span
								class="text-primary text-[14px] mr-[5px] truncate max-w-[200px]">{{couponData.title}}</span>
							<span class="text-[12px] text-[#999]">点击更换</span>
						</div>
			...
			<el-card class="box-card !border-none mt-[15px]" shadow="never">
				...
				<div class="flex flex-col">
					...
					<div class="flex items-center h-[32px] mb-[5px]" v-if="parseFloat(orderData.basic.coupon_money)">
						...
						<span
							class="text-[14px] text-primary">-￥{{parseFloat(orderData.basic.coupon_money).toFixed(2)}}</span>
					</div>
					<div class="flex items-center h-[32px] mb-[5px]"
						v-if="parseFloat(orderData.basic.manjian_discount_money)">
						<span class="w-[120px] text-[14px] text-right text-[#666] mr-[10px]">满减优惠</span>
						<span
							class="text-[14px] text-primary">-￥{{parseFloat(orderData.basic.manjian_discount_money).toFixed(2)}}</span>
					</div>
					<div class="flex items-center h-[32px] mb-[5px]">
						<span class="w-[120px] text-[14px] text-right text-[#666] mr-[10px]">合计</span>
						<span
							class="text-[18px] font-500 text-primary">￥{{parseFloat(orderData.basic.order_money).toFixed(2)}}</span>
					</div>
					<div class="flex items-center h-[32px] mb-[5px]"
						v-if="payList&&payList.length && parseFloat(orderData.basic.order_money)">
						<span class="w-[120px] text-[14px] text-right text-[#666] mr-[10px]">支付方式</span>
						<div class="text-[14px]">
							<el-radio-group v-model="currPay">
								<el-radio v-for="(item,index) in payList" :key="index"
									:label="item.key">{{item.name}}</el-radio>
							</el-radio-group>
						</div>
					</div>
				...
				<div class="bg-[#fff] h-[60px] flex items-center justify-center fixed bottom-0 z-10 right-[15px] border-[0] border-t-[1px] border-[#ececec]"
					:style="settleStyle()">
		...
		<el-dialog align-center v-model="payResultDialogVisible" width="425" @close="payResultCloseFn"
			:close-on-press-escape="false" :close-on-click-modal="false" center :show-close="false">
			<div class="flex flex-col items-center w-[100%] pb-[30px]">
				...
				<div class="flex items-baseline mt-[10px] text-[28px] font-500">
					...
					<span class="text-[22px]"
						v-if="orderData.basic">{{parseFloat(orderData.basic.order_money).toFixed(2)}}</span>
				</div>
				<div v-if="payErrorMsg" class="mx-[10px] text-center text-[15px] mt-[30px] text-[#999]">{{payErrorMsg}}
				</div>
				<div class="flex items-center mt-[60px]">
					<el-button class="w-[120px] !h-[35px]" round @click="toOrder()">查看订单</el-button>
					<el-button type="primary" class="!ml-[25px] w-[120px] !h-[35px]" round
						@click="payResultDialogVisible = false">再下一单</el-button>
				</div>
			</div>
		</el-dialog>
