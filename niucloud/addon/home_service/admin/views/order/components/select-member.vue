<template>
    <el-dialog v-model="dialogMemberVisible" :title="t('请选择会员')"  width="850px">
        <el-form :inline="true" :model="memberList.searchParam" ref="searchFormRef" @submit.prevent>
            <el-form-item  prop="keyword">
                <el-input v-model.trim="memberList.searchParam.keyword" class="!w-[240px]" :placeholder="t('请输入会员编码/昵称/手机号搜索')" clearable  @clear="getMemberListFn()" />
            </el-form-item>
            <el-form-item>
        ...
        <el-table :data="memberList.data" v-loading="memberList.loading" size="large">
            ...
            <el-table-column prop="point" :label="t('积分')" min-width="80" />
            <el-table-column prop="balance" :label="t('余额')" min-width="80" />
            <el-table-column :label="t('操作')" fixed="right" min-width="50" align="right">
                <template #default="{ row }">
                    <el-button type="primary" link @click="confirmEvent(row)">{{ t('选择') }}</el-button>
                </template>
            </el-table-column>
        </el-table>
