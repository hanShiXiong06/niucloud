		...
		<el-dialog v-model="showDialog" :title="t('goodsSelectPopupSelectGoodsDialog')" width="1000px"
			...
			<div class="table w-[100%]" v-loading="goodsTable.loading">
				<div class="table-head flex items-center bg-[#f5f7f9] py-[8px]">
					...
					<div class="w-[50%]">服务信息</div>
					<div class="w-[20%]">服务价格</div>
				</div>
							...
							<div class="flex items-center cursor-pointer w-[50%] pr-[25px]">
								...
								<div class="ml-2 flex flex-col items-start">
									<span :title="row.goods_name"
										class="multi-hidden leading-[1.4]">{{ row.goods_name }}</span>
								...
								<div class="flex items-center cursor-pointer w-[50%]">
									...
									<div class="ml-2">
										<span :title="item.sku_name || row.goods_name"
											class="multi-hidden leading-[1.4]">{{ item.sku_name || row.goods_name }}</span>
										<span class="text-primary text-[12px]">{{ row.goods_type_name }}</span>
									</div>
								</div>
			...
			goodsTable.data.forEach((item : any) => {
				if (prop.mode == 'spu') {
					...
					item.skuList.forEach((skuItem : any) => {
						...
						selectGoods[replacePrefix + skuItem.sku_id].goods_name = item.goods_name; // 多规格模式，要增加服务名称、服务类型，后续还会增加，满足不同业务场景
		...
		if (prop.mode == 'spu') {
			...
			if (isSelect) {
				row.skuList.forEach((item, index) => {
					...
					selectGoods[replacePrefix + item.sku_id].goods_name = row.goods_name; // 多规格模式，要增加服务名称、服务类型，后续还会增加，满足不同业务场景
