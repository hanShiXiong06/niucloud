...
} from '@/addon/home_service/api/goods'
...
const save = () => {
    const sku_list: any = []
    let verify = true // 用于校验价格是否小于等于零或大于服务原价
            ...
            tableData.member_level.forEach((levelItem: any, levelIndex) => {
                if (verify) {
                    ...
                    if (parseFloat(item[`level_${levelItem.level_id}`]) > parseFloat(item.price)) {
                        verify = false
                        ElMessage.error(`[${item.sku_name}][${levelItem.level_name}]的指定价格不能大于服务原价`)
                    }
                }
            })
            sku_list.push(obj)
