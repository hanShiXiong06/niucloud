<template>
	<div class="main-container" v-loading="loading">
		<el-card class="card !border-none mb-[15px]" shadow="never">
			<el-page-header :content="t('editTechnician')" :icon="ArrowLeft" @back="back" />
		</el-card>
		...
		<el-card class="box-card !border-none" shadow="never">
			<el-form :model="formData" label-width="180px" ref="formRef" :rules="formRules" class="page-form">
				...
				<el-form-item :label="t('store')" prop="store_id">
					<div class="!w-[214px] border-[1px] border-[#e4e4e4] border-solid px-[11px] h-[30px]">
						{{ formData.store_name || '' }}
					</div>
					<el-button v-if="formData.audit_status == 0" class="ml-[10px]" type="primary" @click="storeDialog = true">{{t('chooseStore')}}</el-button>
				</el-form-item>
				<el-form-item :label="t('member')" prop="member_id">
					<div class="!w-[214px] border-[1px] border-[#e4e4e4] border-solid px-[11px] h-[30px] leading-[30px]" :class="formData.member_nickname ? 'text-[#000]' : 'text-[#999]'">
						{{ formData.member_nickname || t('chooseMemberPlaceholder') }}
					</div>
					<!-- <el-button class="ml-[10px]" type="primary" @click="dialogMemberVisible = true">{{t('chooseMember')}}</el-button> -->
				</el-form-item>
				<!-- <el-form-item :label="t('level')" prop="level_id">
					<el-select v-model="formData.level_id" :placeholder="t('levelPlaceholder')" clearable>
				...
				<el-form-item :label="t('certificate')" prop="certificate">
					<upload-image v-model="formData.certificate" :limit="6" />
				</el-form-item>
				<el-form-item :label="t('applyReason')" prop="notes">
					<el-input v-model.trim="formData.notes" clearable :placeholder="t('applyReasonPlaceHolder')" type="textarea"
	...
	<el-dialog v-model="storeDialog" title="请选择门店" width="720px">
		<el-card class="box-card !border-none my-[10px] table-search-wrap" shadow="never">
			<el-form :inline="true" :model="storeSearchForm" ref="searchStoreFormRef">
				<el-form-item :label="t('stroeKeyword')" prop="keyword">
					<el-input v-model.trim="storeSearchForm.keyword" :placeholder="t('stroeKeywordPlaceholder')" />
				</el-form-item>
				<el-form-item>
...
<script lang="ts" setup>
	...
	const setFormData = async (id : number = 0) => {
		...
		if(data.store){
			...
			formData.store_name = data.store.contact_name || ''
		}
	...
	const formRules = computed(() => {
		return {
			...
			// notes: [{ required: true, message: t('applyReasonPlaceHolder'), trigger: 'blur' }],
	...
	const onSave = async (formEl : FormInstance | undefined,status) => {
		...
		if(!formData.audit_remark){
			ElMessage.error(t('applyTextPlaceholder'))
			return;
		}
		// 执行表单验证
