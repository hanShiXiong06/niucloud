    ...
    public function handle($params)
    {
        ...
        if (in_array($trade_type, [ OrderDict::ORDER_TYPE_ORDER ])) {
            $order_info = ( new Order() )->where([ [ 'order_id', '=', $params[ 'trade_id' ] ] ])->field('order_id')->with([ 'item' => function($query) {
                $query->field('site_id, order_id, item_name, item_type, item_image, price, num')->append([ 'item_type_name', 'item_image_thumb_mid' ]);
            } ])->findOrEmpty()->toArray();
            $info = [];
            $item = [];
            if (!empty($order_info)) {
                if ($order_info[ 'item' ]) {
                    foreach ($order_info[ 'item' ] as $k => $v) {
                        ...
                        $item[ $k ][ 'item_sub_name' ] = $v[ 'item_type_name' ];//订单项副标题
