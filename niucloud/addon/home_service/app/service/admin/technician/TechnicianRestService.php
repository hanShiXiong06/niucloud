<?php
// +----------------------------------------------------------------------
// | Niucloud-admin 企业快速开发的saas管理平台
// +----------------------------------------------------------------------
// | 官方网址：https://www.niucloud.com
// +----------------------------------------------------------------------
// | niucloud团队 版权所有 开源版本可自由商用
// +----------------------------------------------------------------------
// | Author: Niucloud Team
// +----------------------------------------------------------------------

namespace addon\home_service\app\service\admin\technician;

use addon\home_service\app\service\core\technician\CoreTechnicianRestService;
use core\base\BaseAdminService;

/**
 * 师傅休息服务层
 * Class TechnicianRestService
 * @package app\service\admin\technician
 */
class TechnicianRestService extends BaseAdminService
{
    public function __construct()
    {
        parent::__construct();
    }

    /**
     * 获取师傅休息记录
     * @param array $where
     * @return array
     */
    public function getTechnicianRestList(array $where)
    {
        $where['site_id'] = $this->site_id;
        return (new CoreTechnicianRestService())->getTechnicianRestList($where);
    }

    /**
     * 设置师傅休息
     * @param array $data
     * @return bool
     */
    public function setTechnicianRest(array $data)
    {
        $data['site_id'] = $this->site_id;
        return (new CoreTechnicianRestService())->setTechnicianRest($data);
    }


    /**
     * 师傅休息 月数据 处理
     * @param array $data
     * @return bool
     */
    public function getRestMonthstats(array $data)
    {
        $data['site_id'] = $this->site_id;
        return (new CoreTechnicianRestService())->getRestMonthstats($data);
    }


}
