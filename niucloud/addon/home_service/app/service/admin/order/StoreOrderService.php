 ...
 *门店订单业务
 */
class StoreOrderService extends BaseAdminService
{
    ...
    {
        $field = 'store_id,site_id,technician_commission,technician_additional_commission,store_commission,store_additional_commission,store_id,order_name,order_id,site_id,member_id, order_no, technician_id,order_status, create_time, order_money';
        $order = 'create_time desc';
        $join_where = [];
        if ($where['member_search'] != '') $join_where[] = ['member.member_no|member.nickname|member.username|member.mobile', 'like', "%" . $where['member_search'] . "%"];
        if ($where['technician_name'] != '') $join_where[] = ['technician.real_name', 'like', "%" . $where['technician_name'] . "%"];
        $search_model = $this->model
            ->where([['order.site_id', '=', $this->site_id], ['order.store_id', '=', $where['store_id']]])
            ->where($join_where)
            ->withSearch(['order_no', 'order_name'], $where)->field($field)
            ->withJoin([
                'store' => ['store_name', 'mobile', 'store_id'],
                'member' => ['nickname', 'member_id'],
                'technician' => ['real_name', 'id'],
            ], 'left')
            ->order($order)
