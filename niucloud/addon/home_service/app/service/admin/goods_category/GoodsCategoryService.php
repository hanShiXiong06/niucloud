...
namespace addon\home_service\app\service\admin\goods_category;
...
class GoodsCategoryService extends BaseAdminService
{
    ...
    public function changeSort(int $category_id, int $sort = 0)
    {
        $this->model->where([['category_id', '=', $category_id], ['site_id', '=', $this->site_id]])->update(['sort' => $sort]);
        return true;
    }
    ...
    {
        $list = $this->model->where([['site_id', '=', $this->site_id], ['level', '=', 1]])->field("category_id, category_name, image,is_show,is_settled, intro")->order("create_time desc")->select()->toArray();
        if ($list && $type == 2) {
            foreach ($list as $key => $val) {
                $list[$key]['children'] = $this->model->where([['site_id', '=', $this->site_id], ['pid', '=', $val['category_id']]])->field("category_id, category_name, image, intro")->order("create_time desc")->select()->toArray();
            }
        }
    ...
    public function getWhereList($where = [])
    {
        $list = $this->model->where([['site_id', '=', $this->site_id]])
            ->where($where)
            ->field("category_id, category_name, image,is_show,is_settled")
