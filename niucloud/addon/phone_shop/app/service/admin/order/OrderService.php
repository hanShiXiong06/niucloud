...
use addon\phone_shop\app\model\delivery\Local;
...
class OrderService extends BaseAdminService
{
    ...
    public function getDetail(int $order_id)
    {
        ...
        $info = $this->model->where([ [ 'order_id', '=', $order_id ], [ 'site_id', 'in', "{$this->site_id},100005"] ])->field($field)
            ->with(
                [
                    'order_goods' => function($query) {
    ...
    public function editPrice($data)
    {
        ...
        $order_goods_list = $order_goods_model->where([ [ 'order_id', '=', $order_id ], [ 'site_id', '=', $this->site_id ] ])->select();
        $goods_money = 0;
        Db::startTrans();
        try {
