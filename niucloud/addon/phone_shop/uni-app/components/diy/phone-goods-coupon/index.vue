<template>
	<x-skeleton :type="skeleton.type" :loading="skeleton.loading" :config="skeleton.config"
		v-if="couponList.length > 0 && Object.keys(couponList).length > 0">
		<view :style="warpCss" class="overflow-hidden">
			...
			<view v-if="diyComponent.style == 'style-1'" class="coupon-wrap style-1 relative">
				<scroll-view scroll-x="true" class="coupon-list"
					:style="{ 'background-image': 'url(' + img('addon/phone_shop/diy/goods_coupon/style1_bg2.png') + ')', 'background-size': '100%', 'background-repeat': 'no-repeat' }">
					<view class="coupon-class">
						...
						<block v-else>
							<view v-for="(item, index) in couponList" :key="index"
								class="box-border pt-[14rpx] pl-[44rpx] pr-[44rpx] inline-flex items-center justify-between relative w-[100%] h-[130rpx]"
								:style="{ 'background-image': 'url(' + img('addon/phone_shop/diy/goods_coupon/style1_bg4.png') + ')', 'background-size': '100%', 'background-repeat': 'no-repeat' }">
								<view class="flex price-font text-[var(--price-text-color)] items-baseline">
									<text class="text-[36rpx] mt-[16rpx] mr-[4rpx]">￥</text>
									<text
										class="text-[85rpx] font-bold max-w-[170rpx] truncate">{{ parseFloat(item.price) }}</text>
								</view>
								<view
								...
								<view class="w-[270rpx]">
									<view class="flex items-center mt-[auto]">
										<text
											class="rounded-[4rpx] bg-[#fff3f0] text-[#f2333c] border-[2rpx] border-solid border-[#f2333c] text-[22rpx] px-[6rpx] pb-[4rpx] pt-[6rpx] flex items-center justify-center whitespace-nowrap">{{ item.type_name }}</text>
										<text
				...
				<view
					...
					:style="{ 'background-image': 'url(' + img('addon/phone_shop/diy/goods_coupon/style1_bg.png') + ')', 'background-size': '100% 130rpx', 'background-repeat': 'no-repeat' }">
					...
					<text v-if="diyComponent.btnText" @click="toLink('/addon/phone_shop/pages/coupon/list')"
						class="bg-[#fff] flex leading-[52rpx] justify-center text-[#FF4142] text-[24rpx] min-w-[100rpx] px-[24rpx] box-border h-[48rpx] coupon-buy-btn">{{ diyComponent.btnText }}</text>
				</view>
			</view>
			...
			<view v-else-if="diyComponent.style == 'style-2'" class="coupon-wrap style-2 relative">
				...
				<view
					...
					:style="{ 'background-image': 'url(' + img('addon/phone_shop/diy/goods_coupon/style2_bg.png') + ')', 'background-size': '290rpx 170rpx', 'background-repeat': 'no-repeat' }">
					<text class="text-[32rpx] text-[#fff] font-bold">{{ diyComponent.couponTitle }}</text>
					<text class="text-[22rpx] text-[#fff] mt-[14rpx]">{{ diyComponent.couponSubTitle }}</text>
					<text v-if="diyComponent.btnText" @click="toLink('/addon/phone_shop/pages/coupon/list')"
						class="bg-[#fff] text-[#FF4142] text-[24rpx] min-w-[100rpx] px-[24rpx] box-border h-[48rpx] leading-[52rpx] text-center coupon-buy-btn mt-auto">{{ diyComponent.btnText }}</text>
				</view>
			</view>
...
import { getShopCouponComponents } from '@/addon/phone_shop/api/coupon'
