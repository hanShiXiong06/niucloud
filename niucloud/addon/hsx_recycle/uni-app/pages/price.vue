<template>
    <view class="page">
        ...
        <view class="quick-actions">
            ...
            <view class="action-item" @click="redirect({ url: '/addon/hsx_recycle/pages/order/list' })">
                <up-icon name="order" size="24" color="#007AFF"></up-icon>
                <text>我的订单</text>
            </view>
            <view class="action-item" @click="redirect({ url: '/addon/hsx_recycle/pages/payment/index' })">
...
const _getPaymentList = async () => {
    ...
    if (res.code === 1) {
        ...
        if (res.data.length === 0) {
            uni.navigateTo({ url: '/addon/hsx_recycle/pages/payment/index' });
        }
    }
};
...
onMounted(async () => {
    ...
    if (!userInfo.value) {
        useLogin().setLoginBack({ url: "/addon/hsx_recycle/pages/index" });
    }
