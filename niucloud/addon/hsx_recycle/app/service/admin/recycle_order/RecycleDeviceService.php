...
use addon\hsx_recycle\app\model\RecycleDeviceLog;
...
class RecycleDeviceService extends BaseAdminService
{
    ...
    protected function createReturnOrderForAllDevices(int $orderId, string $remark = '',): bool
    {
        ...
        try {
            ...
            // 创建退货订单
            $returnOrderService = new \addon\hsx_recycle\app\service\admin\RecycleReturnOrderService();
            $returnOrderData = [
                'site_id' => $this->site_id,
                'order_id' => $orderId,
    ...
    public function getImeiInfo(string $imei)
    {
        ...
        try {
            // 使用新的设备查询服务
            $queryService = new \addon\hsx_recycle\app\service\admin\DeviceQueryService();
    ...
    private function getImeiInfoFallback(string $imei)
    {
        try {
            // 使用新的设备查询服务
            $deviceQueryService = new \addon\hsx_recycle\app\service\admin\DeviceQueryService();
