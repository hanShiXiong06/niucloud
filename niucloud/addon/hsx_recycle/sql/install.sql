...
CREATE TABLE `{{prefix}}recycle_order` (
  ...
  `order_source` varchar(20) NOT NULL DEFAULT 'customer' COMMENT '下单来源：customer-客户下单，agent-代下单',
  `agent_uid` int NOT NULL DEFAULT 0 COMMENT '代下单操作人ID',
  `agent_name` varchar(50) NOT NULL DEFAULT '' COMMENT '代下单操作人',
  `agent_mobile` varchar(20) NOT NULL DEFAULT '' COMMENT '代下单人联系方式',
...
CREATE TABLE `{{prefix}}recycle_device` (
  ...
  `cancel_amount` decimal(10,2) NOT NULL DEFAULT 0 COMMENT '撤销回收退款金额',
  `cancel_time` int NOT NULL DEFAULT 0 COMMENT '撤销回收时间',
  `cancel_uid` int NOT NULL DEFAULT 0 COMMENT '撤销回收操作人ID',
  `cancel_reason` varchar(500) NOT NULL DEFAULT '' COMMENT '撤销回收原因',
  ...
  `sale_destination` varchar(20) NOT NULL DEFAULT 'mall' COMMENT '销售去向：mall-商城销售，peer-同行出货，hold-暂存',
  ...
  `refurbishment_required` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否需要整备：0-否，1-是',
  `refurbishment_assignee_uid` int NOT NULL DEFAULT 0 COMMENT '建议整备负责人ID',
  `refurbishment_assignee_name` varchar(100) NOT NULL DEFAULT '' COMMENT '建议整备负责人姓名',
  `refurbishment_reason` varchar(500) NOT NULL DEFAULT '' COMMENT '整备原因',
  `refurbishment_items` text COMMENT '建议整备项目JSON',
  `refurbishment_estimated_cost` decimal(10,2) NOT NULL DEFAULT 0 COMMENT '预估整备成本',
  `remark` varchar(255)   DEFAULT '' COMMENT '备注',
  `create_at` int not null DEFAULT 0 COMMENT '创建时间',
  `update_at` int not null DEFAULT 0 COMMENT '更新时间',
  `export_status` tinyint(1) NOT NULL DEFAULT 0 COMMENT '导出状态：0-未导出，1-已导出',
