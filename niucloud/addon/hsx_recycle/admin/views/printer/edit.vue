<template>
  <PremiumTheme class="printer-edit-wrap">
...
const fetchPrinterInfo = async () => {
  try {
    ...
    const res = await getPrinterInfo(printerId.value);
    
    if (res.code === 1 && res.data) {
        // 填充表单数据
        form.printer_id = res.data.printer_id;
        form.printer_name = res.data.printer_name || '';
        form.brand = res.data.brand || 'xpyun';
        form.sn = res.data.sn || '';
        form.user_key = res.data.user_key || '';
        form.user_name = res.data.user_name || '';
        form.type = res.data.type || 'label';
      
    } else {
      ElMessage.error('获取打印机信息失败');
      goBack();
    }
  } catch (error) {
