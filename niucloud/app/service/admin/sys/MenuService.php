...
//
     ...
     * @param string $addon 用于检测插件筛选
     * @param int $is_button
     * @param int $show_mount
     * @return mixed
     * @throws DataNotFoundException
     * @throws DbException
     * @throws ModelNotFoundException
     */
    public function getSiteMenuListByMenuKeys(int $site_id, array $menu_keys, string $app_type, int $is_tree = 0, $addon = 'all', $is_button = 1, $show_mount = 0)
    {
        sort($menu_keys);
        $cache_name = 'menu' . md5(implode('_', $menu_keys)) . $app_type . $is_tree . $addon . $site_id;
        $menu_list = cache_remember(
            $cache_name,
            function () use ($site_id, $menu_keys, $app_type, $is_tree, $addon) {
                $where = [
                ...
                if($addon != 'all'){
                    ...
                    foreach ($addons as $item_addon) {
                        $delete_menu = $addon_loader->load(['addon' => $item_addon, 'app_type' => $app_type])['delete'] ?? [];
                        if (!empty($delete_menu) && is_array($delete_menu)) $delete_menu_addon[] = $delete_menu;
                    }
                }
        ...
        // 合并挂载菜单
        if ($show_mount) {
            $mount_menu_list = $this->getSiteMountMenuListMenuKeys($menu_keys, $app_type, $is_tree, $addon);
            $menu_list = array_merge($menu_list, $mount_menu_list);
        }
    ...
    /**
     * 通过菜单menu_key获取挂载菜单（店铺端）
     */
    public function getSiteMountMenuListMenuKeys(array $menu_keys, string $app_type, int $is_tree = 0, $addon = 'all')
    {
        $site_id = $this->site_id;
        sort($menu_keys);
        $cache_name = 'mount_menu' . md5(implode('_', $menu_keys)) . $app_type . $is_tree . $addon . $site_id;
        $menu_list = cache_remember(
            $cache_name,
            function () use ($site_id, $menu_keys, $app_type, $is_tree, $addon) {
                $where = [
        ...
        foreach ($menu_list as $key => $item) {
            foreach ($item['mount_key'] as $mount_key) {
                $item['parent_key'] = $mount_key;
                $mount_menu_list[] = $item;
            }
        }

        return $mount_menu_list;
     ...
     * 获取所有接口菜单（店铺端）
     */
    public function getSiteAllMenuList($app_type = '', $status = 'all', $is_tree = 0, $is_button = 0, $show_mount = 0)
    {
        $site_id = $this->site_id;
        $cache_name = 'menu_api_' .$app_type.'_'. $status . '_' . $is_tree . '_' . $is_button . '_'. $site_id;
        $menu_list = cache_remember(
            $cache_name,
            function () use ($site_id, $status, $is_tree, $is_button, $app_type) {
                $where = [
//                ['menu_type', 'in', [0,1]]
                    ['app_type', '=', $app_type],
                ];
        ...
        // 合并挂载菜单
        if($show_mount){
            $mount_menu_list = $this->getSiteAllMountMenuList($app_type, $status, $is_tree, $is_button);
            $menu_list = array_merge($menu_list, $mount_menu_list);
        }
     ...
     * 获取所有挂载菜单（店铺端）
     */
    public function getSiteAllMountMenuList($app_type = '', $status = 'all', $is_tree = 0, $is_button = 0)
    {
        $site_id = $this->site_id;
        $cache_name = 'mount_menu_api_' .$app_type.'_'. $status . '_' . $is_tree . '_' . $is_button . '_'. $site_id;
        $menu_list = cache_remember(
            $cache_name,
            function () use ($site_id, $status, $is_tree, $is_button, $app_type) {
                $where = [
